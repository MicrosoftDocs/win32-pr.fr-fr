...
Pour compiler les ressources générées par le compilateur de message, utilisez la commande suivante :
...
L’exemple suivant montre le fichier d’en-tête généré par le compilateur pour le fichier texte de message ci-dessus. Incluez le fichier d’en-tête dans votre projet.
...
L’exemple suivant montre comment utiliser la fonction [**ReportEvent**](/windows/desktop/api/Winbase/nf-winbase-reporteventa) pour écrire les événements définis dans le fichier texte de message ci-dessus.
...
Avant d’exécuter cet exemple, enregistrez le fournisseur dans le registre. Pour plus d’informations sur les paramètres du Registre, consultez [sources d’événements](event-sources.md). Ajoutez « MyEventProvider » en tant que clé de Registre sous la clé suivante :
...
| EventMessageFile     | SZ de REG \_    | *chemin d’accès* \\provider.dll |
| ParameterMessageFile | SZ de REG \_    | *chemin d’accès* \\provider.dll |
| TypesSupported       | \_valeur DWORD reg | 0x00000007           |
