...
Lorsque vous utilisez la session multimédia pour la lecture, une application peut rechercher et modifier la vitesse de lecture comme suit :

-   Pour effectuer une recherche, appelez [**IMFMediaSession :: Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start) et spécifiez la position de recherche.
-   Pour modifier la vitesse de lecture, utilisez l’interface [**IMFRateControl**](/windows/desktop/api/mfidl/nn-mfidl-imfratecontrol) comme décrit dans la section contrôle de la [Vitesse](rate-control.md).
-   Pour obtenir les images vidéo mises à jour lors de la recherche, définissez le taux de lecture sur zéro avant d’appeler [**IMFMediaSession :: Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start). Cette opération est appelée un *nettoyage*. (Consultez [Comment effectuer un nettoyage](how-to-perform-scrubbing.md).)

Toutefois, pour créer la meilleure expérience utilisateur, vous devez prendre en compte les comportements suivants :

-   La recherche est asynchrone et la session de média met en file d’attente toutes les demandes de recherche sur une file d’attente FIFO. Si vous envoyez plusieurs requêtes de recherche, l’interface utilisateur peut être exécutée avant l’état de lecture réel. Par exemple, supposons que votre application implémente une barre de recherche. Si l’utilisateur fait glisser la barre de recherche vers l’avant et vers l’arrière, il peut y avoir un décalage lors de l’exécution des recherches vers l’avant. Pendant qu’une recherche est en cours, vous devez mettre en cache les demandes de recherche de l’utilisateur. Lorsque l’opération de recherche en cours est terminée, envoyez la demande de recherche la plus récente de l’utilisateur et ignorez les autres.
-   Certaines transitions de taux ne sont pas autorisées dans certains États de transport. Par exemple, le passage de la lecture en avant à la lecture inversée n’est pas autorisé lors de la lecture. Les transitions prises en charge sont décrites dans [**IMFRateControl ::**](/windows/desktop/api/mfidl/nf-mfidl-imfratecontrol-setrate)sesupport. Les changements de taux sont également asynchrones.

La classe d’assistance suivante peut être utilisée pour gérer les demandes de recherche et les modifications de taux. Lorsqu’une opération asynchrone est en attente, la classe met en cache toutes les demandes de recherche ou de modification de taux. Lorsque l’opération en cours se termine, la classe soumet la requête la plus récente, le cas échéant. La classe gère également l’état du transport afin d’éviter les changements de taux non valides.

Voici la déclaration de la classe PlayerSeeking.
...
Voici l’implémentation de la classe PlayerSeeking.
