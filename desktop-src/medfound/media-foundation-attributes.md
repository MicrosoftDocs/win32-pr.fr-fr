---
description: Attributs Media Foundation
ms.assetid: 445fc879-3c9e-409d-8d05-ecd1ff9afc19
title: Attributs Media Foundation
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 29e9283562ef3cd5cc8e52e64f8cd4bfdbc8658d
ms.sourcegitcommit: c16214e53680dc71d1c07111b51f72b82a4512d8
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 03/03/2021
ms.locfileid: "103761485"
---
# <a name="media-foundation-attributes"></a>Attributs Media Foundation

Cette section répertorie tous les GUID d’attribut définis pour Media Foundation, organisés par les objets auxquels ils s’appliquent.

-   [Liste alphabétique des attributs Media Foundation](alphabetical-list-of-media-foundation-attributes.md)
-   [Attributs ASF](asf-attributes.md)
-   [Attributs du convertisseur audio](audio-renderer-attributes.md)
-   [Attributs de flux d’octets](byte-stream-attributes.md)
-   [Capturer les attributs de l’appareil](capture-device-attributes.md)
-   [Attributs du moteur de capture](capture-engine-attributes.md)
-   [Attributs d'événement](event-attributes.md)
-   [Attributs EVR](enhanced-video-renderer-attributes.md)
-   [Attributs du moteur multimédia](media-engine-attributes.md)
-   [Attributs de session multimédia](media-session-attributes.md)
-   [Attributs du type de média](media-type-attributes.md)
-   [Attributs du descripteur de présentation](presentation-descriptor-attributes.md)
-   [Exemples d’attributs](sample-attributes.md)
-   [Attributs du writer du récepteur](sink-writer-attributes.md)
-   [Attributs du lecteur source](source-reader-attributes.md)
-   [Attributs de flux](stream-attributes.md)
-   [Attributs du descripteur de flux](stream-descriptor-attributes.md)
-   [Attributs de topologie](topology-attributes.md)
-   [Attributs de nœud de topologie](topology-node-attributes.md)
-   [Attributs de transcodage](transcode-attributes.md)
-   [Attributs de transformation](transform-attributes.md)

## <a name="related-topics"></a>Rubriques connexes

<dl> <dt>

[**IMFAttributes**](/windows/desktop/api/mfobjects/nn-mfobjects-imfattributes)
</dt> <dt>

[Référence de programmation Media Foundation](media-foundation-programming-reference.md)
</dt> <dt>

[Attributs et propriétés](attributes-and-properties.md)
</dt> </dl>

 

 



