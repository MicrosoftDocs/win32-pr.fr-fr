---
description: Guide de programmation Media Foundation
ms.assetid: 5258b2ff-96d5-490f-9a18-c01dd6924f1c
title: Guide de programmation Media Foundation
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 7ffc8e0d9197f3a9528a6586cb4ddc8da9a3a45f
ms.sourcegitcommit: c16214e53680dc71d1c07111b51f72b82a4512d8
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 03/03/2021
ms.locfileid: "104211230"
---
# <a name="media-foundation-programming-guide"></a>Guide de programmation Media Foundation

Cette section contient des rubriques de présentation de l’utilisation de Microsoft Media Foundation dans votre application C++.

### <a name="getting-started"></a>Mise en route

-   [Concepts essentiels](media-foundation-programming--essential-concepts.md)
-   [Architecture Media Foundation](media-foundation-architecture.md)

### <a name="writing-media-applications"></a>Écriture d’applications multimédias

-   [Lecture audio/vidéo](audio-video-playback.md)
-   [Capture audio/vidéo](audio-video-capture.md)
-   [Codage et création de fichier](encoding-and-file-authoring.md)
-   [Métadonnées de média](media-metadata.md)
-   [Mise en réseau dans Media Foundation](networking-in-media-foundation.md)

### <a name="media-format-support"></a>Prise en charge du format de média

-   [Formats multimédias pris en charge dans Media Foundation](supported-media-formats-in-media-foundation.md)
-   [Media Foundation les objets codec](codecobjects.md)
-   [Prise en charge ASF dans Media Foundation](asf-support-in-media-foundation.md)
-   [Prise en charge MPEG-4 dans Media Foundation](mpeg-4-support-in-media-foundation.md)
-   [Codecs Windows Media](windows-media-codecs.md)

### <a name="low-level-video-apis"></a>API vidéo Low-Level

-   [API vidéo Direct3D 9](direct3d-video-apis.md)
-   [API vidéo Direct3D 11](direct3d-11-video-apis.md)
-   [Accélération vidéo DirectX 2,0](directx-video-acceleration-2-0.md)
-   [Gestionnaire de protection de sortie](output-protection-manager.md)

### <a name="writing-media-foundation-plug-ins"></a>Écriture de plug-ins Media Foundation

-   [Transformations de Media Foundation](media-foundation-transforms.md)
-   [Récepteurs de médias](media-sinks.md)
-   [Sources multimédias](media-sources.md)

## <a name="related-topics"></a>Rubriques connexes

<dl> <dt>

[Architecture Media Foundation](media-foundation-architecture.md)
</dt> <dt>

[Microsoft Media Foundation](microsoft-media-foundation-sdk.md)
</dt> </dl>

 

 



