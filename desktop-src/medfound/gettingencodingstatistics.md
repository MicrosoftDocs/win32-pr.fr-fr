...
-   [MFPKEY \_ TOTALFRAMES](mfpkey-totalframesproperty.md) est le nombre de trames traitées par le biais du flux d’entrée de la DMO.
...
-   [MFPKEY \_ ZEROBYTEFRAMES](mfpkey-zerobyteframesproperty.md) est le nombre de trames qui ne sont pas encodées, car il s’agit déjà d’un contenu dupliqué. Cette valeur n’est pas soustraite du nombre d’images codées signalées par DMO.
