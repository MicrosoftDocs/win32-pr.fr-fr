---
description: Cette section décrit les composants Media Foundation qui prennent en charge les formats MPEG-4, y compris les formats vidéo H. 264, audio AAC et MP4 et 3GP.
ms.assetid: a8fa05c6-74b4-408e-8390-9dcbafa012f1
title: Prise en charge MPEG-4 dans Media Foundation
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 7d4c095ee845eb137a4ce410a3ea38bc28fac26e1737bcf30000aa254efe7fc2
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 08/11/2021
ms.locfileid: "118058955"
---
# <a name="mpeg-4-support-in-media-foundation"></a>Prise en charge MPEG-4 dans Media Foundation

Cette section décrit les composants Media Foundation qui prennent en charge les formats MPEG-4, y compris les formats vidéo H. 264, audio AAC et MP4 et 3GP.

-   [**Décodeur AAC**](aac-decoder.md)
-   [**Encodeur AAC**](aac-encoder.md)
-   [**Décodeur vidéo H. 264**](h-264-video-decoder.md)
-   [**Encodeur vidéo H. 264**](h-264-video-encoder.md)
-   [**Décodeur vidéo MPEG4 2e partie 2**](mpeg4part2videodecoder.md)
-   [Récepteur de fichiers MPEG-4](mpeg-4-file-sink.md)
-   [Source de fichier MPEG-4](mpeg-4-file-source.md)

## <a name="related-topics"></a>Rubriques connexes

<dl> <dt>

[Formats multimédias pris en charge dans Media Foundation](supported-media-formats-in-media-foundation.md)
</dt> <dt>

[Guide de programmation Media Foundation](media-foundation-programming-guide.md)
</dt> </dl>

 

 



