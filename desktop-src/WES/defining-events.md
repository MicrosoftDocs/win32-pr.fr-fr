...
Vous devez définir l’attribut **symbol** pour spécifier la constante symbolique générée par le compilateur pour identifier le descripteur d’événement de l’événement : vous utilisez le descripteur d’événement lors de l’écriture de l’événement. Si vous ne spécifiez pas le symbole, le compilateur générera un nom pour vous.

L’attribut de **message** contient la chaîne de message localisée qui est affichée avec l’événement. Pour inclure des données spécifiques à l’événement dans la chaîne, ajoutez des chaînes d’insertion dans le texte du message. Par exemple, pour inclure le troisième élément de données dans le modèle, incluez %3.

L’exemple suivant montre un manifeste complet qui définit des événements.
