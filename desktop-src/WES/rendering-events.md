...
ms.locfileid: "118587304"
---
# <a name="rendering-events"></a>Événements de rendu

Pour afficher un événement, vous devez appeler la fonction [**EvtRender**](/windows/desktop/api/WinEvt/nf-winevt-evtrender) pour la restituer dans un formulaire affichable. Vous pouvez restituer l’événement sous la forme d’une chaîne XML ou vous pouvez afficher une ou plusieurs valeurs à partir de l’événement. Un événement peut également contenir des chaînes de messages (par exemple, la chaîne de message de l’événement, la chaîne de message du canal ou la chaîne de message du fournisseur). Pour récupérer l’une des chaînes de message de l’événement, appelez la fonction [**EvtFormatMessage**](/windows/desktop/api/WinEvt/nf-winevt-evtformatmessage) . Pour plus d’informations sur l’obtention d’une chaîne de message à partir de l’événement, consultez [mise en forme des messages d’événement](formatting-event-messages.md).

Pour afficher l’événement sous la forme d’une chaîne XML, appelez la fonction [**EvtRender**](/windows/desktop/api/WinEvt/nf-winevt-evtrender) . Toutefois, si vous souhaitez restituer des parties spécifiques de l’événement, vous devez d’abord appeler [**EvtCreateRenderContext**](/windows/desktop/api/WinEvt/nf-winevt-evtcreaterendercontext) pour spécifier les éléments de l’événement que vous souhaitez restituer. Vous pouvez restituer des valeurs spécifiques à partir de l’événement, les valeurs de la section données utilisateur ou données d’événement de l’événement, ou les valeurs des propriétés système de l’événement. Pour plus d’informations sur les composants d’un événement, consultez [schéma d’événement](eventschema-schema.md).

L’exemple suivant montre comment afficher un événement sous la forme d’une chaîne XML.
...
L’exemple suivant montre comment restituer les valeurs des propriétés à partir de la section système de l’événement. Pour afficher les données utilisateur ou les propriétés de données d’événement de l’événement, remplacez l’indicateur EvtRenderContextSystem par l’indicateur EvtRenderContextUser lors de la création du contexte de rendu.
...
L’exemple suivant montre comment restituer les valeurs spécifiques à partir de l’événement. Utilisez une expression XPath pour spécifier le nœud ou l’attribut spécifique à récupérer. Vous pouvez spécifier une ou plusieurs expressions pour extraire une ou plusieurs valeurs.
