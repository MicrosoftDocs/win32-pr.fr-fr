---
title: Obtention des métadonnées d’un fournisseur
description: Un fournisseur utilise un manifeste d’instrumentation pour s’identifier lui-même, définir les événements qu’il écrit et d’autres composants, tels que les canaux, les tâches et les mots clés.
...
ms.locfileid: "103839838"
---
# <a name="getting-a-providers-metadata"></a><span data-ttu-id="11527-103">Obtention des métadonnées d’un fournisseur</span><span class="sxs-lookup"><span data-stu-id="11527-103">Getting a Provider's Metadata</span></span>

<span data-ttu-id="11527-104">Un fournisseur utilise un manifeste d’instrumentation pour s’identifier lui-même, définir les événements qu’il écrit et d’autres composants, tels que les canaux, les tâches et les mots clés.</span><span class="sxs-lookup"><span data-stu-id="11527-104">A provider uses an instrumentation manifest to identify itself, define the events that it writes, and other components such as channels, tasks, and keywords.</span></span> <span data-ttu-id="11527-105">Pour accéder à ces informations au moment de l’exécution, appelez la fonction [**EvtOpenPublisherMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtopenpublishermetadata) pour obtenir un handle vers les métadonnées de l’éditeur.</span><span class="sxs-lookup"><span data-stu-id="11527-105">To access this information at run time, call the [**EvtOpenPublisherMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtopenpublishermetadata) function to get a handle to the publisher's metadata.</span></span>

<span data-ttu-id="11527-106">Les métadonnées sont réparties entre les métadonnées du serveur de publication et les métadonnées d’événement.</span><span class="sxs-lookup"><span data-stu-id="11527-106">The metadata is divided between the publisher metadata and the event metadata.</span></span> <span data-ttu-id="11527-107">Les métadonnées du serveur de publication contiennent les métadonnées qui identifient le serveur de publication et toutes les métadonnées qu’il définit, telles que les canaux, les niveaux, les tâches, les OpCodes et les mots clés.</span><span class="sxs-lookup"><span data-stu-id="11527-107">The publisher metadata contains the metadata that identifies the publisher and any metadata that it defines such as channels, levels, tasks, opcodes, and keywords.</span></span> <span data-ttu-id="11527-108">Pour obtenir la liste des métadonnées du fournisseur que vous pouvez récupérer, consultez l’énumération de l' [**\_ ID de \_ \_ propriété \_ des métadonnées**](/windows/desktop/api/WinEvt/ne-winevt-evt_publisher_metadata_property_id) du serveur de publication evt.</span><span class="sxs-lookup"><span data-stu-id="11527-108">For a list of the provider metadata that you can retrieve, see the [**EVT\_PUBLISHER\_METADATA\_PROPERTY\_ID**](/windows/desktop/api/WinEvt/ne-winevt-evt_publisher_metadata_property_id) enumeration.</span></span> <span data-ttu-id="11527-109">Pour récupérer les métadonnées, appelez la fonction [**EvtGetPublisherMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetpublishermetadataproperty) .</span><span class="sxs-lookup"><span data-stu-id="11527-109">To get the metadata, call the [**EvtGetPublisherMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetpublishermetadataproperty) function.</span></span> <span data-ttu-id="11527-110">Cette fonction retourne les valeurs de propriété pour les propriétés qui identifient le fournisseur et les handles des composants canaux, niveaux, tâches, OpCodes et Mots clés.</span><span class="sxs-lookup"><span data-stu-id="11527-110">This function returns the property values for the properties that identify the provider and handles to the channels, levels, tasks, opcodes, and keywords components.</span></span> <span data-ttu-id="11527-111">Pour accéder aux métadonnées de ces composants, appelez la fonction [**EvtGetObjectArrayProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetobjectarrayproperty) .</span><span class="sxs-lookup"><span data-stu-id="11527-111">To access the metadata for these components, call the [**EvtGetObjectArrayProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetobjectarrayproperty) function.</span></span>

<span data-ttu-id="11527-112">Pour obtenir les métadonnées d’événement, appelez la fonction [**EvtOpenEventMetadataEnum**](/windows/desktop/api/WinEvt/nf-winevt-evtopeneventmetadataenum) pour obtenir la liste des événements définis par le fournisseur.</span><span class="sxs-lookup"><span data-stu-id="11527-112">To get the event metadata, call the [**EvtOpenEventMetadataEnum**](/windows/desktop/api/WinEvt/nf-winevt-evtopeneventmetadataenum) function to get a list of the events that the provider defines.</span></span> <span data-ttu-id="11527-113">Appelez ensuite la fonction [**EvtNextEventMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtnexteventmetadata) dans une boucle pour énumérer les événements.</span><span class="sxs-lookup"><span data-stu-id="11527-113">Then call the [**EvtNextEventMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtnexteventmetadata) function in a loop to enumerate the events.</span></span> <span data-ttu-id="11527-114">Pour obtenir la liste des métadonnées que vous pouvez obtenir pour un événement, consultez l’énumération de l' [**\_ ID de \_ \_ propriété \_ des métadonnées d’événement evt**](/windows/desktop/api/WinEvt/ne-winevt-evt_event_metadata_property_id) .</span><span class="sxs-lookup"><span data-stu-id="11527-114">For a list of metadata that you can get for an event, see the [**EVT\_EVENT\_METADATA\_PROPERTY\_ID**](/windows/desktop/api/WinEvt/ne-winevt-evt_event_metadata_property_id) enumeration.</span></span> <span data-ttu-id="11527-115">Pour récupérer les métadonnées, appelez la fonction [**EvtGetEventMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgeteventmetadataproperty) .</span><span class="sxs-lookup"><span data-stu-id="11527-115">To get the metadata, call the [**EvtGetEventMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgeteventmetadataproperty) function.</span></span> <span data-ttu-id="11527-116">Ces propriétés retournent une valeur entière pour chaque élément de la définition de l’événement.</span><span class="sxs-lookup"><span data-stu-id="11527-116">These properties return an integer value for each element of the event definition.</span></span> <span data-ttu-id="11527-117">Pour l’ID d’événement et la version, l’affichage de la valeur entière est parfait, mais pour les autres éléments, tels que Channel et Level, l’affichage de la valeur entière peut ne pas avoir de sens pour l’utilisateur.</span><span class="sxs-lookup"><span data-stu-id="11527-117">For event ID and version, displaying the integer value is fine but for the other elements, such as channel and level, displaying the integer value may not have any meaning to the user.</span></span> <span data-ttu-id="11527-118">La valeur entière de ces éléments est la valeur de l’attribut value.</span><span class="sxs-lookup"><span data-stu-id="11527-118">The integer value for these elements is the value of the value attribute.</span></span> <span data-ttu-id="11527-119">Vous pouvez utiliser la valeur entière pour effectuer un mappage dans les métadonnées du fournisseur et obtenir la chaîne de message ou la valeur de nom de l’élément et l’afficher à l’utilisateur à la place.</span><span class="sxs-lookup"><span data-stu-id="11527-119">You can use the integer value to map into the provider's metadata and get the message string or name value for the element and display it to the user instead.</span></span>

<span data-ttu-id="11527-120">Pour obtenir un exemple qui montre comment énumérer les fournisseurs inscrits sur l’ordinateur, consultez [énumération des fournisseurs inscrits](#enumerating-registered-providers).</span><span class="sxs-lookup"><span data-stu-id="11527-120">For an example that shows how to enumerate the registered providers on the computer, see [Enumerating registered providers](#enumerating-registered-providers).</span></span>

<span data-ttu-id="11527-121">Pour obtenir un exemple qui montre comment obtenir les métadonnées d’un fournisseur inscrit, consultez [obtention des métadonnées pour un fournisseur](#getting-a-providers-metadata).</span><span class="sxs-lookup"><span data-stu-id="11527-121">For an example that shows how to get the metadata for a registered provider, see [Getting the metadata for a provider](#getting-a-providers-metadata).</span></span>

## <a name="enumerating-registered-providers"></a><span data-ttu-id="11527-122">Énumération des fournisseurs inscrits</span><span class="sxs-lookup"><span data-stu-id="11527-122">Enumerating registered providers</span></span>

<span data-ttu-id="11527-123">L’exemple suivant montre comment énumérer les fournisseurs inscrits sur l’ordinateur.</span><span class="sxs-lookup"><span data-stu-id="11527-123">The following example shows how to enumerate the providers that are registered on the computer.</span></span>
...
## <a name="getting-the-metadata-for-a-provider"></a><span data-ttu-id="11527-124">Obtention des métadonnées pour un fournisseur</span><span class="sxs-lookup"><span data-stu-id="11527-124">Getting the metadata for a provider</span></span>

<span data-ttu-id="11527-125">L’exemple suivant montre comment obtenir les métadonnées pour le fournisseur et les événements qu’il définit.</span><span class="sxs-lookup"><span data-stu-id="11527-125">The following example shows how to get the metadata for the provider and the events that it defines.</span></span>
