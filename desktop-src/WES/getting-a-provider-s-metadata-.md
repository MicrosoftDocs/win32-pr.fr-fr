...
ms.locfileid: "120124209"
---
# <a name="getting-a-providers-metadata"></a>Obtention des métadonnées d’un fournisseur

Un fournisseur utilise un manifeste d’instrumentation pour s’identifier lui-même, définir les événements qu’il écrit et d’autres composants, tels que les canaux, les tâches et les mots clés. Pour accéder à ces informations au moment de l’exécution, appelez la fonction [**EvtOpenPublisherMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtopenpublishermetadata) pour obtenir un handle vers les métadonnées de l’éditeur.

Les métadonnées sont réparties entre les métadonnées du serveur de publication et les métadonnées d’événement. Les métadonnées du serveur de publication contiennent les métadonnées qui identifient le serveur de publication et toutes les métadonnées qu’il définit, telles que les canaux, les niveaux, les tâches, les OpCodes et les mots clés. Pour obtenir la liste des métadonnées du fournisseur que vous pouvez récupérer, consultez l’énumération de l' [**\_ ID de \_ \_ propriété \_ des métadonnées**](/windows/desktop/api/WinEvt/ne-winevt-evt_publisher_metadata_property_id) du serveur de publication evt. Pour récupérer les métadonnées, appelez la fonction [**EvtGetPublisherMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetpublishermetadataproperty) . Cette fonction retourne les valeurs de propriété pour les propriétés qui identifient le fournisseur et les handles des composants canaux, niveaux, tâches, OpCodes et Mots clés. Pour accéder aux métadonnées de ces composants, appelez la fonction [**EvtGetObjectArrayProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetobjectarrayproperty) .

Pour obtenir les métadonnées d’événement, appelez la fonction [**EvtOpenEventMetadataEnum**](/windows/desktop/api/WinEvt/nf-winevt-evtopeneventmetadataenum) pour obtenir la liste des événements définis par le fournisseur. Appelez ensuite la fonction [**EvtNextEventMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtnexteventmetadata) dans une boucle pour énumérer les événements. Pour obtenir la liste des métadonnées que vous pouvez obtenir pour un événement, consultez l’énumération de l' [**\_ ID de \_ \_ propriété \_ des métadonnées d’événement evt**](/windows/desktop/api/WinEvt/ne-winevt-evt_event_metadata_property_id) . Pour récupérer les métadonnées, appelez la fonction [**EvtGetEventMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgeteventmetadataproperty) . Ces propriétés retournent une valeur entière pour chaque élément de la définition de l’événement. Pour l’ID d’événement et la version, l’affichage de la valeur entière est parfait, mais pour les autres éléments, tels que Channel et Level, l’affichage de la valeur entière peut ne pas avoir de sens pour l’utilisateur. La valeur entière de ces éléments est la valeur de l’attribut value. Vous pouvez utiliser la valeur entière pour effectuer un mappage dans les métadonnées du fournisseur et obtenir la chaîne de message ou la valeur de nom de l’élément et l’afficher à l’utilisateur à la place.

Pour obtenir un exemple qui montre comment énumérer les fournisseurs inscrits sur l’ordinateur, consultez [énumération des fournisseurs inscrits](#enumerating-registered-providers).

Pour obtenir un exemple qui montre comment obtenir les métadonnées d’un fournisseur inscrit, consultez [obtention des métadonnées pour un fournisseur](#getting-a-providers-metadata).

## <a name="enumerating-registered-providers"></a>Énumération des fournisseurs inscrits

L’exemple suivant montre comment énumérer les fournisseurs inscrits sur l’ordinateur.
...
## <a name="getting-the-metadata-for-a-provider"></a>Obtention des métadonnées pour un fournisseur

L’exemple suivant montre comment obtenir les métadonnées pour le fournisseur et les événements qu’il définit.
