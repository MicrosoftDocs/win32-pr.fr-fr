---
description: Cet ensemble de documentation décrit des éléments qui peuvent présenter un intérêt pour les développeurs.
ms.assetid: e320733e-a9ec-4ead-9601-c736e6288c7a
title: Notes pour les développeurs
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: b6d9640b770e0b2527dafe6a6f2bec2de00a870b29a255481d8402dcf91b27f6
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 08/11/2021
ms.locfileid: "119654149"
---
# <a name="developer-notes"></a>Notes pour les développeurs

Cet ensemble de documentation décrit des éléments qui peuvent présenter un intérêt pour les développeurs. Elle est divisée en sections suivantes :

-   [Base de données de compatibilité des applications](application-compatibility-database.md)
-   [Application Verifier](application-verifier.md)
-   [Audio et vidéo](audio-and-video-.md)
-   [API Cabinet](cabinet-api.md)
-   [Débogage](debugging.md)
-   [Pilotes](drivers.md)
-   [API de mise à niveau d’édition](edition-upgrade-api.md)
-   [Mise en lots des fonctionnalités](feature-staging.md)
-   [FeedbackHub](feedbackhub.md)
-   [API de l’historique des fichiers](file-history-api.md)
-   [Fichiers](files.md)
-   [Partage IME](ime-share.md)
-   [Prise en charge du client Low-Level Graphics](-dxgkernel-low-level-client-support.md)
-   [Prise en charge des clients de Low-Level divers](-win32-misclowlevelclientsupport.md)
-   [MSHTML](mshtml2.md)
-   [Installation](installation.md)
-   [Transfert de cible iSCSI](/powershell/module/iscsi)
-   [Machine virtuelle Java](java-vm.md)
-   [Débogage jet](jet-debugging.md)
-   [MSN](msn.md)
-   [MSXML 1,0 (obsolète)](msxml-1-0.md)
-   [Réseau](networking.md)
-   [NFS](nfs.md)
-   [Fichiers hors connexion](offline-files.md)
-   [Bibliothèque de Registre hors connexion](offline-registry-library-portal.md)
-   [PStore](pstore.md)
-   [Valeurs de Registre](registry-values.md)
-   [Sécurité](security.md)
-   [SMTP](smtp.md)
-   [Système](system.md)
-   [Mode d’installation Terminal Server](terminal-server-install-mode.md)
-   [Interface utilisateur](user-interface.md)
-   [Visual Studio](visual-studio.md)
-   [Windows Stratégie de mode sécurisé](windows-lockdown-policy.md)
-   [Analyseur lexical](word-breaker.md)

 

 



