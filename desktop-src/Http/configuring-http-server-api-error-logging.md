...
|----------------|-------------|
| <span id="EnableErrorLogging"></span><span id="enableerrorlogging"></span><span id="ENABLEERRORLOGGING"></span>EnableErrorLogging<br /> | Valeur <strong>DWORD</strong> qui peut être définie sur <strong>true</strong> pour activer la journalisation des erreurs, ou <strong>false</strong> pour la désactiver. La valeur par défaut est <strong>true</strong>.<br /> | 
| <span id="ErrorLogFileTruncateSize"></span><span id="errorlogfiletruncatesize"></span><span id="ERRORLOGFILETRUNCATESIZE"></span>ErrorLogFileTruncateSize<br /> | <strong>Valeur DWORD</strong> qui spécifie la taille maximale, en octets, d’un fichier journal des erreurs. La valeur par défaut est un mégaoctet (de 1 à 1).<br /><blockquote>[!Note]<br />La valeur spécifiée ne peut pas être inférieure à la valeur par défaut.</blockquote><br /> | 
| <span id="ErrorLoggingDir"></span><span id="errorloggingdir"></span><span id="ERRORLOGGINGDIR"></span>ErrorLoggingDir<br /> | <strong>Chaîne</strong> qui spécifie le dossier sous lequel l’API du serveur http place ses fichiers de journalisation. <br /> L’API du serveur HTTP crée un sous-dossier nommé « HTTPERR » dans le dossier spécifié dans lequel les fichiers journaux sont placés. Ce sous-dossier et les fichiers journaux reçoivent les mêmes paramètres d’autorisation, ce qui signifie que les comptes administrateur et système local ont un accès complet, alors que les autres utilisateurs n’ont pas accès.<br /> Si aucun dossier n’est spécifié dans le registre, le dossier par défaut est le suivant :<br /> "%SystemRoot%\System32\LogFiles"<br /><blockquote>[!Note]<br />La valeur de chaîne ErrorLoggingDir doit être un chemin d’accès complet, mais elle peut contenir « % SystemRoot% ».</blockquote><br /><br /> | 
