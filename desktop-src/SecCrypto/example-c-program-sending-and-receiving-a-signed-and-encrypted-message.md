...
ms.locfileid: "119007537"
---
# <a name="example-c-program-sending-and-receiving-a-signed-and-encrypted-message"></a>Exemple de programme C : envoi et réception d’un message signé et chiffré

L’exemple suivant signe un message à l’aide de la [*clé privée*](../secgloss/p-gly.md) d’un expéditeur et chiffre le message signé à l’aide de la [*clé publique*](../secgloss/p-gly.md)du récepteur. L’exemple déchiffre ensuite le message à l’aide de la clé privée du récepteur et vérifie la signature à l’aide de la clé publique de l’expéditeur. Le certificat de l’expéditeur contenant la clé publique nécessaire est inclus dans le message chiffré. Cet exemple écrit également le message signé et chiffré dans un fichier. Pour plus d’informations, consultez [exemple de programme C : réception d’un message signé et chiffré](example-c-program-receiving-a-signed-and-encrypted-message.md).

Pour signer le message, la clé privée du signataire et le certificat du signataire doivent être disponibles. Pour chiffrer le message signé, le certificat d’un récepteur incluant la clé publique du récepteur doit être disponible.

Pour déchiffrer le message, la clé privée du récepteur doit être disponible. Une fois le message déchiffré, la signature est vérifiée à l’aide de la clé publique du certificat inclus dans le message chiffré.

> [!Note]  
> Tous les certificats d’un magasin de [*certificats*](../secgloss/c-gly.md) ne donnent pas accès à la [*clé privée*](../secgloss/p-gly.md) associée à ce certificat. Lorsque le message est signé et chiffré, un certificat appartenant au signataire avec accès à la clé privée de ce signataire doit être utilisé. En outre, le récepteur du message doit avoir accès à la clé privée associée à la clé publique utilisée pour chiffrer le message.
...
-   Recherche de certificats pour l’expéditeur et le récepteur du message dans les magasins de certificats ouverts.
-   Recherche et impression du nom de l’objet à partir de certificats.
-   Initialisation des structures de données nécessaires pour signer, chiffrer, déchiffrer et vérifier un message.
-   Appel d’une fonction CryptoAPI pour rechercher la taille requise d’une mémoire tampon, allouer la mémoire tampon de la taille requise et appeler à nouveau la fonction CryptoAPI pour remplir la mémoire tampon. Pour plus d’informations, consultez [récupération de données de longueur inconnue](retrieving-data-of-unknown-length.md).
-   Affichage du contenu chiffré d’une mémoire tampon. La fonction locale incluse, **ShowBytes**, affiche les caractères dans la mémoire tampon avec des valeurs comprises entre « 0 » et « z ». Tous les autres caractères sont affichés en tant que caractère « - ».
...
Cet exemple utilise des fonctions distinctes pour illustrer le processus de signature et de chiffrement et le processus de vérification de déchiffrement/signature. Il utilise également [**MyHandleError**](myhandleerror.md) pour quitter le programme normalement en cas de défaillance. Le code **MyHandleError** est inclus dans l’exemple et peut également être trouvé avec d’autres fonctions auxiliaires sous [usage général fonctions](general-purpose-functions.md).
