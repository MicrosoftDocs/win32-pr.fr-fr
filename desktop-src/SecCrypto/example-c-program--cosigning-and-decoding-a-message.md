...
ms.locfileid: "119874049"
---
# <a name="example-c-program-cosigning-and-decoding-a-message"></a>Exemple de programme C : co-signature et décodage d’un message

Vous pouvez utiliser la fonction [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) pour signer un message. Pour ce faire, vous devez appeler [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) une fois pour signer le message d’origine, puis appeler à nouveau **CryptSignMessage** pour signer le message signé.

Lorsque vous vérifiez la signature d’un message signé, vous utilisez la fonction [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) pour obtenir le nombre de signataires du message, puis appelez [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) pour chaque signature. Si toutes les signatures sont vérifiées, vous savez que le message signé est valide.

L’exemple suivant montre comment signer un message par plusieurs personnes (signer le message), vérifier toutes les signatures et décoder le message.
