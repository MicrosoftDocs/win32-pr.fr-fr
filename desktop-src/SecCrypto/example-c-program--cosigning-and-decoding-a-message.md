---
description: Vous pouvez utiliser la fonction CryptSignMessage pour signer un message.
ms.assetid: b400436f-a71f-426a-ac8a-7fdcfa6d7575
title: 'Exemple de programme C : co-signature et décodage d’un message'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104318413"
---
# <a name="example-c-program-cosigning-and-decoding-a-message"></a><span data-ttu-id="91bac-103">Exemple de programme C : co-signature et décodage d’un message</span><span class="sxs-lookup"><span data-stu-id="91bac-103">Example C Program: Cosigning and Decoding a Message</span></span>

<span data-ttu-id="91bac-104">Vous pouvez utiliser la fonction [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) pour signer un message.</span><span class="sxs-lookup"><span data-stu-id="91bac-104">You can use the [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) function to cosign a message.</span></span> <span data-ttu-id="91bac-105">Pour ce faire, vous devez appeler [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) une fois pour signer le message d’origine, puis appeler à nouveau **CryptSignMessage** pour signer le message signé.</span><span class="sxs-lookup"><span data-stu-id="91bac-105">This is accomplished by calling [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) once to sign the original message, and then call **CryptSignMessage** again to cosign the signed message.</span></span>

<span data-ttu-id="91bac-106">Lorsque vous vérifiez la signature d’un message signé, vous utilisez la fonction [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) pour obtenir le nombre de signataires du message, puis appelez [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) pour chaque signature.</span><span class="sxs-lookup"><span data-stu-id="91bac-106">When you verify the signature of a cosigned message, you use the [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) function to get the number of signers of the message and then call the [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) for each signature.</span></span> <span data-ttu-id="91bac-107">Si toutes les signatures sont vérifiées, vous savez que le message signé est valide.</span><span class="sxs-lookup"><span data-stu-id="91bac-107">If all of the signatures are verified, then you know the cosigned message is valid.</span></span>

<span data-ttu-id="91bac-108">L’exemple suivant montre comment signer un message par plusieurs personnes (signer le message), vérifier toutes les signatures et décoder le message.</span><span class="sxs-lookup"><span data-stu-id="91bac-108">The following example shows how to sign a message by more than one person (cosign the message), verify all signatures, and decode the message.</span></span>
