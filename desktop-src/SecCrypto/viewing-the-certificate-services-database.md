...
3.  Optionnel. Spécifiez des critères de tri et/ou des critères de qualification pour la requête de base de données en appelant la fonction [**ICertView :: SetRestriction**](/windows/desktop/api/Certview/nf-certview-icertview-setrestriction) . Les critères d’inclusion consistent à informer la vue de récupérer des données en fonction de qualificateurs tels que supérieur à, inférieur à, égal à, etc.
