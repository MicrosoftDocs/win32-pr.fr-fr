...
ms.locfileid: "127413468"
---
# <a name="example-c-program-using-certoidtoalgid-and-certcompareintegerblob"></a>Exemple de programme C : utilisation de CertOIDToAlgId et CertCompareIntegerBlob

L’exemple suivant illustre l’utilisation des fonctions [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid) et [**CertCompareIntegerBlob**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcompareintegerblob) .

Tout d’abord, tous les OID disponibles sont énumérés à l’aide de [**CryptEnumOIDInfo**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumoidinfo). Le code utilisé avec cette fonction illustre également l’utilisation d’une fonction de rappel. La fonction de rappel démontre que la logique d’interruption entre chaque groupe OID et après avoir présenté des informations sur un nombre défini d’OID.

Deuxièmement, trois chaînes d' [*identificateur d’objet*](../secgloss/o-gly.md) (OID) sont converties en entiers d’identificateur d’algorithme **DWORD** à l’aide de [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid). Le code montre également que toutes les chaînes OID n’ont pas d’identificateurs d’algorithme associés.

Enfin, l’exemple illustre la comparaison d’objets BLOB d’entiers. Cet exemple illustre la troncation des 0x00's de début à partir de nombres positifs et de 0xFF’s de début à partir de nombres négatifs.

Il montre également que les entiers sont comparés comme s’ils étaient stockés dans un format [*Little endian*](../secgloss/l-gly.md) avec les chiffres les plus significatifs à droite.
