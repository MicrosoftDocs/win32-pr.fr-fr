---
description: L’exemple suivant illustre l’utilisation des fonctions CertOIDToAlgId et CertCompareIntegerBlob.
ms.assetid: 89186d98-80a9-460a-be2b-3e328675c485
title: 'Exemple de programme C : utilisation de CertOIDToAlgId et CertCompareIntegerBlob'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103951252"
---
# <a name="example-c-program-using-certoidtoalgid-and-certcompareintegerblob"></a><span data-ttu-id="dcbe8-103">Exemple de programme C : utilisation de CertOIDToAlgId et CertCompareIntegerBlob</span><span class="sxs-lookup"><span data-stu-id="dcbe8-103">Example C Program: Using CertOIDToAlgId and CertCompareIntegerBlob</span></span>

<span data-ttu-id="dcbe8-104">L’exemple suivant illustre l’utilisation des fonctions [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid) et [**CertCompareIntegerBlob**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcompareintegerblob) .</span><span class="sxs-lookup"><span data-stu-id="dcbe8-104">The following example demonstrates using the [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid) and [**CertCompareIntegerBlob**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcompareintegerblob) functions.</span></span>

<span data-ttu-id="dcbe8-105">Tout d’abord, tous les OID disponibles sont énumérés à l’aide de [**CryptEnumOIDInfo**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumoidinfo).</span><span class="sxs-lookup"><span data-stu-id="dcbe8-105">First, all available OIDs are enumerated using [**CryptEnumOIDInfo**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumoidinfo).</span></span> <span data-ttu-id="dcbe8-106">Le code utilisé avec cette fonction illustre également l’utilisation d’une fonction de rappel.</span><span class="sxs-lookup"><span data-stu-id="dcbe8-106">Code used with this function also demonstrates the use of a callback function.</span></span> <span data-ttu-id="dcbe8-107">La fonction de rappel démontre que la logique d’interruption entre chaque groupe OID et après avoir présenté des informations sur un nombre défini d’OID.</span><span class="sxs-lookup"><span data-stu-id="dcbe8-107">The callback function demonstrates break logic to pause between each OID group and after presenting information on a set number of OIDs.</span></span>

<span data-ttu-id="dcbe8-108">Deuxièmement, trois chaînes d' [*identificateur d’objet*](../secgloss/o-gly.md) (OID) sont converties en entiers d’identificateur d’algorithme **DWORD** à l’aide de [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid).</span><span class="sxs-lookup"><span data-stu-id="dcbe8-108">Second, three [*object identifier*](../secgloss/o-gly.md) (OID) strings are converted into **DWORD** algorithm identifier integers using [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid).</span></span> <span data-ttu-id="dcbe8-109">Le code montre également que toutes les chaînes OID n’ont pas d’identificateurs d’algorithme associés.</span><span class="sxs-lookup"><span data-stu-id="dcbe8-109">The code also demonstrates that all OID strings do not have related algorithm identifiers.</span></span>

<span data-ttu-id="dcbe8-110">Enfin, l’exemple illustre la comparaison d’objets BLOB d’entiers.</span><span class="sxs-lookup"><span data-stu-id="dcbe8-110">Finally, the example demonstrates comparing integer BLOBs.</span></span> <span data-ttu-id="dcbe8-111">Cet exemple illustre la troncation des 0x00's de début à partir de nombres positifs et de 0xFF’s de début à partir de nombres négatifs.</span><span class="sxs-lookup"><span data-stu-id="dcbe8-111">This example demonstrates the truncation of leading 0x00's from positive numbers and leading 0xFF's from negative numbers.</span></span>

<span data-ttu-id="dcbe8-112">Il montre également que les entiers sont comparés comme s’ils étaient stockés dans un format [*Little endian*](../secgloss/l-gly.md) avec les chiffres les plus significatifs à droite.</span><span class="sxs-lookup"><span data-stu-id="dcbe8-112">It also shows that integers are compared as though they are stored in [*little-endian*](../secgloss/l-gly.md) form with the most significant digits on the right.</span></span>
