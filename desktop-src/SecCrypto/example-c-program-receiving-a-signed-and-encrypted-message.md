...
L’exemple suivant fonctionne conjointement avec le programme dans l' [exemple de programme C : envoi et réception d’un message signé et chiffré](example-c-program-sending-and-receiving-a-signed-and-encrypted-message.md). Il lit le message signé et chiffré, puis déchiffre et vérifie le message.

Pour déchiffrer et vérifier le message, la [*clé privée*](../secgloss/p-gly.md) du destinataire prévu du message doit être disponible. Le certificat du signataire est inclus dans l' [*objet BLOB*](../secgloss/b-gly.md) de messages lu dans à partir du fichier.

Cet exemple illustre les tâches suivantes :

-   Ouverture et fermeture des magasins de certificats système.
-   Lecture d' [**un \_ \_ objet blob de nom de certificat**](/previous-versions/windows/desktop/legacy/aa381414(v=vs.85)) à partir d’un fichier.
...
Cet exemple utilise [**MyHandleError**](myhandleerror.md) pour quitter le programme normalement en cas de défaillance. Le code **MyHandleError** est inclus avec l’exemple et peut également être trouvé avec d’autres fonctions auxiliaires sous [usage général fonctions](general-purpose-functions.md).
