...
---
# <a name="example-c-program-using-cryptencryptmessage-and-cryptdecryptmessage"></a><span data-ttu-id="6d1f2-103">Exemple de programme C : utilisation de CryptEncryptMessage et CryptDecryptMessage</span><span class="sxs-lookup"><span data-stu-id="6d1f2-103">Example C Program: Using CryptEncryptMessage and CryptDecryptMessage</span></span>

<span data-ttu-id="6d1f2-104">[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) est le seul appel de fonction nécessaire pour accomplir toutes les tâches listées dans [chiffrement d’un message](../secauthn/encrypting-a-message.md).</span><span class="sxs-lookup"><span data-stu-id="6d1f2-104">[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) is the only function call necessary to accomplish all of the tasks listed in [Encrypting a Message](../secauthn/encrypting-a-message.md).</span></span> <span data-ttu-id="6d1f2-105">L’initialisation des structures de données est nécessaire.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-105">Initialization of data structures is necessary.</span></span> <span data-ttu-id="6d1f2-106">L’illustration suivante montre la relation entre les paramètres de fonction qui pointent vers des structures ou des tableaux et leurs données initialisées.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-106">The following illustration shows the relationship between those function parameters that point to structures or arrays and their initialized data.</span></span> <span data-ttu-id="6d1f2-107">Cet exemple déchiffre également le message à l’aide de [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span><span class="sxs-lookup"><span data-stu-id="6d1f2-107">This example also decrypts the message using [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span></span>

![mappage d’initialisation pour un appel à cryptencryptmessage](images/crypenc.png)

<span data-ttu-id="6d1f2-109">**Pour chiffrer les données**</span><span class="sxs-lookup"><span data-stu-id="6d1f2-109">**To encrypt data**</span></span>

1.  <span data-ttu-id="6d1f2-110">Obtient un pointeur vers le contenu à chiffrer.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-110">Get a pointer to the content to be encrypted.</span></span>
2.  <span data-ttu-id="6d1f2-111">Déterminez la taille du contenu à chiffrer.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-111">Determine the size of the content to be encrypted.</span></span>
3.  <span data-ttu-id="6d1f2-112">Obtenir un descripteur d’un fournisseur de services de chiffrement.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-112">Acquire a handle to a cryptographic provider.</span></span>
4.  <span data-ttu-id="6d1f2-113">Ouvrez un [*magasin de certificats*](../secgloss/c-gly.md).</span><span class="sxs-lookup"><span data-stu-id="6d1f2-113">Open a [*certificate store*](../secgloss/c-gly.md).</span></span>
5.  <span data-ttu-id="6d1f2-114">Obtient le certificat du destinataire.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-114">Get the recipient certificate.</span></span>
6.  <span data-ttu-id="6d1f2-115">Créez le tableau de certificats du destinataire.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-115">Create the recipient certificate array.</span></span>
7.  <span data-ttu-id="6d1f2-116">Initialisez la structure d’identificateur de l' [**\_ algorithme \_**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) de chiffrement.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-116">Initialize the [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) structure.</span></span>
8.  <span data-ttu-id="6d1f2-117">Initialisez la structure de [**\_ \_ \_ para-message Encrypt Encrypt**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) .</span><span class="sxs-lookup"><span data-stu-id="6d1f2-117">Initialize the [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) structure.</span></span>
9.  <span data-ttu-id="6d1f2-118">Appelez [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) pour chiffrer le contenu et créer un message enveloppé numériquement.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-118">Call [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) to encrypt the content and create a digitally enveloped message.</span></span>

<span data-ttu-id="6d1f2-119">L’exemple suivant implémente cette procédure.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-119">The following example implements this procedure.</span></span>

<span data-ttu-id="6d1f2-120">Les commentaires lient les fragments de code à chaque étape de la procédure.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-120">Comments relate code fragments to each step in the procedure.</span></span> <span data-ttu-id="6d1f2-121">Pour plus d’informations sur la fonction, consultez [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span><span class="sxs-lookup"><span data-stu-id="6d1f2-121">For more information about the function, see [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span></span> <span data-ttu-id="6d1f2-122">Pour plus d’informations sur les structures de données, consultez [**\_ \_ ID d’algorithme**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) de [**\_ chiffrement et \_ \_ para-chiffre du message de chiffrement**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span><span class="sxs-lookup"><span data-stu-id="6d1f2-122">For more information about the data structures, see [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) and [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span></span>

<span data-ttu-id="6d1f2-123">Cet exemple utilise les fonctions [**MyHandleError**](myhandleerror.md) et [**ByteToStr**](bytetostr.md).</span><span class="sxs-lookup"><span data-stu-id="6d1f2-123">This example uses the functions [**MyHandleError**](myhandleerror.md) and [**ByteToStr**](bytetostr.md).</span></span> <span data-ttu-id="6d1f2-124">Le code de cette fonction est inclus dans l’exemple.</span><span class="sxs-lookup"><span data-stu-id="6d1f2-124">The code for this function is included with the sample.</span></span> <span data-ttu-id="6d1f2-125">Le code pour cette fonction et d’autres fonctions auxiliaires est également listé sous [usage général Functions](general-purpose-functions.md).</span><span class="sxs-lookup"><span data-stu-id="6d1f2-125">Code for this and other auxiliary functions is also listed under [General Purpose Functions](general-purpose-functions.md).</span></span>
