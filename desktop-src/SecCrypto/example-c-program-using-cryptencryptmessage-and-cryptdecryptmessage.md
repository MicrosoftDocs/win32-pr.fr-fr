...
[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) est le seul appel de fonction nécessaire pour accomplir toutes les tâches listées dans [chiffrement d’un message](../secauthn/encrypting-a-message.md). L’initialisation des structures de données est nécessaire. L’illustration suivante montre la relation entre les paramètres de fonction qui pointent vers des structures ou des tableaux et leurs données initialisées. Cet exemple déchiffre également le message à l’aide de [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).

![mappage d’initialisation pour un appel à cryptencryptmessage](images/crypenc.png)

**Pour chiffrer les données**

1.  Obtient un pointeur vers le contenu à chiffrer.
2.  Déterminez la taille du contenu à chiffrer.
3.  Obtenir un descripteur d’un fournisseur de services de chiffrement.
4.  Ouvrez un [*magasin de certificats*](../secgloss/c-gly.md).
5.  Obtient le certificat du destinataire.
6.  Créez le tableau de certificats du destinataire.
7.  Initialisez la structure d’identificateur de l' [**\_ algorithme \_**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) de chiffrement.
8.  Initialisez la structure de [**\_ \_ \_ para-message Encrypt Encrypt**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) .
9.  Appelez [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) pour chiffrer le contenu et créer un message enveloppé numériquement.

L’exemple suivant implémente cette procédure.

Les commentaires lient les fragments de code à chaque étape de la procédure. Pour plus d’informations sur la fonction, consultez [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage). Pour plus d’informations sur les structures de données, consultez [**\_ \_ ID d’algorithme**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) de [**\_ chiffrement et \_ \_ para-chiffre du message de chiffrement**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).

Cet exemple utilise les fonctions [**MyHandleError**](myhandleerror.md) et [**ByteToStr**](bytetostr.md). Le code de cette fonction est inclus dans l’exemple. Le code pour cette fonction et d’autres fonctions auxiliaires est également listé sous [usage général Functions](general-purpose-functions.md).
