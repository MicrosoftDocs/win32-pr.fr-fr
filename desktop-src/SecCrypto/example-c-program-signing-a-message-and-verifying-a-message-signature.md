...
---
# <a name="example-c-program-signing-a-message-and-verifying-a-message-signature"></a>Exemple de programme C : signature d’un message et vérification d’une signature de message

L’exemple suivant implémente la procédure décrite dans [procédure de signature des données](procedure-for-signing-data.md). Pour obtenir des informations générales, consultez [messages simplifiés](simplified-messages.md). Vous trouverez plus d’informations sur les fonctions et les structures dans [fonctions de chiffrement de base](cryptography-functions.md), [fonctions de message simplifiées](cryptography-functions.md)et [structures CryptoAPI](cryptography-structures.md).

Cet exemple comprend également du code permettant de vérifier la signature du message créé. Ce code se trouve généralement dans un programme distinct, mais il est inclus ici par souci d’exhaustivité et de clarté.

Cet exemple illustre les fonctions CryptoAPI suivantes :
...
La signature du message ne peut être effectuée qu’à l’aide d’un accès à un certificat disposant d’une [*clé privée*](../secgloss/p-gly.md)disponible. La vérification du message peut être effectuée uniquement avec l’accès à la clé publique associée à la clé privée utilisée pour signer le certificat. L’utilisateur peut modifier l’instruction de **\# définition** en spécifiant le nom de l’objet à partir de l’un des certificats personnels de l’utilisateur.

Cet exemple illustre également l’initialisation des structures de message de signature de chiffre et de nom de message de vérification de la valeur de nom de \_ \_ \_ \_ \_ message \_ requis pour les appels à [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) et [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature).

Cet exemple utilise également la fonction [**MyHandleError**](myhandleerror.md). Le code de cette fonction est inclus dans l’exemple de programme et peut également être affiché dans les [fonctions de usage général](general-purpose-functions.md).
