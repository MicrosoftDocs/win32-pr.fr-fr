...
Les services de certificats Microsoft sont fournis avec un intermédiaire (les pages d’inscription par le Web) pour HTTP. Un autre exemple d’intermédiaire est le composant logiciel enfichable MMC certificats Microsoft Windows (qui permet l’appel de l’Assistant demande de certificat). Si d’autres protocoles de couche de transport doivent être utilisés avec les services de certificats, un développeur peut créer un intermédiaire pour chaque protocole de couche de transport souhaité.
