...
ms.locfileid: "117773693"
---
# <a name="alternate-code-for-encoding-an-enveloped-message"></a>Autre code pour l’encodage d’un message enveloppé

L’exemple suivant illustre un autre processus d’encodage d’un message signé, en utilisant ce message signé comme [*contenu interne*](../secgloss/i-gly.md) pour un message enveloppé. En préparation du décodage, le contenu interne est testé pour déterminer son type de contenu interne.
...
Cet exemple utilise également les fonctions [**MyHandleError**](myhandleerror.md) et [**GetSignerCert**](getsignercert.md). Le code C de ces fonctions est inclus dans l’exemple. Pour obtenir du code qui illustre ces fonctions et d’autres fonctions auxiliaires, consultez [usage général Functions](general-purpose-functions.md).
