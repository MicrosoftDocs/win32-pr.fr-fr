...
ms.locfileid: "127121082"
---
# <a name="example-c-program-deleting-certificates-from-a-certificate-store"></a>Exemple de programme C : suppression de certificats d’un magasin de certificats

L’exemple suivant répertorie les certificats dans un [*magasin de certificats*](../secgloss/c-gly.md)système, en affichant le nom de l’objet de chaque certificat et permet à l’utilisateur de choisir de supprimer des certificats du magasin. L’exemple obtient le nom du magasin de certificats de l’utilisateur et peut donc être utilisé pour gérer le contenu de n’importe quel magasin de certificats système.

Cet exemple illustre les tâches et les fonctions [*CryptoAPI*](../secgloss/c-gly.md) suivantes :

-   Ouverture d’un magasin de certificats système à l’aide de [**CertOpenSystemStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopensystemstorea).
-   Liste des certificats dans un magasin de certificats à l’aide de [**CertEnumCertificatesInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatesinstore).
-   Obtention du nom du sujet d’un certificat à l’aide de [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa).
-   Comparaison du nom de l’objet du certificat avec le nom de l’émetteur du certificat à l’aide de [**CertCompareCertificateName**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcomparecertificatename).
-   Vérification pour déterminer si la clé publique du certificat actuel correspond à la clé publique d’un certificat précédent à l’aide de [**CertComparePublicKeyInfo**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcomparepublickeyinfo).
-   Duplication d’un pointeur vers un [*contexte de certificat*](../secgloss/c-gly.md) à l’aide de [**CertDuplicateCertificateContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-certduplicatecertificatecontext).
-   Comparaison des membres d' [**\_ informations**](/windows/desktop/api/Wincrypt/ns-wincrypt-cert_info) de certificat de chaque certificat à l’aide de [**CertCompareCertificate**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcomparecertificate).
-   Suppression d’un certificat d’un magasin à l’aide de [**CertDeleteCertificateFromStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certdeletecertificatefromstore).
-   Fermeture d’un magasin de certificats à l’aide de [**CertCloseStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore).

Cet exemple obtient le nom d’un magasin de certificats système de l’utilisateur, ouvre cette banque et parcourt les certificats de ce magasin. Pour chaque certificat, le nom du sujet du certificat est affiché et l’utilisateur a la possibilité de supprimer ce certificat.
