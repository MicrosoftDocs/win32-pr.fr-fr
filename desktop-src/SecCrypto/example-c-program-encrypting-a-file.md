...
ms.locfileid: "127121054"
---
# <a name="example-c-program-encrypting-a-file"></a>Exemple de programme C : chiffrement d’un fichier

L’exemple suivant chiffre un fichier de données. L’exemple demande de manière interactive le nom du fichier qui contient du [*texte brut*](../secgloss/p-gly.md) à chiffrer et le nom d’un fichier dans lequel les données chiffrées doivent être écrites.

L’exemple invite l’utilisateur à entrer les noms d’un fichier d’entrée et d’un fichier de sortie. Elle invite également l’utilisateur à indiquer si un mot de passe doit être utilisé pour créer la clé de session de chiffrement. Si un mot de passe doit être utilisé dans le chiffrement des données, le même mot de passe doit être utilisé dans le programme qui déchiffre le fichier. Pour plus d’informations, consultez [exemple de programme C : déchiffrement d’un fichier](example-c-program-decrypting-a-file.md).
