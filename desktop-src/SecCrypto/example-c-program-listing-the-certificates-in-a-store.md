...
L’exemple de code suivant répertorie tous les certificats dans un [*magasin de certificats*](../secgloss/c-gly.md) système et le nom de l’objet, ainsi que toutes les propriétés de [*contexte de certificat*](../secgloss/c-gly.md) de chacun de ces certificats. L’exemple obtient le nom du magasin de certificats de l’utilisateur et peut donc être utilisé pour répertorier le contenu de n’importe quel magasin de certificats système. En outre, cet exemple illustre l’utilisation de deux nouvelles fonctions d’interface utilisateur, une qui affiche un certificat et l’autre, une interface utilisateur qui permet à l’utilisateur de sélectionner un certificat dans la liste des certificats d’un magasin.

Cet exemple de code illustre les tâches et les fonctions [*CryptoAPI*](../secgloss/c-gly.md) suivantes :

-   Ouverture d’un magasin système à l’aide de [**CertOpenSystemStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopensystemstorea).
-   Dans une boucle, énumération de tous les certificats dans le magasin ouvert à l’aide de [**CertEnumCertificatesInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatesinstore).
-   Affichage d’un certificat à l’aide de [**CryptUIDlgViewContext**](/windows/desktop/api/Cryptuiapi/nf-cryptuiapi-cryptuidlgviewcontext).
-   Obtention du nom du sujet du certificat à l’aide de [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa).
-   Dans une boucle, en utilisant [**CertEnumCertificateContextProperties**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatecontextproperties) pour obtenir les identificateurs de propriété de toutes les propriétés associées au certificat.
-   Utilisation de [**CertGetCertificateContextProperty**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetcertificatecontextproperty) pour récupérer chacune des propriétés.
-   Affichage d’une liste de certificats dans un magasin et autorisation d’un utilisateur à en sélectionner un à l’aide de [**CryptUIDlgSelectCertificateFromStore**](/windows/desktop/api/Cryptuiapi/nf-cryptuiapi-cryptuidlgselectcertificatefromstore).
-   Fermeture du magasin de certificats à l’aide de [**CertCloseStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore).
...
L’exemple suivant illustre l’énumération et l’affichage des certificats dans un magasin. Pour compiler cet exemple, vous devez configurer votre compilateur pour qu’il utilise un jeu de caractères codés sur plusieurs octets.
