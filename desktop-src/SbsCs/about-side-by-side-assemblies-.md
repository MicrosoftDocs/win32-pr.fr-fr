...
Un assembly côte à côte Windows est décrit par des [manifestes](manifests.md). Un assembly côte à côte contient une collection de ressources (un groupe de dll, de classes Windows, de serveurs COM, de bibliothèques de types ou d’interfaces) qui sont toujours fournies aux applications ensemble. Celles-ci sont décrites dans le manifeste de l’assembly.
