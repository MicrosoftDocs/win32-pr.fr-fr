---
title: Exemple de plug-in client DVC
description: Exemple de code C++ qui montre comment créer un plug-in de canal virtuel dynamique (DVC) de client Connexion Bureau à distance (RDC).
...
ms.locfileid: "103939829"
---
# <a name="dvc-client-plug-in-example"></a><span data-ttu-id="ad17f-103">Exemple de plug-in client DVC</span><span class="sxs-lookup"><span data-stu-id="ad17f-103">DVC Client Plug-in Example</span></span>

<span data-ttu-id="ad17f-104">L’exemple de code C++ suivant fournit un exemple de création d’un plug-in de canal virtuel dynamique (DVC) de client Connexion Bureau à distance (RDC).</span><span class="sxs-lookup"><span data-stu-id="ad17f-104">The following C++ code sample provides an example of how to create a Remote Desktop Connection (RDC) client dynamic virtual channel (DVC) plug-in.</span></span>
