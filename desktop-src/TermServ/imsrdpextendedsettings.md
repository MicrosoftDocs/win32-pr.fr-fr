---
title: Interface IMsRdpExtendedSettings
description: Utilisé pour définir et récupérer des propriétés nommées pour le contrôle client.
ms.assetid: b78eebc1-e514-4201-becf-770ee4a15187
ms.tgt_platform: multiple
keywords:
- Services Bureau à distance de l’interface IMsRdpExtendedSettings
- Services Bureau à distance de l’interface IMsRdpExtendedSettings, Description
topic_type:
- apiref
api_name:
- IMsRdpExtendedSettings
api_location:
- MsTscAx.dll
api_type:
- COM
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: 57dd61a399176f70b66b46b6180d0e10756780f7
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 09/13/2021
ms.locfileid: "127118725"
---
# <a name="imsrdpextendedsettings-interface"></a>Interface IMsRdpExtendedSettings

Utilisé pour définir et récupérer des propriétés nommées pour le contrôle client.

## <a name="members"></a>Membres

L’interface **IMsRdpExtendedSettings** hérite de l’interface [**IUnknown**](/windows/desktop/api/unknwn/nn-unknwn-iunknown) . **IMsRdpExtendedSettings** a également les types de membres suivants :

-   [Propriétés](#properties)

### <a name="properties"></a>Propriétés

L’interface **IMsRdpExtendedSettings** possède les propriétés suivantes.



| Propriété                                                       | Type d’accès           | Description                           |
|:---------------------------------------------------------------|:----------------------|:--------------------------------------|
| [**Propriété**](imsrdpextendedsettings-property.md)<br/> | Lecture/écriture<br/> | Contient une propriété nommée.<br/> |



 

## <a name="requirements"></a>Spécifications



| Condition requise | Valeur |
|-------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Client minimal pris en charge<br/> | Windows 8<br/>                                                                                                                                                                                                                                                                                                                                                                                                 |
| Serveur minimal pris en charge<br/> | Windows Server 2012<br/>                                                                                                                                                                                                                                                                                                                                                                                       |
| Bibliothèque de types<br/>             | <dl> <dt>MsTscAx.dll</dt> </dl>                                                                                                                                                                                                                                                                                                                               |
| DLL<br/>                      | <dl> <dt>MsTscAx.dll</dt> </dl>                                                                                                                                                                                                                                                                                                                               |
| CLSID<br/>                    | Le CLSID \_ MsRdpClient10NotSafeForScripting est défini en tant que A0C63C30-F08D-4AB4-907C-34905D770C7D<br/> Le CLSID \_ MsRdpClient7NotSafeForScripting est défini en tant que 54d38bf7-b1ef-4479-9674-1bd6ea465258<br/> Le CLSID \_ MsRdpClient8NotSafeForScripting est défini en tant que A3BC03A0-041D-42E3-AD22-882B7865C9C5<br/> Le CLSID \_ MsRdpClient9NotSafeForScripting est défini en tant que 8B918B82-7985-4C24-89DF-C33AD2BBFBCD<br/> |
| IID<br/>                      | IID \_ IMsRdpExtendedSettings est défini en tant que 302D8188-0052-4807-806A-362B628F9AC5<br/>                                                                                                                                                                                                                                                                                                                            |



 

