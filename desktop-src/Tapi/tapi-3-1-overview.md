...
Les [fournisseurs de services multimédias](media-service-providers-start-page.md) (MSP) offrent un moyen uniforme d’accéder aux flux de média dans un appel, en prenant en charge l’API DirectShow<sup>TM</sup> comme gestionnaire de flux de média principal. Les MSP TAPI implémentent des interfaces DirectShow pour un TSP particulier et sont requis pour tout service de téléphonie qui utilise la diffusion en continu DirectShow. Les flux génériques sont gérés par l’application.
