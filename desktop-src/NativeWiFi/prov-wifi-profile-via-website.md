...
JavaScript n’a aucun moyen de déterminer la version de build exacte de Windows. Toutefois, si votre utilisateur utilise le navigateur Web Microsoft Edge, vous pouvez déterminer la version de Edge en inspectant la valeur de l' `User-agent` en-tête http. Si la version est supérieure ou égale à `18.nnnnn` , le flux de travail est pris en charge.
