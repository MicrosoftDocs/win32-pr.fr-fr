...
keywords:
- Windows Interface utilisateur, applications compatibles DPI
- Windows Interface utilisateur, haute résolution
...
-   [**SetProcessDpiAwareness**](/windows/desktop/api/ShellScalingAPI/nf-shellscalingapi-setprocessdpiawareness) (ou entrée de manifeste ppp) définit le processus sur un niveau de sensibilisation ppp spécifié, qui détermine ensuite comment Windows met à l’échelle l’interface utilisateur. Cela remplace [**SetProcessDPIAware**](/windows/desktop/api/winuser/nf-winuser-setprocessdpiaware).
...
-   NativeHelpers. vcxproj : il s’agit d’un projet d’assistance natif qui implémente la fonctionnalité de base permettant à une application WPF de prendre en charge la résolution par moniteur en utilisant les Win32APIs ci-dessus. Le projet contient deux classes :
    ...
    -   PerMonitorDPIWindow : classe de base dérivée de **System. Windows. Fenêtre** qui implémente les fonctionnalités permettant de rendre une fenêtre d’application WPF compatible avec la résolution des DPI par moniteur. Ajuste la taille de la fenêtre, la taille de rendu graphique et la taille de police en fonction des PPP du moniteur plutôt que de la résolution du système.
...
2.  démarrez Microsoft Visual Studio et sélectionnez **fichier > ouvrir > Project/Solution**
...
2.  démarrez Visual Studio et sélectionnez **fichier > ouvrir > Project/Solution**
3.  accédez au répertoire qui contient une application WPF existante, puis double-cliquez sur le fichier de Solution Visual Studio (. sln).
4.  cliquez avec le bouton droit sur **Solution > ajouter > Project existante** ![ une capture d’écran qui illustre la sélection de menu ajouter : projet existant](images/scrvs-image1.png)
...
9.  Développez votre projet d’application WPF, développez **Propriétés**, puis ouvrez **AssemblyInfo. cs**. Apporter les ajouts suivants à AssemblyInfo. cs
    -   Ajoutez une référence au système. Windows. Média dans la section de référence (à l’aide de System. Windows. Media
