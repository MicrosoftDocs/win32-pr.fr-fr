...
N’oubliez pas que l’appel de la **\_ souris**, du **\_ clavier**, du **\_ \* Journal** des opérations, de l' **\_ interpréteur** de commandes et des crochets de bas niveau peut être appelé sur le thread qui a installé le hook plutôt que sur le thread traitant le hook. Pour ces raccordements, il est possible que les crochets 32 bits et 64 bits soient appelés si un hook 32 bits est avant un hook 64 bits dans la chaîne de raccordement. Pour plus d’informations, consultez [utilisation de hooks](../winmsg/using-hooks.md).
