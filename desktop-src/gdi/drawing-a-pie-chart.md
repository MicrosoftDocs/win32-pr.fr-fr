...
Les valeurs indiquées ci-dessus produisent le graphique en secteurs suivant.

![capture d’écran du graphique à secteurs résultant](images/sampleapp.png)

Le modèle de boîte de dialogue a été trouvé dans le script de ressources de l’application (. RC) spécifie les caractéristiques de la boîte de dialogue précédente (sa hauteur, les contrôles qu’elle contient et son style), comme suit.
...
La procédure de la boîte de dialogue, qui se trouve dans le fichier source de l’application, récupère les données (coordonnées du centre, rayon de l’arc et angles de démarrage et de balayage) en procédant comme suit :

1.  La fonction ClearBits définie par l’application initialise le tableau qui reçoit l’entrée utilisateur à zéro.
2.  La fonction GetStrLngth définie par l’application récupère la longueur de la chaîne entrée par l’utilisateur.
3.  La fonction RetrieveInput définie par l’application récupère la valeur entrée par l’utilisateur.

L’exemple de code suivant illustre la procédure de la boîte de dialogue.
...
Pour dessiner chaque section du graphique à secteurs, transmettez les valeurs entrées par l’utilisateur à la fonction [**AngleArc**](/windows/desktop/api/Wingdi/nf-wingdi-anglearc) . Pour remplir le graphique à secteurs à l’aide du pinceau actuel, incorporez l’appel à **AngleArc** dans un crochet de tracé. L’exemple de code suivant montre le crochet de chemin défini et l’appel à **AngleArc**.
