...
ms.locfileid: "127414314"
---
# <a name="using-filled-shapes"></a>Utilisation de formes remplies

Cette section explique comment utiliser les fonctions de forme remplie. L’exemple utilise la procédure de fenêtre principale d’une application qui permet à l’utilisateur de dessiner des ellipses, des rectangles et des rectangles avec des angles arrondis.

L’utilisateur dessine une forme remplie en sélectionnant une forme particulière dans le menu, en positionnant le curseur dans l’angle supérieur gauche de la forme (ou le rectangle englobant de la forme dans le cas d’une ellipse), puis en faisant glisser la souris jusqu’à ce que les dimensions souhaitées soient obtenues.

L’illustration suivante montre trois formes remplies dessinées à l’aide de l’exemple de code de cette section.
...
Pour permettre à l’utilisateur de dessiner des formes pleines, incluez le menu contextuel suivant dans votre application.
...
Les valeurs d’élément de menu du modèle de menu sont des constantes que vous devez définir comme suit dans le fichier d’en-tête de votre application.
...
Enfin, incluez la procédure de fenêtre suivante dans votre application.
