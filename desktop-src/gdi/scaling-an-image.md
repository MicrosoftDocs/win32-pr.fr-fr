---
description: Certaines applications mettre à l’échelle des images ; autrement dit, ils affichent des vues zoomées ou réduites d’une image. Par exemple, une application de dessin peut fournir une fonctionnalité de zoom qui permet à l’utilisateur d’afficher et de modifier un dessin pixel par pixel.
ms.assetid: ab7d5224-62de-40a8-909f-564f61c45d01
title: Mise à l’échelle d’une image
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104034373"
---
# <a name="scaling-an-image"></a><span data-ttu-id="83ee1-104">Mise à l’échelle d’une image</span><span class="sxs-lookup"><span data-stu-id="83ee1-104">Scaling an Image</span></span>

<span data-ttu-id="83ee1-105">Certaines applications mettre à l’échelle des images ; autrement dit, ils affichent des vues zoomées ou réduites d’une image.</span><span class="sxs-lookup"><span data-stu-id="83ee1-105">Some applications scale images; that is, they display zoomed or reduced views of an image.</span></span> <span data-ttu-id="83ee1-106">Par exemple, une application de dessin peut fournir une fonctionnalité de zoom qui permet à l’utilisateur d’afficher et de modifier un dessin pixel par pixel.</span><span class="sxs-lookup"><span data-stu-id="83ee1-106">For example, a drawing application may provide a zoom feature that enables the user to view and edit a drawing on a pixel-by-pixel basis.</span></span>

<span data-ttu-id="83ee1-107">Les applications regroupent les images en appelant la fonction [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) .</span><span class="sxs-lookup"><span data-stu-id="83ee1-107">Applications scale images by calling the [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) function.</span></span> <span data-ttu-id="83ee1-108">À l’instar de la fonction [**BitBlt**](/windows/desktop/api/Wingdi/nf-wingdi-bitblt) , **StretchBlt** copie les données bitmap d’une image bitmap dans un contexte de périphérique ([**DC**](/windows/desktop/api/Winuser/nf-winuser-getdcex)) source dans une image bitmap d’un contrôleur de périphérique cible.</span><span class="sxs-lookup"><span data-stu-id="83ee1-108">Like the [**BitBlt**](/windows/desktop/api/Wingdi/nf-wingdi-bitblt) function, **StretchBlt** copies bitmap data from a bitmap in a source device context ([**DC**](/windows/desktop/api/Winuser/nf-winuser-getdcex)) into a bitmap in a target DC.</span></span> <span data-ttu-id="83ee1-109">Toutefois, contrairement à la fonction **BitBlt** , **StretchBlt** met à l’échelle l’image en fonction des dimensions spécifiées des rectangles source et cible.</span><span class="sxs-lookup"><span data-stu-id="83ee1-109">However, unlike the **BitBlt** function, **StretchBlt** scales the image based on the specified dimensions of the source and target rectangles.</span></span> <span data-ttu-id="83ee1-110">Si le rectangle source est plus grand que le rectangle cible, l’image résultante semblera rétrécie ; Si le rectangle source est plus petit que le rectangle cible, l’image résultante semblera être développée.</span><span class="sxs-lookup"><span data-stu-id="83ee1-110">If the source rectangle is larger than the target rectangle, the resultant image will appear to have shrunk; if the source rectangle is smaller than the target rectangle, the resultant image will appear to have expanded.</span></span>

<span data-ttu-id="83ee1-111">Si le rectangle cible est plus petit que le rectangle source, [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) supprime les données de couleur de l’image en fonction d’un mode d’étirement spécifié, comme indiqué dans le tableau suivant.</span><span class="sxs-lookup"><span data-stu-id="83ee1-111">If the target rectangle is smaller than the source rectangle, [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) removes color data from the image according to a specified stretch mode as shown in the following table.</span></span>



| <span data-ttu-id="83ee1-112">Mode Stretch</span><span class="sxs-lookup"><span data-stu-id="83ee1-112">Stretch mode</span></span> | <span data-ttu-id="83ee1-113">Méthode</span><span class="sxs-lookup"><span data-stu-id="83ee1-113">Method</span></span>                                                                                                                    |
|--------------|---------------------------------------------------------------------------------------------------------------------------|
| <span data-ttu-id="83ee1-114">BLACKONWHITE</span><span class="sxs-lookup"><span data-stu-id="83ee1-114">BLACKONWHITE</span></span> | <span data-ttu-id="83ee1-115">Effectue une opération AND logique sur les données de couleur pour les pixels éliminés et les données de couleur pour les pixels restants.</span><span class="sxs-lookup"><span data-stu-id="83ee1-115">Performs a logical AND operation on the color data for the eliminated pixels and the color data for the remaining pixels.</span></span> |
| <span data-ttu-id="83ee1-116">WHITEONBLACK</span><span class="sxs-lookup"><span data-stu-id="83ee1-116">WHITEONBLACK</span></span> | <span data-ttu-id="83ee1-117">Effectue une opération OR logique sur les données de couleur pour les pixels éliminés et les données de couleur pour les pixels restants.</span><span class="sxs-lookup"><span data-stu-id="83ee1-117">Performs a logical OR operation on the color data for the eliminated pixels and the color data for the remaining pixels.</span></span>  |
| <span data-ttu-id="83ee1-118">COLORONCOLOR</span><span class="sxs-lookup"><span data-stu-id="83ee1-118">COLORONCOLOR</span></span> | <span data-ttu-id="83ee1-119">Élimine complètement les données de couleur des pixels supprimés.</span><span class="sxs-lookup"><span data-stu-id="83ee1-119">Eliminates the color data of the deleted pixels completely.</span></span>                                                               |
| <span data-ttu-id="83ee1-120">-</span><span class="sxs-lookup"><span data-stu-id="83ee1-120">HALFTONE</span></span>     | <span data-ttu-id="83ee1-121">Arrondit les données de couleur d’origine (source) dans la destination.</span><span class="sxs-lookup"><span data-stu-id="83ee1-121">Approximates the original (source) color data in the destination.</span></span>                                                         |



 

<span data-ttu-id="83ee1-122">Vous définissez le mode Stretch en appelant la fonction [**SetStretchBltMode**](/windows/desktop/api/Wingdi/nf-wingdi-setstretchbltmode) .</span><span class="sxs-lookup"><span data-stu-id="83ee1-122">You set the stretch mode by calling the [**SetStretchBltMode**](/windows/desktop/api/Wingdi/nf-wingdi-setstretchbltmode) function.</span></span>

<span data-ttu-id="83ee1-123">L’exemple de code suivant est tiré d’une application qui illustre les quatre modes Stretch disponibles avec la fonction [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) .</span><span class="sxs-lookup"><span data-stu-id="83ee1-123">The following example code is taken from an application that demonstrates all four of the stretch modes available with the [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) function.</span></span>
