...
ms.locfileid: "119613739"
---
# <a name="scaling-an-image"></a>Mise à l’échelle d’une image

Certaines applications mettre à l’échelle des images ; autrement dit, ils affichent des vues zoomées ou réduites d’une image. Par exemple, une application de dessin peut fournir une fonctionnalité de zoom qui permet à l’utilisateur d’afficher et de modifier un dessin pixel par pixel.

Les applications regroupent les images en appelant la fonction [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) . À l’instar de la fonction [**BitBlt**](/windows/desktop/api/Wingdi/nf-wingdi-bitblt) , **StretchBlt** copie les données bitmap d’une image bitmap dans un contexte de périphérique ([**DC**](/windows/desktop/api/Winuser/nf-winuser-getdcex)) source dans une image bitmap d’un contrôleur de périphérique cible. Toutefois, contrairement à la fonction **BitBlt** , **StretchBlt** met à l’échelle l’image en fonction des dimensions spécifiées des rectangles source et cible. Si le rectangle source est plus grand que le rectangle cible, l’image résultante semblera rétrécie ; Si le rectangle source est plus petit que le rectangle cible, l’image résultante semblera être développée.

Si le rectangle cible est plus petit que le rectangle source, [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) supprime les données de couleur de l’image en fonction d’un mode d’étirement spécifié, comme indiqué dans le tableau suivant.
...
| WHITEONBLACK | Effectue une opération OR logique sur les données de couleur pour les pixels éliminés et les données de couleur pour les pixels restants.  |
| COLORONCOLOR | Élimine complètement les données de couleur des pixels supprimés.                                                               |
| -     | Arrondit les données de couleur d’origine (source) dans la destination.                                                         |



 

Vous définissez le mode Stretch en appelant la fonction [**SetStretchBltMode**](/windows/desktop/api/Wingdi/nf-wingdi-setstretchbltmode) .

L’exemple de code suivant est tiré d’une application qui illustre les quatre modes Stretch disponibles avec la fonction [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) .
