...
L’exemple JScript suivant utilise un *média*. **attributeCount** pour déterminer le nombre d’attributs disponibles dans l’élément multimédia actuel. Le code utilise cette valeur pour imprimer une liste de noms d’attributs et de valeurs dans une zone de texte HTML, nommée myText. L’objet **Player** a été créé avec ID = "Player".
