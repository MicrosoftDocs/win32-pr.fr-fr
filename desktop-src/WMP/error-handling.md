---
title: gestion des erreurs (kit de développement logiciel Lecteur Windows Media)
description: Gestion des erreurs
ms.assetid: f0567c77-a855-4b93-9fb7-a36cde63859f
keywords:
- Lecteur Windows Media, gestion des erreurs
- Lecteur Windows Media modèle objet, gestion des erreurs
- modèle objet, gestion des erreurs
- contrôle de ActiveX Lecteur Windows Media, gestion des erreurs
- contrôle ActiveX, gestion des erreurs
- Lecteur Windows Media contrôle de ActiveX Mobile, gestion des erreurs
- Lecteur Windows Media Mobile, gestion des erreurs
...
ms.locfileid: "117935106"
---
# <a name="error-handling-windows-media-player-sdk"></a>gestion des erreurs (kit de développement logiciel Lecteur Windows Media)

le contrôle Lecteur Windows Media 6,4 ActiveX fournit la gestion des erreurs par défaut en affichant des messages d’erreur dans les boîtes de dialogue et dans la barre d’état. Vous pouvez également fournir une gestion personnalisée des erreurs en traitant les erreurs dans votre script. La gestion des erreurs est pilotée par les événements, ce qui signifie que vous recevez une notification pour chaque erreur et que vous devez décider comment traiter chaque événement d’erreur lorsqu’il se produit. pour plus d’informations sur la gestion des erreurs à l’aide du modèle d’objet de la version 6,4, consultez la section gestion des erreurs du Guide du modèle d’objet du lecteur version 6,4, qui fait partie du kit de développement logiciel (SDK) Lecteur Windows Media.

le modèle objet Lecteur Windows Media 7 ou version ultérieure fournit l’objet d' **erreur** et l’objet **ErrorItem** pour gérer les erreurs. Ces deux objets fonctionnent ensemble pour vous fournir un mécanisme de gestion des erreurs qui vous donne un contrôle complet et flexible du processus de gestion des erreurs. L’objet **Error** donne accès à une collection d’objets **ErrorItem** ; chaque objet **ErrorItem** fournit des détails sur un message d’erreur individuel.
...
vous pouvez créer un gestionnaire d’événements d’erreur pour Lecteur Windows Media à l’aide d’un script. l’exemple de JScript suivant montre comment récupérer l’élément d’erreur le plus récent à partir de la file d’attente d’erreurs et afficher le code d’erreur et la description d’erreur à l’aide du modèle objet Lecteur Windows Media 7 ou version ultérieure. L’objet **lecteur** a été créé avec l’ID = « wmp9 ».
