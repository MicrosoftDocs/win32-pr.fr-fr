...
L’élément **AlbumInfo** spécifie l’URL de la page Web que le lecteur Windows Media affiche lorsque l’utilisateur choisit d’afficher plus d’informations sur un élément multimédia particulier.
...
Quand l’utilisateur clique sur un bouton dans le lecteur Windows Media pour afficher des informations supplémentaires sur un élément multimédia particulier, le lecteur envoie la demande d’URL avec des paramètres attachés à l’aide d’un séparateur de chaîne de requête de point d’interrogation ( ?). Le tableau suivant détaille les paramètres envoyés avec la demande d’URL. D’autres peuvent être présents à des fins de compatibilité héritée.
