...
La méthode **DoProcessOutput** effectue le traitement du signal numérique. Il s’agit de la méthode qui apporte les modifications aux données fournies par le lecteur Windows Media. C’est le résultat de cette méthode que vous entendrez comme un effet d’écho lorsque votre plug-in Echo Sample est terminé.
