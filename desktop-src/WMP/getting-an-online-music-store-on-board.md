---
title: obtention d’un magasin Musique en ligne sur le tableau
description: obtention d’un magasin Musique en ligne sur le tableau
...
ms.locfileid: "127008433"
---
# <a name="getting-an-online-music-store-on-board"></a>obtention d’un magasin Musique en ligne sur le tableau

cette rubrique décrit le processus de mise en ligne d’un magasin de supports numériques en ligne pour Lecteur Windows Media. Le temps nécessaire pour que le processus d’intégration commence à se terminer est d’environ 45-60 jours OUVRABLEs. Les deux étapes du processus d’intégration sont décrites dans le tableau suivant.
...
| Étape | Description | 
|-------|-------------|
| Pending | <ul><li>Vous envoyez à Microsoft les informations de contact, les informations de démarrage et les comptes de validation requis.</li><li>Microsoft vous envoie une clé de test et une clé de production.</li><li>vous testez votre boutique en ligne avec Lecteur Windows Media.</li><li>Vous soumettez des contrats et contrats signés à Microsoft.</li></ul> | 
| Validation | Microsoft valide votre magasin en ligne. | 
...
Vous devez tester tous les aspects de votre boutique en ligne, y compris son interface utilisateur et son plug-in. dans le cadre de votre processus de test, vous devez exécuter les tests décrits dans [tests de Validation pour les magasins de Musique en ligne de Type 2](validation-tests-for-type-2-online-music-stores.md).

> [!Note]  
> Les magasins de type 1 doivent réussir tous les tests de validation pour les magasins de type 2 et certains tests supplémentaires qui sont spécifiques à l’expérience de type 1. pour plus d’informations sur les tests de validation de type 1, contactez l’équipe virtuelle des Services Lecteur Windows Media à l’adresse mpsvctm@microsoft.com .

 

## <a name="contact-information"></a>Informations de contact

Le tableau suivant présente les informations de contact dont Microsoft a besoin pour votre boutique en ligne. remplissez le [formulaire d’informations de Contact](contact-information-form-for-an-online-music-store.md) et envoyez-le à l’équipe virtuelle des Services Lecteur Windows Media à l’adresse mpsvctm@microsoft.com .
...
| Paramètres régionaux du Store             | Windows paramètres régionaux utilisateur que votre magasin doit afficher dans                                |
...
le [formulaire d’informations de démarrage d’une rubrique en ligne Musique Store](startup-information-form-for-an-online-music-store.md) contient deux copies du formulaire d’informations de démarrage et une copie du formulaire comptes de Validation. remplissez les formulaires et envoyez-les à l’équipe virtuelle des Services Lecteur Windows Media à l’adresse mpsvctm@microsoft.com .
...
| Élément                                                                                                     | Description                                                                                                                                                                                                                                   |
|----------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| URL XML des informations de service (2048-limite de caractères)                                                              | URL où Lecteur Windows Media obtient votre [document XML ServiceInfo](serviceinfo-document.md).                                                                                                                                         |
...
| Nom convivial (30 caractères maximum)                                                                       | nom de votre magasin affiché dans le sélecteur de service Lecteur Windows Media.                                                                                                                                                        |
| URL de l’image de menu (2048-limite de caractères)                                                                    | URL où Lecteur Windows Media récupère le logo de 15 x 15 pixels qu’il affiche dans le sélecteur de service.                                                                                                                                 |
| acheter Musique URL (2048 caractères maximum)<br/> (Magasins de musique intégrés uniquement)<br/>                | URL utilisée par les liens « acheter un CD-rom » et « acheter pour Musique Online » du lecteur.                                                                                                                                                              |
| URL d’achat 10 ft UI (2048 caractères maximum)<br/> (Magasins de musique intégrés uniquement--facultatif)<br/> | l’URL qui est utilisée par les liens « acheter un CD » et « acheter à Musique Online » du lecteur dans Windows XP édition media center et dans Windows media center dans Windows Vista.                                                                              |
| Logo de la boutique (130W x sur 30 h)<br/> (Joindre le fichier PNG séparément.)<br/>                              | Logo de la boutique affiché dans l’info-bulle qui s’affiche lorsque la souris se trouve sur l’image parcourir tout. ce logo doit être un fichier au format PNG et, de préférence, être mélangé en caractères alphanumériques afin qu’il puisse s’adapter aux changements de couleur dans Lecteur Windows Media. |
...
> [!Note]  
> l’image de recherche de pixels 108 x 108 est requise pour Lecteur Windows Media 11 et versions ultérieures.

 

> [!Note]  
> dans Lecteur Windows Media 11 et versions ultérieures, les éléments ServiceTask2 et ServiceTask3 du document XML ServiceInfo sont ignorés. Pour plus d’informations sur le document XML ServiceInfo, voir le [document serviceInfo](serviceinfo-document.md).
...
Au cours de l’étape de validation, Microsoft vérifie que les fonctionnalités principales de votre boutique en ligne fonctionnent correctement. pour tous les magasins (type 1 et type 2), Microsoft exécute les tests de vérification détaillés dans les [tests de Validation pour les magasins de type 2 en ligne Musique](validation-tests-for-type-2-online-music-stores.md). Microsoft exécute également des tests de validation supplémentaires pour les magasins de type 1. Si vous avez des questions sur l’étape de validation pour les magasins de type 1, envoyez un e-mail à mpsvctm@microsoft.com .
...
La clé de test est entièrement destinée à votre propre utilisation. lorsque votre clé de test se trouve dans le registre, Lecteur Windows Media utilise l’URL ServiceInfo que vous avez envoyée pour votre magasin de test.

lorsque votre clé de production se trouve dans le registre, Lecteur Windows Media utilise l’URL ServiceInfo que vous avez envoyée pour votre magasin de production. Microsoft utilise votre clé de production pendant l’étape de validation. Microsoft n’utilise jamais votre clé de test pour la validation.

lorsque votre magasin est en ligne, Lecteur Windows Media utilise l’URL ServiceInfo que vous avez envoyée pour votre magasin de production.
...
pour obtenir une liste plus complète des problèmes d’intégration courants et des questions fréquemment posées compilées par l’équipe virtuelle Lecteur Windows Media Services, consultez [problèmes d’intégration courants pour les magasins de Musique en ligne](common-on-boarding-problems-for-online-music-stores.md).
