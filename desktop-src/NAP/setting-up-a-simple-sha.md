---
title: Configuration d’un SHA simple
description: L’exemple suivant configure un simple agent d’intégrité système (SHA) et affiche deux notifications de modification d’intégrité SoH (SoH) et un vidage du cache SoH.
...
---
# <a name="setting-up-a-simple-sha"></a><span data-ttu-id="110ab-103">Configuration d’un SHA simple</span><span class="sxs-lookup"><span data-stu-id="110ab-103">Setting Up a Simple SHA</span></span>

> [!Note]  
> <span data-ttu-id="110ab-104">La plate-forme de protection d’accès réseau n’est pas disponible à partir de Windows 10</span><span class="sxs-lookup"><span data-stu-id="110ab-104">The Network Access Protection platform is not available starting with Windows 10</span></span>

 

<span data-ttu-id="110ab-105">L’exemple suivant configure un simple agent d’intégrité système (SHA) et affiche deux actions facultatives : notification de modification de déclaration d’intégrité (SoH) et vidage du cache SoH.</span><span class="sxs-lookup"><span data-stu-id="110ab-105">The following example sets up a simple system health agent (SHA) and shows two optional actions: Statement of Health (SoH) change notification and flushing of the SoH cache.</span></span> <span data-ttu-id="110ab-106">Notez que le traitement des erreurs n’est pas inclus dans la fonction main () pour des raisons de simplicité dans cet exemple.</span><span class="sxs-lookup"><span data-stu-id="110ab-106">Note that error processing is not included in the main() function for simplicity of this example.</span></span>

> [!Note]  
> <span data-ttu-id="110ab-107">Le kit de développement logiciel (SDK) NAP contient également un ensemble complet d’exemples de code, qui se trouvent dans le... \\ Exemples de \\ NetDS \\ NAP... Répertoire de votre installation du kit de développement logiciel (SDK).</span><span class="sxs-lookup"><span data-stu-id="110ab-107">The NAP SDK also contains a full set of sample code, found in the ...\\Samples\\NetDS\\NAP... directory of your SDK installation.</span></span> <span data-ttu-id="110ab-108">Cet exemple de jeu comprend un SHA, un programme de validation d’intégrité système (SHV) et un client de contrainte (EC).</span><span class="sxs-lookup"><span data-stu-id="110ab-108">This sample set includes an SHA, system health validator (SHV), and enforcement client (EC).</span></span> <span data-ttu-id="110ab-109">Il dispose de scénarios de protection d’accès réseau complets qui configurent la communication entre SHA-SHV et SHA-EC.</span><span class="sxs-lookup"><span data-stu-id="110ab-109">It has full working NAP scenarios setting up communication between SHA-SHV and SHA-EC.</span></span>
