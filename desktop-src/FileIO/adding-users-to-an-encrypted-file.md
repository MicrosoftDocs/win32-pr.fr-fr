...
ms.locfileid: "119766289"
---
# <a name="adding-users-to-an-encrypted-file"></a>Ajout d’utilisateurs à un fichier chiffré

L’exemple de code de cette rubrique ajoute un nouvel utilisateur à un fichier chiffré existant à l’aide de la fonction [**AddUsersToEncryptedFile**](/windows/desktop/api/Winefs/nf-winefs-adduserstoencryptedfile) . Elle nécessite que le certificat de l’utilisateur système de fichiers EFS (EFS) (provenant du Active Directory) existe dans le magasin de certificats utilisateurs de personnes autorisées.

Cet exemple ajoute un nouveau champ de récupération de données au fichier chiffré. Par conséquent, l’utilisateur nouvellement ajouté peut déchiffrer le fichier chiffré. L’appelant doit déjà avoir accès au fichier chiffré, que ce soit le propriétaire d’origine, l’agent de récupération de données ou en tant qu’utilisateur qui a été précédemment ajouté au fichier chiffré.
