...
Le clonage de bloc est disponible uniquement sur le type de [système de fichiers ReFS](/windows/desktop/w8cookbook/resilient-file-system--refs-) commençant par Windows Server 2016.
...
Cette implémentation requiert que les offsets de fichier de début et de fin soient alignés sur les limites du cluster. dans les références sur Windows Server 2016, les clusters ont une taille de 4 ko par défaut, mais peuvent éventuellement avoir la valeur 64 ko. La taille du cluster est un paramètre au niveau du volume défini au moment de la mise en forme.
...
-   les fichiers source et de destination doivent avoir le même paramètre d' [**Flux d’intégrité**](file-attribute-constants.md) (c’est-à-dire que l’intégrité Flux doit être activée dans les deux fichiers ou désactivée dans les deux fichiers).
...
-   le volume ReFS doit avoir été mis en forme avec Windows Server 2016, et si le clustering de basculement Windows est en cours d’utilisation, le niveau fonctionnel de clustering doit avoir été Windows Server 2016 ou une version ultérieure au format heure.
