---
description: Les exemples suivants illustrent l’utilisation des fonctions de gestion de la mémoire.
ms.assetid: 4437beb4-14cf-4028-a8ab-ad18a189cd81
title: Utilisation des fonctions de gestion de la mémoire
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: a6e66df974d202f0f21275b9c9c74e9ada828561
ms.sourcegitcommit: 831e8f3db78ab820e1710cede244553c70e50500
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 01/07/2021
ms.locfileid: "104319049"
---
# <a name="using-the-memory-management-functions"></a>Utilisation des fonctions de gestion de la mémoire

Les exemples suivants illustrent l’utilisation des fonctions de gestion de la mémoire :

-   [Réservation et validation de la mémoire](reserving-and-committing-memory.md)
-   [Création de pages de garde](creating-guard-pages.md)
-   [Énumération d’un segment de mémoire](enumerating-a-heap.md)
-   [Obtention des tas de processus](getting-process-heaps.md)
-   [Utilisation du mappage de fichiers](using-file-mapping.md)
-   [Exemple AWE](awe-example.md)
-   [Allocation de mémoire à partir d’un nœud NUMA](allocating-memory-from-a-numa-node.md)

 

 



