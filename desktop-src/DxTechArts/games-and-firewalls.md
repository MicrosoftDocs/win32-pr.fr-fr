...
-   [Windows Le pare-feu XP SP2 est mieux](#windows-xp-sp2-firewall-is-better)
...
pour ceux qui ne sont pas sur Windows XP SP2, l’activation du pare-feu windows est fortement recommandée. Consultez les instructions « utiliser un pare-feu Internet » lors de https://www.microsoft.com/security/protect la première étape de la sécurisation de votre ordinateur. Le pare-feu de connexion Internet (ICF) fournit un mappage de port pour remplacer le filtre de paquets. Pour l’essentiel, vous spécifiez le port à ouvrir et il reste ouvert jusqu’à ce que vous le fermiez. Si l’utilisateur redémarre avant qu’il ne soit fermé, il restera ouvert jusqu’à ce qu’il soit fermé spécifiquement. Le contrôle du pare-feu et la gestion des mappages de port s’effectuent via [**INetSharingManager**](/previous-versions/windows/desktop/api/netcon/nn-netcon-inetsharingmanager) (IPv4) et [**INetFwV6Mgr**](/previous-versions/windows/desktop/ics/inetfwv6mgr) (IPv6).

le pare-feu Windows pour Windows XP SP2 est une solution plus complète qui prend en charge des piles réseau tierces, et gère les ports de manière plus intelligente : les ports restent ouverts uniquement tant que l’application qui en a besoin est toujours active.

## <a name="windows-xp-sp2-firewall-is-better"></a>Windows Le pare-feu XP SP2 est mieux

Windows XP SP2 met en sortie les paramètres et les choix de sécurité. L’objectif est de protéger par défaut et de permettre à l’utilisateur de faire des choix informés sur les applications qui sont autorisées à s’exécuter sur leur ordinateur.

le nouveau pare-feu de Windows est disponible à la fois sur Windows XP SP2 et Windows Server 2003 Service Pack 1 (SP1). à l’instar du pare-feu windows, il s’agit d’un pare-feu logiciel qui prend en charge IPv4 et IPv6, mais contrairement à icf, le pare-feu Windows :
...
| **Exécutable** | (Racine SDK) \\ Exemples \\ d' \\ \\ arc de casiers divers C++ \\ &lt; &gt; \\FirewallInstallHelper.dll |
...
2.  Ajoutez le FirewallInstallHelper.dll au projet en tant que fichier de support.
    ...
    1.  sous Project onglet Assistant, ouvrez l’onglet fichiers d’Application.
...
Le pare-feu est ici pour rester en contact. ces recommandations permettront à vos clients de bénéficier d’une bonne expérience de pare-feu avec votre jeu de Windows :
