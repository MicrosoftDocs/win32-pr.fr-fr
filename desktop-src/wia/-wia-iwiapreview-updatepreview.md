...
Spécifie un pointeur vers l’élément [_ *IWiaItem2* *](-wia-iwiaitem2.md) , qui est un enfant de l’élément **IWiaItem2** spécifié par le paramètre *pWiaItem2* de la méthode [**IWiaPreview :: GetNewPreview**](-wia-iwiapreview-getnewpreview.md) . Ou, si l’application requiert une version préliminaire de l’ensemble du plateau, spécifie un pointeur vers le paramètre *pWiaItem2* de la méthode **IWiaPreview :: GetNewPreview** . Quand *pChildWiaItem* est un enfant du paramètre *pWiaItem2* de **IWiaPreview :: GetNewPreview**, cet élément enfant est généralement créé par le filtre de segmentation.
