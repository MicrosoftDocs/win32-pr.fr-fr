...
5.  Créez un déclencheur quotidien à l’aide [**de la propriété déclencheurs de ITaskDefinition**](/windows/desktop/api/taskschd/nf-taskschd-itaskdefinition-get_triggers) pour accéder à l’interface [**ITriggerCollection**](/windows/desktop/api/taskschd/nn-taskschd-itriggercollection) pour la tâche.

    Utilisez la méthode [**ITriggerCollection :: Create**](/windows/desktop/api/taskschd/nf-taskschd-itriggercollection-create) pour spécifier que vous souhaitez créer un déclencheur quotidien. Vous pouvez définir la limite de début et l’intervalle en jours pour le déclencheur afin que les actions de la tâche soient planifiées pour s’exécuter à une heure spécifiée, sur certains jours. L’exemple montre également comment définir un modèle de répétition pour le déclencheur afin de répéter la tâche.
