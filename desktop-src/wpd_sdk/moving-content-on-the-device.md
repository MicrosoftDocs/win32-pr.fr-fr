...
ms.locfileid: "119928369"
---
# <a name="moving-content-on-the-device"></a>Déplacement du contenu sur l’appareil

Une autre opération courante accomplie par une application WPD est le transfert de contenu d’un emplacement à un autre sur l’appareil.
...
| [**Interface IPortableDevicePropVariantCollection**](iportabledevicepropvariantcollection.md) | Fournit l’accès aux méthodes spécifiques à la propriété. |



 

La fonction MoveContentAlreadyOnDevice dans le module ContentTransfer. cpp de l’exemple d’application montre comment une application peut déplacer le contenu d’un emplacement à un autre.

La première tâche accomplie par la fonction MoveContentAlreadyOnDevice consiste à interroger le pilote de périphérique pour voir s’il prend en charge le déplacement du contenu. Pour ce faire, vous devez appeler la fonction d’assistance SupportsCommand et passer la \_ gestion des objets de commande wpd déplacer les \_ \_ \_ \_ objets en tant que deuxième argument.
...
L’étape suivante consiste à inviter l’utilisateur à entrer deux identificateurs d’objet. La première est l’identificateur du contenu qui sera déplacé. Le deuxième est l’identificateur de l’emplacement vers lequel l’application doit déplacer ce contenu.
...
Ensuite, l’exemple récupère un objet [**IPortableDeviceContent**](/windows/desktop/api/portabledeviceapi/nn-portabledeviceapi-iportabledevicecontent) qui est utilisé pour accéder à la méthode [**IPortableDeviceContent :: Move**](/windows/desktop/api/PortableDeviceApi/nf-portabledeviceapi-iportabledevicecontent-move) .
...
Enfin, le contenu est déplacé. Ce processus implique les opérations suivantes :

1.  Création d’un objet [**IPortableDevicePropVariantCollection**](iportabledevicepropvariantcollection.md) qui reçoit une structure PROPVARIANT pour l’objet à déplacer.
2.  Ajout de PROPVARIANT à l’objet IPortableDevicePropVariantCollection.
3.  Appel de la méthode IPortableDeviceContent :: Move.
