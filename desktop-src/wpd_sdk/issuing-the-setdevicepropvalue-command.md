...
ms.locfileid: "127522049"
---
# <a name="issuing-the-setdevicepropvalue-command"></a>Émission de la commande SetDevicePropValue

L’exemple de cette section appelle la commande MTP **SetDevicePropValue** . (Pour obtenir une description complète de cette commande et de ses paramètres, consultez la [spécification MTP](https://www.usb.org/sites/default/files/MTPv1_1.zip).)

Comme cette commande comprend des données (ou une phase de données), elle est plus complexe que l' [exemple](issuing-the-getnumobjects-command.md)précédent. Les commandes qui incluent une phase de données peuvent être divisées en trois parties :

1.  Initiation : l’application initie la commande en informant l’appareil que les données sont arrivées ou attendues.
2.  Transfert : l’application transfère les données (en écrivant ou en lisant les données).
3.  Achèvement : l’application (ou l’appareil) signale que la commande est terminée et récupère un code de réponse.

La liste précédente se traduit par la séquence de commandes suivante :

1.  Commande \_ wpd \_ MTP \_ ext \_ Execute \_ Command \_ , avec les données \_ \_ à \_ écrire ou la commande wpd \_ \_ MTP \_ ext \_ Execute \_ Command \_ , avec les \_ données \_ à \_ lire.
2.  \_Commande wpd \_ MTP \_ ext \_ Write \_ Data ou la \_ commande wpd \_ MTP \_ ext \_ Read \_ Data.
3.  \_transfert de \_ données de fin MTP MTP de la commande wpd \_ \_ \_ \_ .

Dans le cas de **SetDevicePropValue**, l’exemple de code utilise la séquence suivante :

1.  commande \_ wpd \_ MTP \_ ext \_ Execute \_ Command \_ avec les \_ données \_ à \_ écrire.
2.  \_données d' \_ écriture MTP ext de la commande wpd \_ \_ \_ .
3.  \_transfert de \_ données de fin MTP MTP de la commande wpd \_ \_ \_ \_ .

L’exemple de code suivant montre comment une application WPD lance la séquence de commandes.
...
L’exemple de code suivant montre comment une application WPD transmet les données après l’initialisation de la commande.
...
L’exemple de code suivant montre comment une application récupère un code de réponse de l’appareil.
