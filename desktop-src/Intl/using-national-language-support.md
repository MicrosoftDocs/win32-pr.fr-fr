---
description: Utilisation de la prise en charge linguistique nationale
ms.assetid: 48d96954-90ca-4a4b-b62d-3dd2348fc61c
title: Utilisation de la prise en charge linguistique nationale
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 88a01664bb8af1f5ad3b740ee815ae99b0b9ee74
ms.sourcegitcommit: 831e8f3db78ab820e1710cede244553c70e50500
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 01/08/2021
ms.locfileid: "104115922"
---
# <a name="using-national-language-support"></a>Utilisation de la prise en charge linguistique nationale

La discussion sur l’utilisation du service NLS (National Language Support) est composée des rubriques suivantes :

-   [Appel des fonctions « nom des paramètres régionaux »](calling-the--locale-name--functions.md)
-   [Gestion des noms de domaine internationaux (IDNs)](handling-internationalized-domain-names--idns.md)
-   [Gestion du tri dans vos applications](handling-sorting-in-your-applications.md)
-   [Mappage des données de paramètres régionaux](mapping-locale-data.md)
-   [Récupération et définition des informations relatives aux paramètres régionaux](retrieving-and-setting-locale-information.md)
-   [Récupération des informations de date et d’heure](retrieving-time-and-date-information.md)
-   [Gestion d’ères pour le calendrier japonais](era-handling-for-the-japanese-calendar.md)
-   [Utilisation des données de paramètres régionaux persistants](using-persistent-locale-data.md)
-   [Utilisation de Pseudo-Locales pour le test de la localisation](using-pseudo-locales-for-localization-testing.md)
-   [Utilisation de la normalisation Unicode pour représenter des chaînes](using-unicode-normalization-to-represent-strings.md)
-   [Utilisation de paramètres régionaux personnalisés](working-with-custom-locales.md)
-   [NLS : exemple de conversion IDN (Internationalized Domain Name)](nls--internationalized-domain-name--idn--conversion-sample.md)
-   [NLS : exemple d’atténuation des noms de domaine internationaux (IDN)](nls--internationalized-domain-name--idn--mitigation-sample.md)
-   [NLS : exemple d’API basées sur un nom](nls--name-based-apis-sample.md)
-   [NLS : Unicode Normalization, exemple](nls--unicode-normalization-sample.md)

 

 



