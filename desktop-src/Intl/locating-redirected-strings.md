...
si votre application MUI utilise un service Windows, elle doit afficher le nom et la description de l’affichage du service. les ressources associées sont décrites dans « créer des ressources de type chaîne pour un Service Windows » dans [utilisation de la redirection de chaînes de registre](using-registry-string-redirection.md).
