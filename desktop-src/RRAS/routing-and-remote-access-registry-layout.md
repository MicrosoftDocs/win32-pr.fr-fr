...
---
# <a name="routing-and-remote-access-registry-layout"></a><span data-ttu-id="90886-104">Disposition du Registre du routage et de l’accès distant</span><span class="sxs-lookup"><span data-stu-id="90886-104">Routing and Remote Access Registry Layout</span></span>

<span data-ttu-id="90886-105">La syntaxe suivante montre un exemple de disposition du Registre pour le service de routeur.</span><span class="sxs-lookup"><span data-stu-id="90886-105">The following syntax shows an example registry layout for the router service.</span></span>
...
<span data-ttu-id="90886-106">Chaque gestionnaire de routeur installé dans le système a une clé de Registre créée sous la clé de routeur.</span><span class="sxs-lookup"><span data-stu-id="90886-106">Every router manager installed in the system has a registry key created under the Router key.</span></span> <span data-ttu-id="90886-107">La variable DLLPath spécifie l’emplacement de la DLL qui correspond au gestionnaire de routage et la variable ProtocolID spécifie l’identificateur de famille de protocole pour le gestionnaire de routeur.</span><span class="sxs-lookup"><span data-stu-id="90886-107">The DLLPath variable specifies the location of the DLL that corresponds to the router manager and the ProtocolID variable specifies the protocol family identifier for the router manager.</span></span>

<span data-ttu-id="90886-108">La clé interfaces est remplie avec les interfaces qui ont été ajoutées au système local à partir de la configuration du routeur.</span><span class="sxs-lookup"><span data-stu-id="90886-108">The Interfaces key is populated with the interfaces that have been added to the local system from the Router configuration.</span></span> <span data-ttu-id="90886-109">Chaque interface est associée à un type (interne, dédié ou dynamique) et des sous-clés pour chaque gestionnaire de routeur (par exemple, IP et IPX).</span><span class="sxs-lookup"><span data-stu-id="90886-109">Each interface has an associated Type (Internal, Dedicated, or Dynamic) and subkeys for each router manager (IP and IPX, for example).</span></span>
