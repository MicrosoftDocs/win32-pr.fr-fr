---
title: Structures (COM)
ms.assetid: 07f65855-f529-4d57-81fe-150167a03901
description: 'En savoir plus sur : structures (COM)'
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: fb4926110161f103e3cdac9f55c17945e58d03fa
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 09/13/2021
ms.locfileid: "127221761"
---
# <a name="structures-com"></a>Structures (COM)

Les structures suivantes sont fournies par COM :

-   [**\_accès ACTRL**](/windows/win32/api/accctrl/ns-accctrl-actrl_accessa)
-   [**\_entrée d’accès ACTRL \_**](/windows/desktop/api/AccCtrl/ns-accctrl-actrl_access_entrya)
-   [**\_liste des \_ entrées d’accès ACTRL \_**](/windows/desktop/api/AccCtrl/ns-accctrl-actrl_access_entry_lista)
-   [**entrée de la \_ propriété ACTRL \_**](/windows/desktop/api/AccCtrl/ns-accctrl-actrl_property_entrya)
-   [**LIAISONS \_ OPTS**](/windows/win32/api/objidl/ns-objidl-bind_opts)
-   [**LIER \_ OPTS2**](/windows/win32/api/objidl/ns-objidl-bind_opts2~r1)
-   [**LIER \_ OPTS3**](/windows/win32/api/objidl/ns-objidl-bind_opts3~r1)
-   [**CALLFRAME \_ MARSHALCONTEXT**](/windows/win32/api/callobj/ns-callobj-callframe_marshalcontext)
-   [**CALLFRAMEINFO**](/windows/win32/api/callobj/ns-callobj-callframeinfo)
-   [**CALLFRAMEPARAMINFO**](/windows/win32/api/callobj/ns-callobj-callframeparaminfo)
-   [**CATEGORYINFO**](/windows/win32/api/comcat/ns-comcat-categoryinfo)
-   [**COAUTHIDENTITY**](/windows/desktop/api/wtypesbase/ns-wtypesbase-coauthidentity)
-   [**COAUTHINFO**](/windows/desktop/api/wtypesbase/ns-wtypesbase-coauthinfo)
-   [**CONNECTDATA**](/windows/win32/api/ocidl/ns-ocidl-connectdata)
-   [**COSERVERINFO**](/windows/win32/api/objidlbase/ns-objidlbase-coserverinfo)
-   [**CSPLATFORM**](/windows/win32/api/wtypes/ns-wtypes-csplatform)
-   [**INTERFACEINFO**](/windows/win32/api/objidl/ns-objidl-interfaceinfo)
-   [**\_Qi multiples**](/windows/win32/api/objidlbase/ns-objidlbase-multi_qi)
-   [**ORPC \_ dbg \_ tout**](orpc-dbg-all.md)
-   [**\_ \_ mémoire tampon dbg ORPC**](orpc-dbg-all.md)
-   [**ORPC \_ init \_ args**](orpc-init-args.md)
-   [**CONTEXTE de requête**](/windows/win32/api/wtypes/ns-wtypes-querycontext)
-   [**RPCOLEMESSAGE**](/windows/win32/api/objidlbase/ns-objidlbase-rpcolemessage)
-   [**\_informations d’authentification exclusives \_**](/windows/win32/api/objidlbase/ns-objidlbase-sole_authentication_info)
-   [**\_liste d’authentification unique \_**](/windows/win32/api/objidlbase/ns-objidlbase-sole_authentication_list)
-   [**\_service d’authentification unique \_**](/windows/win32/api/objidlbase/ns-objidlbase-sole_authentication_service)
-   [**SOleTlsData**](/previous-versions/windows/desktop/legacy/ms690269(v=vs.85))

 

 
