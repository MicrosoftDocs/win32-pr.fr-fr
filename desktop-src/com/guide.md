---
title: Guide (COM)
description: Ce guide présente le modèle COM (Component Object Model) de Microsoft.
ms.assetid: 9c67dbfd-31ce-4664-a34a-4d26d97e1b1d
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 1fdb6473146d1c30142c24d350261a18149e0198
ms.sourcegitcommit: 8fa6614b715bddf14648cce36d2df22e5232801a
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 12/10/2020
ms.locfileid: "104383676"
---
# <a name="guide-com"></a>Guide (COM)

Ce guide présente le modèle COM (Component Object Model) de Microsoft.

-   [Nouveautés de COM](what-s-new-in-com.md)
-   [COM (Component Object Model)](the-component-object-model.md)
-   [Processus, threads et Apartments](processes--threads--and-apartments.md)
-   [Clients et serveurs COM](com-clients-and-servers.md)
-   [Établissement et traitement des appels asynchrones](making-and-processing-asynchronous-calls.md)
-   [Définition d’interfaces COM](defining-com-interfaces.md)
-   [Inscription des applications COM](registering-com-applications.md)
-   [Sécurité dans COM](security-in-com.md)
-   [Gestion des erreurs dans COM](error-handling-in-com.md)
-   [Gestionnaires COM](com-handlers.md)
-   [Substituts de DLL](dll-surrogates.md)
-   [Monikers](monikers.md)
-   [Événements dans les objets COM et connectables](events-in-com-and-connectable-objects.md)
-   [Implémentation du gestionnaire de catégories de composants](component-categories-manager-implementation.md)

## <a name="related-topics"></a>Rubriques connexes

<dl> <dt>

[Informations de référence](reference.md)
</dt> </dl>

 

 




