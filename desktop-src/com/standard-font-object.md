...
l’objet font fournit une propriété hFont qui est un Windows handle de police conforme aux autres attributs spécifiés pour la police. L’objet font retarde la réalisation de cette police dans la mesure du possible. par conséquent, la définition consécutive de deux propriétés sur une police n’entraîne pas la mise en place d’une police intermédiaire. En outre, en tant qu’optimisation, l’objet de police standard gère un cache de poignées de police. Deux objets de police dans le même processus qui ont des propriétés identiques retournent le même descripteur de police. L’objet de police peut supprimer des polices de ce cache à la place, ce qui présente des considérations spéciales pour les clients qui utilisent cette propriété hFont. Pour plus d’informations, consultez [**IFont :: obtenir \_ Hfont**](/windows/desktop/api/OCIdl/nf-ocidl-ifont-get_hfont) .
