...
Compte le nombre d’entrées dans l’index actuel à partir de la position actuelle vers l’avant. La position actuelle est incluse dans le nombre. Le nombre peut être supérieur au nombre total d’enregistrements dans la table si l’index actuel est sur une colonne à valeurs multiples et que les instances de la colonne ont des valeurs multiples. Si la table est vide, la valeur 0 est retournée pour le nombre.
...
<!-- end list -->
  ...
  - TableID  
    ...
    Curseur dans lequel compter les enregistrements.
...
<!-- end list -->
  ...
  - maxRecordsToCount  
    ...
    Nombre maximal d’enregistrements à compter. La valeur 0 indique que le nombre est illimité.
