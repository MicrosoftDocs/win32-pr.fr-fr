...
Répertoire dans lequel la sauvegarde est stockée. Si le chemin d’accès de sauvegarde a la valeur NULL pour utiliser, la fonction tronque les journaux, si possible.
...
La sauvegarde incrémentielle n’est possible qu’après avoir effectué une sauvegarde complète. En outre, les sauvegardes incrémentielles sont possibles uniquement si la journalisation circulaire est désactivée. Il est recommandé de ne pas inclure d’autres fichiers dans le répertoire de sauvegarde, celui impliqué dans la sauvegarde ou ajouté par une sauvegarde précédente réussie.
...
Il est vivement recommandé de ne pas utiliser la sauvegarde pour d’autres fonctions, puis la sauvegarde et la restauration au niveau du moteur. Cela réduira la modification de l’obtention des erreurs pendant les opérations de sauvegarde et de restauration.
...
| <p><strong>Unicode</strong></p> | <p>Implémenté en tant que <strong>JetBackupInstanceW</strong> (Unicode) et <strong>JetBackupInstanceA</strong> (ANSI).</p> | 
