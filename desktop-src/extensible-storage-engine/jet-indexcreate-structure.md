...
Taille, en octets, de cette structure. Affectez à ce membre la valeur sizeof (JET_INDEXCREATE).

**szIndexName**

Nom de l’index à créer.

Le nom de l’index doit respecter les conditions suivantes :

  - Elle doit être inférieure à JET_cbNameMost, à l’exclusion de la valeur null de fin.

  - Il doit comporter les caractères suivants : 0 (zéro) à 9, A à Z, a à z et toutes les autres signes de ponctuation, à l’exception de « \! » (point d’exclamation), « , » (virgule), « \[ » (crochet ouvrant) et « \] » (crochet fermant), c’est-à-dire les caractères ASCII 0x20, 0X22 à 0x2D, 0x2F à 0x5A, 0x5c, 0x5d à 0x7F.
...
Le nombre de colonnes spécifié dans **szKey** ne peut pas dépasser la valeur de **JET_ccolKeyMost** (une constante dépendante de la version).
...
Déconseillé : après l’ID de langue, il est possible de passer **cbVarSegMac** comme UShort. Le comportement n’est pas défini si le USHORT est défini à la fois dans **szKey** et si **cbVarSegMac** est défini dans la structure.
...
Si vous spécifiez la valeur 0 (zéro) pour ce champ, cela équivaut à :
