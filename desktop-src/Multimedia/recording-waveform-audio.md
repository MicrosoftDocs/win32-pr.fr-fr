---
title: Enregistrement de la forme d’onde
description: Enregistrement de la forme d’onde
ms.assetid: b090c3e6-5c95-449f-8312-fcd07f453be2
keywords:
- audio Wave, enregistrement
- Waveform-interface audio, enregistrement
- enregistrement de la forme d’onde audio, à propos de
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 0bc8d01ab6109caec0991fabb8df7427462ef34d
ms.sourcegitcommit: 9eebab0ead09cecdbc24f5f84d56c8b6a7c22736
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 09/10/2021
ms.locfileid: "124367724"
---
# <a name="recording-waveform-audio"></a>Enregistrement de la forme d’onde

Les rubriques suivantes décrivent l’utilisation de l’interface Wave-audio pour l’enregistrement audio.

-   [Types de données d’entrée Waveform Audio](waveform-audio-input-data-types.md)
-   [Interrogation Waveform-Audio périphériques d’entrée](querying-waveform-audio-input-devices.md)
-   [Ouverture de Waveform-Audio périphériques d’entrée](opening-waveform-audio-input-devices.md)
-   [Gestion de l’enregistrement Waveform-Audio](managing-waveform-audio-recording.md)
-   [Utilisation de messages de fenêtre pour gérer Waveform-Audio enregistrement](using-window-messages-to-manage-waveform-audio-recording.md)

 

 




