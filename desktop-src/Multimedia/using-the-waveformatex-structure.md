...
ms.locfileid: "127011732"
---
# <a name="using-the-waveformatex-structure"></a>Utilisation de la structure WAVEFORMATEX

Pour les données audio PCM sur un maximum de deux canaux et avec des exemples de 8 ou 16 bits, utilisez la structure [**WAVEFORMATEX**](/windows/win32/api/mmeapi/ns-mmeapi-waveformatex) pour spécifier le format de données.

L’exemple suivant montre comment configurer une structure **WAVEFORMATEX** pour une chaîne mono 8 bits 11,025 kilohertz (kHz) et pour une chaîne stéréo de 44,1 kHz 16 bits. Après la configuration de **WAVEFORMATEX**, l’exemple appelle la fonction IsFormatSupported pour vérifier que l’appareil de sortie Waveform PCM prend en charge le format. Le code source pour IsFormatSupported est présenté dans un exemple pour [déterminer la prise en charge des formats non standard](determining-nonstandard-format-support.md).
