...
Comme indiqué précédemment, pour recevoir des horodatages avec entrée MIDI, vous devez utiliser une fonction de rappel. Le paramètre *dwParam2* de la fonction de rappel spécifie l’horodatage des données associées aux messages [**MIM \_**](mim-data.md) et [**\_ LONGDATA MIM**](mim-longdata.md) .
