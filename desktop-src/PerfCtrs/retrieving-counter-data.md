...
Les objets de performance peuvent définir un ou plusieurs compteurs. Les données de compteur des compteurs se trouvent dans le bloc de mémoire du [**\_ \_ bloc de compteur de performances**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_block) . L’emplacement du bloc de compteur dans le bloc d’objets varie selon que l’objet contient des compteurs d’instance unique ou des compteurs à instances multiples. Pour plus d’informations, consultez [format des données de performances](performance-data-format.md).

Vous utilisez les membres **CounterOffset** et **CounterSize** de [**la \_ \_ définition du compteur de performance**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition) pour accéder aux données du compteur dans le bloc de compteur. Actuellement, les données de compteur sont limitées aux types de données **DWORD** et **ULONGLONG** (ce sont les seuls types pris en charge par l’outil Performance Tool).

Le **membre de** la [**\_ \_ définition du compteur**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition) de performances vous indique les autres informations dont vous avez besoin à partir de l’objet de performance afin d’utiliser les données du compteur. Pour certains compteurs, vous pouvez utiliser les données de compteur directement, mais pour d’autres, vous pouvez avoir besoin d’informations de base de temps ou de données d’un autre compteur afin de calculer une valeur affichable.

L’exemple suivant montre comment utiliser le type de compteur pour déterminer les informations que vous devez récupérer à partir des données de performances d’un compteur afin de calculer une valeur de compteur affichable. Pour obtenir un exemple qui calcule une valeur affichable en fonction du type de compteur, consultez [calcul des valeurs de compteur](calculating-counter-values.md).
