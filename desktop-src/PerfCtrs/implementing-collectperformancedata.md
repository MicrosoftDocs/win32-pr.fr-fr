...
ms.locfileid: "119775619"
---
# <a name="implementing-collectperformancedata"></a>Implémentation de CollectPerformanceData

Une fois que le système a correctement appelé votre fonction [**OpenPerformanceData**](/previous-versions/windows/desktop/legacy/aa372200(v=vs.85)) , il appelle votre fonction [**CollectPerformanceData**](/windows/win32/api/winperf/nc-winperf-pm_collect_proc) pour collecter les données de compteur. Si le fournisseur prend en charge les objets interrogés, il contacte le service, le pilote ou l’application à laquelle il est associé et lui demande les données du compteur.

L’exemple suivant illustre une implémentation de la fonction [*CollectPerformanceData*](/windows/win32/api/winperf/nc-winperf-pm_collect_proc) . Le fichier d’en-tête qui contient la définition des compteurs utilisés dans cette fonction est illustré dans [implémentation de OpenPerformanceData](implementing-openperformancedata.md). Si vous utilisez C++ pour implémenter cette fonction, veillez à utiliser extern "C" lorsque vous déclarez votre fonction.
