...
ms.locfileid: "120033919"
---
# <a name="displaying-object-instance-and-counter-names"></a>Affichage des noms d’objet, d’instance et de compteur

Si vous souhaitez afficher une liste d’objets et leurs compteurs dans une interface utilisateur, vous devez récupérer les données de performances. Les données de performances contiennent un nombre variable d’objets de performance et leurs instances et compteurs. Pour plus d’informations sur le format des données de performances, consultez [format des données de performances](performance-data-format.md).

Les données de performances ne contiennent pas les noms d’objet et de compteur. Au lieu de cela, les données contiennent des valeurs d’index que vous utilisez pour récupérer les noms des objets et des compteurs, qui sont stockés dans des ressources connues de l’infrastructure des compteurs de performance. Toutefois, les noms d’instance sont stockés dans les données de performances. Pour obtenir un exemple qui lit les noms d’objet et de compteur à partir du Registre et crée une table d’index pour accéder aux noms ultérieurement, consultez [récupération des noms de compteur et du texte d’aide](retrieving-counter-names-and-help-text.md).

L’exemple suivant montre comment récupérer l’objet, l’instance et les blocs de compteur à partir des données de performances. Après avoir récupéré les noms d’objets, les noms d’instance et les noms de compteur, l’exemple trie les objets par nom et, pour chaque objet, trie ses instances et compteurs par nom. L’exemple imprime ensuite les listes triées.
