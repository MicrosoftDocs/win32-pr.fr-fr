---
description: Si vous souhaitez afficher une liste d’objets et leurs compteurs dans une interface utilisateur, vous devez récupérer les données de performances.
ms.assetid: 0d122fa8-2ed8-4bd8-a52e-6cb20fe81741
title: Affichage des noms d’objet, d’instance et de compteur
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103941454"
---
# <a name="displaying-object-instance-and-counter-names"></a><span data-ttu-id="befab-103">Affichage des noms d’objet, d’instance et de compteur</span><span class="sxs-lookup"><span data-stu-id="befab-103">Displaying Object, Instance, and Counter Names</span></span>

<span data-ttu-id="befab-104">Si vous souhaitez afficher une liste d’objets et leurs compteurs dans une interface utilisateur, vous devez récupérer les données de performances.</span><span class="sxs-lookup"><span data-stu-id="befab-104">If you want to display a list of objects and their counters in a user interface, you must retrieve the performance data.</span></span> <span data-ttu-id="befab-105">Les données de performances contiennent un nombre variable d’objets de performance et leurs instances et compteurs.</span><span class="sxs-lookup"><span data-stu-id="befab-105">The performance data contains a variable number of performance objects and their instances and counters.</span></span> <span data-ttu-id="befab-106">Pour plus d’informations sur le format des données de performances, consultez [format des données de performances](performance-data-format.md).</span><span class="sxs-lookup"><span data-stu-id="befab-106">For information on the format of the performance data, see [Performance Data Format](performance-data-format.md).</span></span>

<span data-ttu-id="befab-107">Les données de performances ne contiennent pas les noms d’objet et de compteur.</span><span class="sxs-lookup"><span data-stu-id="befab-107">The performance data does not contain the object and counter names.</span></span> <span data-ttu-id="befab-108">Au lieu de cela, les données contiennent des valeurs d’index que vous utilisez pour récupérer les noms des objets et des compteurs, qui sont stockés dans des ressources connues de l’infrastructure des compteurs de performance.</span><span class="sxs-lookup"><span data-stu-id="befab-108">Instead, the data contains index values that you use to retrieve the names of the objects and counters, which are stored in resources known to the performance counters infrastructure.</span></span> <span data-ttu-id="befab-109">Toutefois, les noms d’instance sont stockés dans les données de performances.</span><span class="sxs-lookup"><span data-stu-id="befab-109">However, the instance names are stored in performance data.</span></span> <span data-ttu-id="befab-110">Pour obtenir un exemple qui lit les noms d’objet et de compteur à partir du Registre et crée une table d’index pour accéder aux noms ultérieurement, consultez [récupération des noms de compteur et du texte d’aide](retrieving-counter-names-and-help-text.md).</span><span class="sxs-lookup"><span data-stu-id="befab-110">For an example that reads the object and counter names from the registry and creates an index table for accessing the names later, see [Retrieving Counter Names and Help Text](retrieving-counter-names-and-help-text.md).</span></span>

<span data-ttu-id="befab-111">L’exemple suivant montre comment récupérer l’objet, l’instance et les blocs de compteur à partir des données de performances.</span><span class="sxs-lookup"><span data-stu-id="befab-111">The following example shows how to retrieve the object, instance, and counter blocks from the performance data.</span></span> <span data-ttu-id="befab-112">Après avoir récupéré les noms d’objets, les noms d’instance et les noms de compteur, l’exemple trie les objets par nom et, pour chaque objet, trie ses instances et compteurs par nom.</span><span class="sxs-lookup"><span data-stu-id="befab-112">After retrieving the object names, instance names, and counter names, the example sorts the objects by name and for each object, sorts its instances and counters by name.</span></span> <span data-ttu-id="befab-113">L’exemple imprime ensuite les listes triées.</span><span class="sxs-lookup"><span data-stu-id="befab-113">The example then prints the sorted lists.</span></span>
