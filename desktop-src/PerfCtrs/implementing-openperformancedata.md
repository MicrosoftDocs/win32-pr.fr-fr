...
ms.locfileid: "127007185"
---
# <a name="implementing-openperformancedata"></a>Implémentation de OpenPerformanceData

La fonction [**OpenPerformanceData**](/previous-versions/windows/desktop/legacy/aa372200(v=vs.85)) donne au fournisseur la possibilité d’initialiser ses structures de données de performances. Le système appelle votre fonction Open la première fois qu’un consommateur appelle [**RegQueryValueEx**](/windows/desktop/api/winreg/nf-winreg-regqueryvalueexa), ou si le consommateur utilise la fonction [**RegOpenKey**](/windows/desktop/api/winreg/nf-winreg-regopenkeya) ou [**RegConnectRegistry**](/windows/desktop/api/winreg/nf-winreg-regconnectregistrya) pour ouvrir les **\_ \_ données de performance HKEY**.

L’exemple suivant illustre une implémentation de la fonction [**OpenPerformanceData**](/previous-versions/windows/desktop/legacy/aa372200(v=vs.85)) . Le fichier d’en-tête qui contient la définition des compteurs utilisés dans cette fonction suit cet exemple. Si vous utilisez C++ pour implémenter cette fonction, veillez à utiliser extern "C" lorsque vous déclarez votre fonction. Les constantes de décalage de compteur utilisées dans cet exemple sont définies dans le fichier CounterOffsets. h présenté dans [Ajout de noms de compteur et de description au registre](adding-counter-names-and-descriptions-to-the-registry.md).
...
L’exemple suivant montre le fichier de définition de module (. def) utilisé pour exporter les fonctions ouvrir, collecter et fermer.
...
L’exemple suivant illustre une implémentation de la fonction [*ClosePerformanceData*](/windows/win32/api/winperf/nc-winperf-pm_close_proc) . Le système appelle votre fonction Close lorsqu’un consommateur appelle le [**échec RegCloseKey**](/windows/desktop/api/winreg/nf-winreg-regclosekey) pour fermer **les \_ \_ données de performances de HKEY**. Les fournisseurs utilisent cet appel pour libérer toutes les ressources qu’ils ont allouées.
