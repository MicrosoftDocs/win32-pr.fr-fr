---
title: Méthodes de propriété IADsUser (IADs. h)
description: Les méthodes de propriété de l’interface IADsUser obtiennent ou définissent les propriétés décrites dans le tableau suivant. Pour plus d’informations, consultez Méthodes de propriété d’interface.
ms.assetid: 02d0e5f1-8bc9-4ef6-962d-432654ca8433
ms.tgt_platform: multiple
keywords:
- Méthodes de propriété IADsUser ADSI
topic_type:
- apiref
api_name:
...
Les méthodes de propriété de l’interface [**IADsUser**](/windows/desktop/api/Iads/nn-iads-iadsuser) obtiennent ou définissent les propriétés décrites dans le tableau suivant. Pour plus d’informations, consultez [méthodes de propriété d’interface](interface-property-methods.md).
...
Indicateur qui spécifie si le compte est ou doit être désactivé.
...
Date et heure à partir desquelles l’utilisateur ne peut pas ouvrir de session.
...
Le dernier nœud considéré comme un intrus possible ; Cela est possible si la détection d’intrus est active.
...
Nombre de tentatives de connexion incorrectes depuis la dernière réinitialisation.
...
Service, unité d’organisation, au sein de la société à laquelle l’utilisateur appartient.
...
Division au sein d’une entreprise ou d’une organisation.
...
Numéro de télécopie, ou numéros, de l’utilisateur. Dans Active Directory, cette propriété est à valeur unique et le tableau de **variants** a un élément.
...
Nombre de fois où l’utilisateur peut ouvrir une session après l’expiration du mot de passe.
...
Nombre d’ouvertures de session autorisées avant le verrouillage du compte.
...
URL de la page d’hébergement de l’utilisateur.
...
Indicateur qui signale si le compte est verrouillé en raison de la détection d’un intrus. L’utilisation de cette propriété est limitée lorsqu’elle est utilisée avec le fournisseur ADSI LDAP. Pour plus d’informations sur ces limitations, consultez [verrouillage de compte (fournisseur LDAP)](ldap-account-lockout.md).
...
Tableau de noms de langages **BSTR** pour l’utilisateur.
...
Date et heure de la dernière connexion réseau qui a échoué.
...
Périodes de temps pour chaque jour de la semaine pendant lequel les ouvertures de session sont autorisées pour l’utilisateur. Représenté sous la forme d’un tableau de valeurs booléennes pour la semaine, chacun indiquant si cette plage horaire est un délai d’ouverture de session valide. N’oubliez pas que la représentation est spécifique au fournisseur et au répertoire.
...
Chemin d’accès du script d’ouverture de session.
...
Adresses ou noms de stations de travail, du type de données **BSTR** , à partir desquels l’utilisateur peut ouvrir une session.
...
Nom supplémentaire, par exemple, le deuxième prénom, pour l’utilisateur.
...
Heure à laquelle le mot de passe a été modifié pour la dernière fois.
...
Indicateur qui signale si le mot de passe est requis.
...
Codes postaux sous forme de tableau **BSTR** . Les codes postaux sont liés de façon positionnée au tableau **PostalAddresses** . Dans Active Directory, toutefois, cette propriété est à valeur unique et le tableau a un seul élément.
...
Indicateur qui signale si un nouveau mot de passe doit être différent de ceux connus à l’aide de l’historique des mots de passe.
...
Tableau de ADsPaths d’autres objets associés à l’utilisateur.
...
Tableau de numéros de téléphone personnels de l’utilisateur. Dans Active Directory, cette propriété est à valeur unique et le tableau a un élément.
...
Tableau de numéros de téléphone mobile de l’utilisateur. Dans Active Directory cette propriété est à valeur unique et le tableau a un seul élément.
...
Tableau de numéros de téléphone associés à l’utilisateur, généralement liés à un travail. Dans Active Directory cette propriété est à valeur unique et le tableau est d’un élément unique.
...
Pour plus d’informations sur les fonctionnalités spécifiques du fournisseur LDAP ADSI de l’objet de classe utilisateur, consultez [objet utilisateur LDAP](ldap-user-object.md) dans le [fournisseur LDAP ADSI](adsi-ldap-provider.md). La rubrique comprend [**IADsUser**](/windows/desktop/api/Iads/nn-iads-iadsuser), ainsi que des exemples de code pour la gestion d’un compte d’utilisateur.

## <a name="examples"></a>Exemples

L’exemple de code suivant montre comment effectuer une liaison à un objet de compte d’utilisateur et récupérer le nom complet de l’utilisateur.
...
| IID<br/>                      | IID \_ IADsUser est défini en tant que 3E37E320-17E2-11CF-ABC4-02608C9E7553<br/>             |
...
[**IADs :: obtient**](/windows/desktop/api/Iads/nf-iads-iads-get)
</dt> <dt>

[**IADs ::P ut**](/windows/desktop/api/Iads/nf-iads-iads-put)
</dt> <dt>

[Objet utilisateur Winnt](winnt-user-object.md)
</dt> <dt>

[Fournisseur WinNT ADSI](adsi-winnt-provider.md)
</dt> <dt>

[Objet utilisateur LDAP](ldap-user-object.md)
</dt> <dt>
