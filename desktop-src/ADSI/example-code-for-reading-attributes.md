---
title: Exemple de code pour la lecture d’attributs
description: L’exemple de code suivant énumère les propriétés de l’utilisateur spécifié dans le domaine actuel, en recherchant l’utilisateur, puis en utilisant IADsPropertyList pour énumérer ses propriétés.
...
---
# <a name="example-code-for-reading-attributes"></a><span data-ttu-id="5d7bd-104">Exemple de code pour la lecture d’attributs</span><span class="sxs-lookup"><span data-stu-id="5d7bd-104">Example Code for Reading Attributes</span></span>

<span data-ttu-id="5d7bd-105">L’exemple de code suivant énumère les propriétés de l’utilisateur spécifié dans le domaine actuel, en recherchant l’utilisateur, puis en utilisant [**IADsPropertyList**](/windows/desktop/api/Iads/nn-iads-iadspropertylist) pour énumérer ses propriétés.</span><span class="sxs-lookup"><span data-stu-id="5d7bd-105">The following code example enumerates the properties of the specified user in the current domain, by searching for the user and then using [**IADsPropertyList**](/windows/desktop/api/Iads/nn-iads-iadspropertylist) to enumerate its properties.</span></span> <span data-ttu-id="5d7bd-106">N’oubliez pas que les valeurs de date et d’heure, sous la forme d’entiers volumineux, sont gérées et comment les chaînes d’octets pour **objectSID** et **objectGUID** sont gérées.</span><span class="sxs-lookup"><span data-stu-id="5d7bd-106">Be aware that time and date values, as large integers, are handled and how octet strings for **objectSID** and **objectGUID** are handled.</span></span>
...
<span data-ttu-id="5d7bd-107">L’exemple de code Visual Basic suivant montre comment obtenir les propriétés d’un objet utilisateur.</span><span class="sxs-lookup"><span data-stu-id="5d7bd-107">The following Visual Basic code example shows how to get the properties of a user object.</span></span> <span data-ttu-id="5d7bd-108">Pour utiliser cet exemple de code, créez une référence à la bibliothèque de types Active Directory et à la bibliothèque de ActiveX Data Objects Microsoft dans votre projet Visual Basic.</span><span class="sxs-lookup"><span data-stu-id="5d7bd-108">To use this code example, create a reference to the Active DS Type Library and the Microsoft ActiveX Data Objects Library in your Visual Basic project.</span></span>
