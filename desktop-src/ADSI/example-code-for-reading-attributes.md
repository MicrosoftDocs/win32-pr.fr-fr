...
L’exemple de code suivant énumère les propriétés de l’utilisateur spécifié dans le domaine actuel, en recherchant l’utilisateur, puis en utilisant [**IADsPropertyList**](/windows/desktop/api/Iads/nn-iads-iadspropertylist) pour énumérer ses propriétés. N’oubliez pas que les valeurs de date et d’heure, sous la forme d’entiers volumineux, sont gérées et comment les chaînes d’octets pour **objectSID** et **objectGUID** sont gérées.
