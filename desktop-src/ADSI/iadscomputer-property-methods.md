...
ms.locfileid: "118179577"
---
# <a name="iadscomputer-property-methods"></a>Méthodes de propriété IADsComputer

Les méthodes de l’interface [**IADsComputer**](/windows/desktop/api/Iads/nn-iads-iadscomputer) lisent et écrivent les propriétés décrites dans cette rubrique. Pour plus d’informations, consultez [méthodes de propriété d’interface](interface-property-methods.md).
...
Identificateur global unique affecté à chaque ordinateur.
...
L’unité d’organisation (OU), telle que Department, à laquelle cet ordinateur appartient.
...
Division, au sein d’une organisation, à laquelle cet ordinateur appartient.
...
Emplacement physique affecté de cet ordinateur.
...
Taille, en mégaoctets, de la mémoire vive (Random Access) de cet ordinateur.
...
Marque et modèle de cet ordinateur.
...
Tableau de champs d’adresse réseau qui représentent les adresses par lesquelles cet ordinateur peut être atteint. NetAddress est un **BSTR** spécifique au fournisseur composé de deux sous-chaînes séparées par un signe deux-points ( :). La sous-chaîne de gauche indique le type d’adresse, et la sous-chaîne de droite est une représentation sous forme de chaîne d’une adresse de ce type. Par exemple, les adresses TCP/IP se présentent sous la forme : IP : 100.201.301.45. Les adresses de type IPX se présentent sous la forme : IPX : 10.123456.80.
...
Le système d’exploitation utilisé sur cet ordinateur.
...
**OperatingSystemVersion renvoyée**
</dt> <dd> <dl>
...
Nom de la personne à contacter, par exemple un administrateur, pour cet ordinateur.
...
Identificateur global unique qui identifie le site dans lequel cet ordinateur a été installé. Un site est une région physique de bonne connectivité dans un réseau.
...
Différents fournisseurs peuvent choisir d’exposer différentes propriétés d’un objet ordinateur. Pour plus d’informations, consultez [fournisseurs système ADSI](adsi-system-providers.md).

Vous pouvez découvrir les propriétés prises en charge en inspectant les propriétés obligatoires et facultatives par le biais de sa classe de schéma. Pour plus d’informations, consultez l’interface [**IADsClass**](/windows/desktop/api/Iads/nn-iads-iadsclass) .

Pour examiner l’état d’un ordinateur ou pour effectuer l’opération d’arrêt sur le réseau, vous devez utiliser l’interface [**IADsComputerOperations**](/windows/desktop/api/Iads/nn-iads-iadscomputeroperations) .

## <a name="examples"></a>Exemples

l’exemple de code Visual Basic suivant examine les propriétés de l’ordinateur prises en charge par le fournisseur winnt ADSI.
...
L’exemple de code C++ suivant examine les propriétés de l’ordinateur prises en charge par le fournisseur Winnt ADSI.
...
| IID<br/>                      | IID \_ IADsComputer est défini en tant que EFE3CC70-1D9F-11CF-B1F3-02608C9E7553<br/>         |
...
[Fournisseurs système ADSI](adsi-system-providers.md)
</dt> <dt>
