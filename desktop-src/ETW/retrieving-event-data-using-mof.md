...
Une fois que vous avez trouvé la classe d’événements correcte, énumérez ses classes de type d’événement enfant pour rechercher la classe qui contient le format des données d’événement. La classe de type d’événement correcte contient un qualificateur de classe **eventType** dont la valeur correspond à la valeur du membre **header. class. type** de la structure de [**\_ suivi d’événement**](/windows/win32/api/evntrace/ns-evntrace-event_trace) .

Vous pouvez ensuite utiliser l’API WMI pour énumérer les propriétés de la classe MOF. Utilisez les qualificateurs et le type de données de chaque propriété pour déterminer la taille de l’élément de données dans les données d’événement à lire et comment le mettre en forme. Pour obtenir la liste des qualificateurs MOF pris en charge par ETW, consultez [qualificateurs MOF de suivi d’événements](event-tracing-mof-qualifiers.md).
