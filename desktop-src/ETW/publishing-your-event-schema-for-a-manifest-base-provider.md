...
---
# <a name="publishing-your-event-schema-for-a-manifest-based-provider"></a>Publication de votre schéma d’événement pour un fournisseur basé sur un manifeste

Les fournisseurs [basés sur un manifeste](about-event-tracing.md) utilisent un manifeste pour publier le schéma de leurs événements. Le manifeste est incorporé dans le fichier binaire du fournisseur, ce qui signifie que le fournisseur doit être disponible sur l’ordinateur pour que le consommateur consomme ses événements. Pour plus d’informations sur l’écriture d’un manifeste, consultez [écriture d’un manifeste d’instrumentation](../wes/writing-an-instrumentation-manifest.md).

Le manifeste suivant définit les événements utilisés dans les exemples de la section [fourniture](providing-events.md) d’événements et d' [événements de consommation](consuming-events.md) du document.
