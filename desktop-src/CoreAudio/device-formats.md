...
-   Description du format WAVE \_ \_ WMA \_ SPDIF Wave-format dans la documentation de Windows DDK.
-   Le livre blanc intitulé « prise en charge des pilotes audio pour le format WMA Pro-on-S/PDIF » sur le site Web [des technologies de périphériques audio pour Windows](https://www.microsoft.com/whdc/device/audio/default.mspx) .
...
Le format Mix que le moteur audio utilise pour son traitement interne des flux en mode partagé est étroitement lié à, mais il n’est pas nécessairement identique au format de flux que l’appareil de point de terminaison audio utilise en mode partagé. Dans le panneau de configuration multimédia de Windows, Mmsys.cpl, l’utilisateur peut sélectionner le format de flux qu’un périphérique de point de terminaison audio utilisera lorsqu’il fonctionne en mode partagé. La procédure comporte trois étapes :
