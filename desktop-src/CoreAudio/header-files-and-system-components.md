...
Chaque API du tableau précédent se compose d’un ensemble d’interfaces COM associées. étant donné que certains aspects de la diffusion audio dépendent de la faible latence et de la synchronisation précise, les implémentations des api MMDevice, WASAPI, DeviceTopology et EndpointVolume n’utilisent pas l’environnement Microsoft .NET Framework ou d’exécution managée.
