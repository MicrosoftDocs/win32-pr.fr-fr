...
Sur Xbox One, les fonctionnalités de son spatial Microsoft sont toujours disponibles pour le consommateur et sont activées via l’application paramètres sous **général-> Volume & sortie audio**.
...
Sélectionnez Dolby Atmos pour casque, le casque DTS : X ou Windows Sonic pour casque dans la liste déroulante **format du casque** sous **casque audio** .
...
Sur HoloLens 2, le son spatial Microsoft est activé par défaut et utilise le déchargement matériel DSP conçu spécifiquement pour Windows Sonic pour casque.
...
De nombreux convertisseurs audio ciblent un point de terminaison [**IAudioClient**](/windows/desktop/api/Audioclient/nn-audioclient-iaudioclient) de l’API de session audio Windows (WASAPI), où l’application alimente les mémoires tampons de données audio en mode mixte et conformes au format dans un récepteur audio WASAPI. les mémoires tampons fournies sont ensuite consommées pour être mélangées avec d’autres clients, le traitement au niveau du système final et le rendu.
...
| Dolby Atmos (casque & enceintes intégrées)     | 17 (8.1.4.4)                     | 16                                        | 16                                       | N/D |
| Casque DTS : X (casque) | 17 (8.1.4.4)        | 16                                        | 32                                      | N/D |
...
Le son spatial Microsoft est axé sur la simulation de positionnement du son sur une sphère idéale autour de l’écouteur. Windows Sonic pour casque, le casque DTS : X et Dolby Atmos implémentent la mise en correspondance des intervenants et la virtualisation au casque, mais notez que de nombreux autres aspects de la simulation spatiale sonore, généralement implémentés en mode de création de contenu, sont laissés à des moteurs existants. Les créateurs de contenu continuent à utiliser les outils de jeu existants et les processus qu’ils ont déjà eu pour des indications spatiales comme Doppler, l’atténuation basée sur la distance et le filtrage, l’occlusion et l’obstruction, et l’environnement reverberation.
...
-   Dolby offre un certain nombre de ressources de support liées à Dolby Atmos et à l’application Dolby Access à l’adresse <https://developer.dolby.com> .
