...
TraceLogging offre la simplicité du préprocesseur de suivi logiciel Windows (WPP) avec l’avantage supplémentaire de pouvoir inclure des données structurées avec des événements, la possibilité de mettre en corrélation des événements et tout cela sans nécessiter de fichier XML de manifeste d’instrumentation distinct. Les événements sont auto-descriptifs, ce qui signifie qu’un binaire contenant le manifeste d’instrumentation n’a pas besoin d’être inscrit sur le système pour pouvoir utiliser les API TDH (Trace Data Helper) pour décoder et afficher les événements.
