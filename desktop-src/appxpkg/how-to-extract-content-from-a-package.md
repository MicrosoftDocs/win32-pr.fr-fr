...
ms.locfileid: "119049047"
---
# <a name="extract-app-package-contents-c"></a>Extraire le contenu du package d’application (C++)

découvrez comment extraire des fichiers du package d’application pour une application Windows à l’aide de l' [API de packaging](interfaces.md).

Vous pouvez également utiliser l’outil MakeAppx.exe pour extraire des fichiers d’un package d’application ou d’un bundle. Pour plus d’informations, consultez [extraire des fichiers d’un package ou d’une offre groupée](/windows/msix/package/create-app-package-with-makeappx-tool#extract-files-from-a-package-or-bundle) .

### <a name="create-a-package-reader"></a>Créer un lecteur de package

Pour créer un lecteur de package, appelez la méthode [**IAppxFactory :: CreatePackageReader**](/windows/desktop/api/AppxPackaging/nf-appxpackaging-iappxfactory-createpackagereader) . Le premier paramètre est un flux d’entrée pour le package (fichier. AppX). Le deuxième paramètre est un paramètre de sortie qui reçoit un pointeur vers un pointeur [**IAppxPackageReader**](/windows/desktop/api/AppxPackaging/nn-appxpackaging-iappxpackagereader) .
...
Le code précédent utilise ces définitions de variables et la `GetOutputStream` fonction d’assistance.
...
### <a name="extract-payload-files"></a>Extraire les fichiers de charge utile

Appelez la méthode [**IAppxPackageReader :: GetPayloadFiles**](/windows/desktop/api/AppxPackaging/nf-appxpackaging-iappxpackagereader-getpayloadfiles) pour énumérer les fichiers de charge utile. Chaque fichier de charge utile est représenté par une interface [**IAppxFile**](/windows/desktop/api/AppxPackaging/nn-appxpackaging-iappxfile) . La `ExtractFile` fonction de cet exemple utilise les méthodes [**GetName**](/windows/desktop/api/AppxPackaging/nf-appxpackaging-iappxfile-getname), [**getContentType**](/windows/desktop/api/AppxPackaging/nf-appxpackaging-iappxfile-getcontenttype)et [](/windows/desktop/api/AppxPackaging/nf-appxpackaging-iappxfile-getsize) Quantity de **IAppxFile** pour afficher des informations de base sur le fichier de charge utile.

Ce code utilise la `ExtractFile` fonction d’assistance indiquée à l’étape précédente pour créer le flux du manifeste du package.
...
### <a name="clean-up-the-package-reader"></a>Nettoyer le lecteur de package

Avant de retourner à partir de la `wmain` fonction, appelez la méthode [**Release**](/windows/desktop/api/unknwn/nf-unknwn-iunknown-release) pour nettoyer le lecteur de package et appeler la fonction [**CoUninitialize**](/windows/desktop/api/combaseapi/nf-combaseapi-couninitialize) .
...
[Exemple d’extraction de contenu de package d’application](https://github.com/microsoft/Windows-classic-samples/tree/master/Samples/AppxPackingExtractAppx)
</dt> <dt>
