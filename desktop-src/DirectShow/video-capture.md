---
description: découvrez comment utiliser Microsoft DirectShow pour écrire des applications de capture vidéo à l’aide des articles liés dans cet article.
ms.assetid: 92051a84-5011-42ed-be68-e8841552ca1b
title: Capture vidéo (DirectShow)
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: b9d63da84f74152c11e1126d11f6dfc450d8547b
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 09/13/2021
ms.locfileid: "127240563"
---
# <a name="video-capture-directshow"></a>Capture vidéo (DirectShow)

cette section décrit comment utiliser Microsoft DirectShow pour écrire des applications de capture vidéo.

-   [À propos de la capture vidéo dans DirectShow](about-video-capture-in-directshow.md)
-   [Sélection d’un périphérique de capture](selecting-a-capture-device.md)
-   [Aperçu de la vidéo](previewing-video.md)
-   [Capture vidéo dans un fichier](capturing-video-to-a-file.md)
-   [Contrôle d’un Graph de capture](controlling-a-capture-graph.md)
-   [Tâches de capture vidéo](video-capture-tasks.md)
-   [Vidéo numérique en DirectShow](digital-video-in-directshow.md)
-   [Contrôle d’un caméscope DV](controlling-a-dv-camcorder.md)
-   [Télévision analogique](analog-television.md)
-   [Rubriques avancées sur la capture](advanced-capture-topics.md)

## <a name="related-topics"></a>Rubriques connexes

<dl> <dt>

[Utilisation de DirectShow](using-directshow.md)
</dt> </dl>

 

 



