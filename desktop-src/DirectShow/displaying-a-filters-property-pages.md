...
Si vous avez créé directement une instance d’un filtre (en appelant **CoCreateInstance**), vous avez déjà un pointeur vers le filtre. Si ce n’est pas le cas, vous pouvez énumérer les filtres dans le graphique à l’aide de la méthode [**IFilterGraph :: EnumFilters**](/windows/desktop/api/Strmif/nf-strmif-ifiltergraph-enumfilters) . Pour plus d’informations, consultez [énumération d’objets dans un graphique de filtre](enumerating-objects-in-a-filter-graph.md).
