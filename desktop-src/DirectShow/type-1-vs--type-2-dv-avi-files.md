...
En revanche, les fichiers de type 1 sont moins efficaces pour la modification en temps réel. L’application doit extraire le contenu audio du flux entrelacé, effectuer les modifications et entrelacer à nouveau les données. en outre, le format type-1 n’est pas compatible avec Microsoft® Video for Windows® (VFW). DirectShow peut gérer les deux types de fichiers.
