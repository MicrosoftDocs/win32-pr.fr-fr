---
description: Types de média MPEG-2
ms.assetid: 20fb2dc3-76b5-421a-b8e1-d99fad2ce2dd
title: Types de média MPEG-2
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 51c9cfdbc306a486ed284d67453d080b4b09a0b8
ms.sourcegitcommit: c16214e53680dc71d1c07111b51f72b82a4512d8
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 03/03/2021
ms.locfileid: "103953416"
---
# <a name="mpeg-2-media-types"></a>Types de média MPEG-2

Cette section répertorie les types de médias utilisés pour les données MPEG-2.

-   [Types de média MPEG-2 Splitter](mpeg-2-splitter-media-types.md)
-   [Types de média du démultiplexeur MPEG-2](mpeg-2-demultiplexer-media-types.md)
-   [**Types de média de noyau MPEG-2**](mpeg-2-kernel-media-types.md)

## <a name="related-topics"></a>Rubriques connexes

<dl> <dt>

[**Types de supports DVD**](dvd-media-types.md)
</dt> </dl>

 

 



