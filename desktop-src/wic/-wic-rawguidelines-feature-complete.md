...
| <a href="/windows/desktop/api/Wincodecsdk/nn-wincodecsdk-iwicmetadatablockreader"><strong>IWICMetadataBlockReader</strong></a> | Décodeurs | Requis pour énumérer et itérer au sein des blocs de métadonnées et appeler les lecteurs de métadonnées appropriés lors de la lecture à partir d’un fichier image. <br /><blockquote>[!Note]<br />Si le format de conteneur brut est compatible TIFF ou utilise des IFD ou IRBs standard pour stocker les métadonnées EXIF ou XMP, les auteurs de codec peuvent choisir d’appeler les lecteurs de métadonnées intégrés plutôt que de les écrire.</blockquote><br /><br /> | 
