...
Handle des informations d’identification du serveur. Le serveur appelle la fonction [**AcquireCredentialsHandle (Kerberos)**](acquirecredentialshandle--kerberos.md) avec l’indicateur SECPKG \_ cred \_ entrant ou SECPKG \_ cred \_ défini pour récupérer ce handle.
...
Pointeur vers une structure [CtxtHandle](sspi-handles.md) . Lors du premier appel à **AcceptSecurityContext (Kerberos)**, ce pointeur est **null**. Lors des appels suivants, *phContext* est le handle du contexte partiellement formé qui a été retourné dans le paramètre *phNewContext* par le premier appel.
...
Pointeur vers une structure [**SecBufferDesc**](/windows/win32/api/sspi/ns-sspi-secbufferdesc) générée par un appel client à [**InitializeSecurityContext (Kerberos)**](initializesecuritycontext--kerberos.md) qui contient le descripteur de mémoire tampon d’entrée.
...
Pointeur vers une structure [CtxtHandle](sspi-handles.md) . Lors du premier appel à **AcceptSecurityContext (Kerberos)**, ce pointeur reçoit le nouveau handle de contexte. Lors des appels suivants, *phNewContext* peut être le même que le handle spécifié dans le paramètre *phContext* .
...
2.  Le serveur appelle **AcceptSecurityContext (Kerberos)** la première fois, qui génère un jeton de réponse qui est ensuite envoyé au client.
3.  Le client reçoit le jeton et le passe à [**InitializeSecurityContext (Kerberos)**](initializesecuritycontext--kerberos.md). Si **InitializeSecurityContext (Kerberos)** retourne sec \_ E \_ OK, l’authentification mutuelle est terminée et une session sécurisée peut commencer. Si **InitializeSecurityContext (Kerberos)** retourne un code d’erreur, la négociation de l’authentification mutuelle se termine. Dans le cas contraire, le jeton de sécurité retourné par **InitializeSecurityContext (Kerberos)** est envoyé au client et les étapes 2 et 3 sont répétées.
