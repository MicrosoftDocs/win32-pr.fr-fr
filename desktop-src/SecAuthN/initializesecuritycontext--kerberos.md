...
Handle des informations d’identification retournées par [**AcquireCredentialsHandle (Kerberos)**](acquirecredentialshandle--kerberos.md). Ce descripteur est utilisé pour générer le [*contexte de sécurité*](../secgloss/s-gly.md). La fonction **InitializeSecurityContext (Kerberos)** requiert au moins des informations d’identification sortantes.
...
Pointeur vers une structure [CtxtHandle](sspi-handles.md) . Lors du premier appel à **InitializeSecurityContext (Kerberos)**, ce pointeur est **null**. Lors du deuxième appel, ce paramètre est un pointeur vers le handle vers le contexte partiellement formé retourné dans le paramètre *phNewContext* par le premier appel.
...
Pointeur vers une structure [CtxtHandle](sspi-handles.md) . Lors du premier appel à **InitializeSecurityContext (Kerberos)**, ce pointeur reçoit le nouveau handle de contexte. Lors du deuxième appel, *phNewContext* peut être le même que le handle spécifié dans le paramètre *phContext* .
...
Pointeur vers une structure d' [**horodatage**](timestamp.md) qui reçoit l’heure d’expiration du contexte. Nous recommandons que le [*package de sécurité*](../secgloss/s-gly.md) retourne toujours cette valeur en heure locale. Étant donné que ce paramètre est facultatif, la **valeur null** doit être transmise pour les clients à courte durée de vie.
...
|----------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <dl> <dt>**SEC \_ E \_ OK**</dt> </dl>                      | Le [*contexte de sécurité*](../secgloss/s-gly.md) a été initialisé avec succès. Il n’est pas nécessaire d’avoir un autre appel de [**InitializeSecurityContext (Kerberos)**](initializesecuritycontext--kerberos.md) . Si la fonction retourne un jeton de sortie, autrement dit, si le \_ jeton SECBUFFER dans *pOutput* a une longueur différente de zéro, ce jeton doit être envoyé au serveur.<br/> |
| <dl> <dt>**s \_ j’ai \_ terminé \_ et je \_ continue**</dt> </dl> | Le client doit appeler [**CompleteAuthToken**](/windows/win32/api/sspi/nf-sspi-completeauthtoken) , puis transmettre la sortie au serveur. Le client attend ensuite un jeton retourné et le transmet, dans un autre appel, à [**InitializeSecurityContext (Kerberos)**](initializesecuritycontext--kerberos.md).<br/>                                                                                                                                  |
...
| <dl> <dt>**SEC \_ je \_ continue à avoir \_ besoin**</dt> </dl>        | Le client doit envoyer le jeton de sortie au serveur et attendre un jeton de retour. Le jeton retourné est ensuite passé dans un autre appel à [**InitializeSecurityContext (Kerberos)**](initializesecuritycontext--kerberos.md). Le jeton de sortie peut être vide.<br/>                                                                                                                                                       |
...
La fonction **InitializeSecurityContext (Kerberos)** est utilisée par un client pour initialiser un contexte sortant.
...
3.  Le client envoie le jeton retourné dans la mémoire tampon *pOutput* au serveur cible. Le serveur transmet ensuite le jeton en tant qu’argument d’entrée dans un appel à la fonction [**AcceptSecurityContext (Kerberos)**](acceptsecuritycontext--kerberos.md) .
4.  [**AcceptSecurityContext (Kerberos)**](acceptsecuritycontext--kerberos.md) peut retourner un jeton, que le serveur envoie au client pour un deuxième appel à **InitializeSecurityContext (Kerberos)** si le premier appel a retourné sec \_ \_ \_ .
...
3.  Lors de la réception de la réponse du serveur, le client appelle à nouveau **InitializeSecurityContext (Kerberos)** , avec *phContext* défini sur le descripteur retourné à partir du dernier appel. Le jeton reçu du serveur est fourni dans le paramètre *pInput* .
...
L’action effectuée par le client dépend du code de retour de cette fonction. Si le code de retour est SEC \_ E \_ OK, il n’y aura aucun deuxième appel de **InitializeSecurityContext (Kerberos)** et aucune réponse du serveur ne sera attendue. Si le code de retour est \_ sec \_ , je continue \_ nécessaire, le client attend un jeton en réponse du serveur et le transmet dans un deuxième appel à **InitializeSecurityContext (Kerberos)**. Le code de retour de SEC \_ I \_ \_ requis indique que le client doit terminer la génération du message et appeler la fonction [**CompleteAuthToken**](/windows/win32/api/sspi/nf-sspi-completeauthtoken) . Les secondes \_ que j’ai \_ effectuées \_ et qui \_ poursuivent le code incorporent ces deux actions.

Si **InitializeSecurityContext (Kerberos)** renvoie une réussite sur le premier appel (ou uniquement), l’appelant doit finalement appeler la fonction [**DeleteSecurityContext**](/windows/win32/api/sspi/nf-sspi-deletesecuritycontext) sur le handle retourné, même si l’appel échoue sur un tronçon ultérieur de l’échange d’authentification.

Le client peut appeler **InitializeSecurityContext (Kerberos)** une fois qu’il s’est terminé avec succès. Cela indique au [*package de sécurité*](../secgloss/s-gly.md) qu’une réauthentification est souhaitée.
