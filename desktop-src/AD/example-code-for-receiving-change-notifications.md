...
---
# <a name="example-code-for-receiving-change-notifications"></a>Exemple de code pour recevoir des notifications de modification

L’exemple de code suivant montre comment utiliser le contrôle de notification de modification LDAP pour recevoir des notifications de modifications apportées à un objet dans Active Directory Domain Services. L’exemple s’inscrit pour les notifications, lit l’état initial de l’objet, puis utilise une boucle pour attendre et traiter les modifications apportées à l’objet.

Tout d’abord, l’exemple de code appelle la fonction [**LDAP \_ Search \_ ext**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_ext) , qui est une opération de recherche asynchrone qui retourne après l’inscription d’une demande de notification. Deuxièmement, après la configuration de la demande de notification, l’exemple appelle la fonction [**LDAP \_ Search \_ s**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_s) , qui est une opération de recherche synchrone qui lit l’état actuel de l’objet. Troisièmement, l’exemple utilise une boucle qui appelle \_ le résultat LDAP pour attendre les résultats de l’opération de recherche asynchrone. Lorsque la fonction [**LDAP \_ result**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) est retournée, l’exemple traite les résultats de la recherche et répète la boucle.

Sachez que si l’exemple lit l’état actuel, puis configurez la demande de notification, il se peut qu’il y ait un laps de temps pendant lequel les modifications peuvent se produire avant l’inscription de la demande de notification. En lisant l’objet après avoir configuré la demande de notification, la fenêtre fonctionne en sens inverse, vous pouvez recevoir des notifications des modifications qui se sont produites avant la lecture de l’état initial. Pour gérer cela, l’exemple met en cache la valeur de l’attribut [**uSNChanged**](/windows/desktop/ADSchema/a-usnchanged) de l’objet lorsqu’il lit l’état initial de l’objet. Ensuite, lorsque [**le \_ résultat LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) est retourné avec une notification de modification, l’exemple compare la valeur de l’attribut **uSNChanged** mis en cache avec la valeur signalée par le **\_ résultat LDAP**. Si la nouvelle valeur de l’attribut **uSNChanged** est inférieure ou égale à la valeur mise en cache, l’exemple ignore les résultats, car ils indiquent une modification qui s’est produite avant l’opération de lecture initiale.

Cet exemple effectue une recherche au niveau de la base qui surveille un seul objet. Vous pouvez spécifier l’étendue du niveau d' **\_ \_ étendue LDAP** pour surveiller tous les objets enfants de l’objet spécifié. Vous pouvez également modifier le code pour inscrire jusqu’à cinq demandes de notification, puis utiliser la fonction de [**\_ résultat LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) pour attendre les notifications de toutes les demandes. N’oubliez pas que les demandes de notification de modification ont un impact sur les performances du serveur. Pour plus d’informations sur l’amélioration des performances, consultez [notifications de modification dans Active Directory Domain Services](change-notifications-in-active-directory-domain-services.md).
