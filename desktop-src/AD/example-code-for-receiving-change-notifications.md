---
title: Exemple de code pour recevoir des notifications de modification
description: L’exemple de code suivant montre comment utiliser le contrôle de notification de modification LDAP pour recevoir des notifications de modifications apportées à un objet dans Active Directory Domain Services.
ms.assetid: e1d36d1c-ee50-4c2f-92f6-eee1dae1c5af
ms.tgt_platform: multiple
keywords:
- Active Directory des exemples Active Directory, réception de notifications de modification
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103940825"
---
# <a name="example-code-for-receiving-change-notifications"></a><span data-ttu-id="5a8f4-104">Exemple de code pour recevoir des notifications de modification</span><span class="sxs-lookup"><span data-stu-id="5a8f4-104">Example Code for Receiving Change Notifications</span></span>

<span data-ttu-id="5a8f4-105">L’exemple de code suivant montre comment utiliser le contrôle de notification de modification LDAP pour recevoir des notifications de modifications apportées à un objet dans Active Directory Domain Services.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-105">The following code example shows how to use the LDAP change notification control to receive notifications of changes to an object in Active Directory Domain Services.</span></span> <span data-ttu-id="5a8f4-106">L’exemple s’inscrit pour les notifications, lit l’état initial de l’objet, puis utilise une boucle pour attendre et traiter les modifications apportées à l’objet.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-106">The example registers for notifications, reads the initial state of the object, and then uses a loop to wait for and process changes to the object.</span></span>

<span data-ttu-id="5a8f4-107">Tout d’abord, l’exemple de code appelle la fonction [**LDAP \_ Search \_ ext**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_ext) , qui est une opération de recherche asynchrone qui retourne après l’inscription d’une demande de notification.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-107">First, the code example calls the [**ldap\_search\_ext**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_ext) function, which is an asynchronous search operation that returns after registering a notification request.</span></span> <span data-ttu-id="5a8f4-108">Deuxièmement, après la configuration de la demande de notification, l’exemple appelle la fonction [**LDAP \_ Search \_ s**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_s) , qui est une opération de recherche synchrone qui lit l’état actuel de l’objet.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-108">Second, after setting up the notification request, the example calls the [**ldap\_search\_s**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_s) function, which is a synchronous search operation that reads the current state of the object.</span></span> <span data-ttu-id="5a8f4-109">Troisièmement, l’exemple utilise une boucle qui appelle \_ le résultat LDAP pour attendre les résultats de l’opération de recherche asynchrone.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-109">Third, the example, uses a loop that calls ldap\_result to wait for results of the asynchronous search operation.</span></span> <span data-ttu-id="5a8f4-110">Lorsque la fonction [**LDAP \_ result**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) est retournée, l’exemple traite les résultats de la recherche et répète la boucle.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-110">When the [**ldap\_result**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) function returns, the example processes the search results and repeats the loop.</span></span>

<span data-ttu-id="5a8f4-111">Sachez que si l’exemple lit l’état actuel, puis configurez la demande de notification, il se peut qu’il y ait un laps de temps pendant lequel les modifications peuvent se produire avant l’inscription de la demande de notification.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-111">Be aware that if the example read the current state and then set up the notification request, there would be a period of time during which changes could occur before the notification request was registered.</span></span> <span data-ttu-id="5a8f4-112">En lisant l’objet après avoir configuré la demande de notification, la fenêtre fonctionne en sens inverse, vous pouvez recevoir des notifications des modifications qui se sont produites avant la lecture de l’état initial.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-112">By reading the object after setting up the notification request, the window works in reverse–you could receive notifications of changes that occurred before reading the initial state.</span></span> <span data-ttu-id="5a8f4-113">Pour gérer cela, l’exemple met en cache la valeur de l’attribut [**uSNChanged**](/windows/desktop/ADSchema/a-usnchanged) de l’objet lorsqu’il lit l’état initial de l’objet.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-113">To handle this, the example caches the object [**uSNChanged**](/windows/desktop/ADSchema/a-usnchanged) attribute value when it reads the object's initial state.</span></span> <span data-ttu-id="5a8f4-114">Ensuite, lorsque [**le \_ résultat LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) est retourné avec une notification de modification, l’exemple compare la valeur de l’attribut **uSNChanged** mis en cache avec la valeur signalée par le **\_ résultat LDAP**.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-114">Then, when [**ldap\_result**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) returns with a change notification, the example compares the cached **uSNChanged** attribute value with the value reported by **ldap\_result**.</span></span> <span data-ttu-id="5a8f4-115">Si la nouvelle valeur de l’attribut **uSNChanged** est inférieure ou égale à la valeur mise en cache, l’exemple ignore les résultats, car ils indiquent une modification qui s’est produite avant l’opération de lecture initiale.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-115">If the new **uSNChanged** attribute value is less than or equal to the cached value, the example discards the results because they indicate a change that occurred prior to the initial read operation.</span></span>

<span data-ttu-id="5a8f4-116">Cet exemple effectue une recherche au niveau de la base qui surveille un seul objet.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-116">This example performs a base level search that monitors a single object.</span></span> <span data-ttu-id="5a8f4-117">Vous pouvez spécifier l’étendue du niveau d' **\_ \_ étendue LDAP** pour surveiller tous les objets enfants de l’objet spécifié.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-117">You can specify the **LDAP\_SCOPE\_ONELEVEL** scope to monitor all child objects of the specified object.</span></span> <span data-ttu-id="5a8f4-118">Vous pouvez également modifier le code pour inscrire jusqu’à cinq demandes de notification, puis utiliser la fonction de [**\_ résultat LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) pour attendre les notifications de toutes les demandes.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-118">You can also modify the code to register up to five notification requests and then use the [**ldap\_result**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) function to wait for notifications from any of the requests.</span></span> <span data-ttu-id="5a8f4-119">N’oubliez pas que les demandes de notification de modification ont un impact sur les performances du serveur.</span><span class="sxs-lookup"><span data-stu-id="5a8f4-119">Remember that change notification requests impact the performance of the server.</span></span> <span data-ttu-id="5a8f4-120">Pour plus d’informations sur l’amélioration des performances, consultez [notifications de modification dans Active Directory Domain Services](change-notifications-in-active-directory-domain-services.md).</span><span class="sxs-lookup"><span data-stu-id="5a8f4-120">For more information about how to improve performance, see [Change Notifications in Active Directory Domain Services](change-notifications-in-active-directory-domain-services.md).</span></span>
