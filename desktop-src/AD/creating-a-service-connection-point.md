...
---
# <a name="creating-a-service-connection-point"></a>Création d’un point de connexion de service

L’exemple de code suivant montre comment créer et initialiser un point de connexion de service. L’exemple de code exécute des étapes supplémentaires qui permettent au service de mettre à jour les valeurs SCP au moment de l’exécution. En règle générale, un programme d’installation de service effectue ces étapes dans le cadre de l’installation d’une instance de service sur un ordinateur hôte.

Cet exemple de code crée l’objet SCP en tant qu’objet enfant pour l’objet de l’ordinateur local. Elle utilise la fonction [**GetComputerObjectName**](/windows/desktop/api/secext/nf-secext-getcomputerobjectnamea) pour récupérer le DN de l’objet ordinateur local. Il utilise ensuite le DN pour établir une liaison avec un pointeur [**IDirectoryObject**](/windows/desktop/api/iads/nn-iads-idirectoryobject) pour l’objet ordinateur. La méthode [**IDirectoryObject :: CreateDSObject**](/windows/desktop/api/iads/nf-iads-idirectoryobject-createdsobject) crée le SCP et spécifie des valeurs initiales pour les attributs SCP importants.

[**CreateDSObject**](/windows/desktop/api/iads/nf-iads-idirectoryobject-createdsobject) retourne un pointeur vers le nouveau SCP, que l’exemple de code utilise pour récupérer un pointeur [**IADs**](/windows/desktop/api/iads/nn-iads-iads) pour le SCP. L’exemple de code utilise des méthodes **IADs** pour récupérer les attributs **objectGUID** et **distinguishedName** du SCP. L’exemple de code utilise l' **objectGUID** pour composer une chaîne utilisée pour effectuer une liaison au SCP, puis met en cache la chaîne de liaison GUID dans le registre local où elle peut être récupérée par le service au moment de l’exécution. Le **distinguishedName** est retourné à l’appelant de fonction pour une utilisation dans la composition d’un nom de principal du service (SPN) pour l’instance de service.

L’exemple de code suivant appelle cette fonction dans le cadre des étapes de base de l’installation d’un service prenant en charge les annuaires. Pour plus d’informations, consultez [installation d’un service sur un ordinateur hôte](installing-a-service-on-a-host-computer.md).
