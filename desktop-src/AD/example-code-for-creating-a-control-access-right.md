...
ms.locfileid: "118962408"
---
# <a name="example-code-for-creating-a-control-access-right"></a>Exemple de code pour la création d’un droit d’accès de contrôle

l’exemple de Visual Basic suivant crée un objet **controlAccessRight** dans le conteneur Extended-Rights.
...
L’exemple de code C++ suivant est une fonction qui crée un objet **controlAccessRight** dans le conteneur Extended-Rights. Quand vous appelez cette fonction, utilisez le format suivant pour spécifier la chaîne GUID pour le paramètre *pszRightsGUID* .
...
Le tableau **ADSVALUE** pour la propriété **appliesTo** utilise le même format GUID et définit le membre **dwType** sur **ADSTYPE \_ case \_ ignore \_ String**.
...
Cet exemple de fonction **CreateExtendedRight** peut être appelé avec l’exemple de code suivant.
