---
title: Exemple de code pour la vérification d’un droit d’accès de contrôle dans l’ACL d’un objet
description: L’exemple suivant peut être utilisé pour vérifier que l’utilisateur actuellement connecté dispose d’autorisations pour un droit d’accès de contrôle sur l’objet spécifié.
...
ms.locfileid: "103839337"
---
# <a name="example-code-for-checking-a-control-access-right-in-an-objects-acl"></a><span data-ttu-id="8f1d8-104">Exemple de code pour la vérification d’un droit d’accès de contrôle dans l’ACL d’un objet</span><span class="sxs-lookup"><span data-stu-id="8f1d8-104">Example Code for Checking a Control Access Right in an Object's ACL</span></span>

<span data-ttu-id="8f1d8-105">L’exemple suivant peut être utilisé pour vérifier que l’utilisateur actuellement connecté dispose d’autorisations pour un droit d’accès de contrôle sur l’objet spécifié.</span><span class="sxs-lookup"><span data-stu-id="8f1d8-105">The following example can be used to verify that the currently logged-on user has permissions for a control access right on the specified object.</span></span>
