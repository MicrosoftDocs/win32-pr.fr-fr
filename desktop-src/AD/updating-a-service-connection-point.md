...
---
# <a name="updating-a-service-connection-point"></a><span data-ttu-id="95757-104">Mise à jour d’un point de connexion de service</span><span class="sxs-lookup"><span data-stu-id="95757-104">Updating a Service Connection Point</span></span>

<span data-ttu-id="95757-105">L’exemple de code suivant montre comment mettre à jour un point de connexion de service.</span><span class="sxs-lookup"><span data-stu-id="95757-105">The following code example shows how to update a service connection point.</span></span> <span data-ttu-id="95757-106">Ce code est généralement exécuté par un service au démarrage.</span><span class="sxs-lookup"><span data-stu-id="95757-106">This code is typically executed by a service when it starts.</span></span>

<span data-ttu-id="95757-107">Cet exemple récupère la chaîne de liaison du GUID du SCP que le programme d’installation du service a mis en cache dans le registre.</span><span class="sxs-lookup"><span data-stu-id="95757-107">This example retrieves the SCP's GUID binding string that the service installer cached in the registry.</span></span> <span data-ttu-id="95757-108">Elle utilise cette chaîne pour établir une liaison à un pointeur [**IDirectoryObject**](/windows/desktop/api/iads/nn-iads-idirectoryobject) sur l’objet SCP, puis appelle la méthode [**IDirectoryObject :: GetObjectAttributes**](/windows/desktop/api/iads/nf-iads-idirectoryobject-getobjectattributes) pour récupérer les attributs **servicednsname** et **serviceBindingInformation** du SCP.</span><span class="sxs-lookup"><span data-stu-id="95757-108">It uses this string to bind to an [**IDirectoryObject**](/windows/desktop/api/iads/nn-iads-idirectoryobject) pointer on the SCP object, and then calls the [**IDirectoryObject::GetObjectAttributes**](/windows/desktop/api/iads/nf-iads-idirectoryobject-getobjectattributes) method to get the **serviceDNSName** and **serviceBindingInformation** attributes of the SCP.</span></span> <span data-ttu-id="95757-109">N’oubliez pas que votre service peut nécessiter la vérification et la mise à jour d’attributs supplémentaires.</span><span class="sxs-lookup"><span data-stu-id="95757-109">Be aware that your service may require to verify and update additional attributes.</span></span>

<span data-ttu-id="95757-110">Le code compare la valeur **servicednsname** au nom DNS retourné par la fonction [**GetComputerNameEx**](/windows/desktop/api/sysinfoapi/nf-sysinfoapi-getcomputernameexa) .</span><span class="sxs-lookup"><span data-stu-id="95757-110">The code compares the **serviceDNSName** value to the DNS name returned by the [**GetComputerNameEx**](/windows/desktop/api/sysinfoapi/nf-sysinfoapi-getcomputernameexa) function.</span></span> <span data-ttu-id="95757-111">Il compare également le numéro de port actuel du service au numéro de port stocké dans l’attribut **serviceBindingInformation** .</span><span class="sxs-lookup"><span data-stu-id="95757-111">It also compares the service's current port number to the port number stored in the **serviceBindingInformation** attribute.</span></span> <span data-ttu-id="95757-112">Si l’une de ces valeurs a changé, le code appelle la méthode [**IDirectoryObject :: SetObjectAttributes**](/windows/desktop/api/iads/nf-iads-idirectoryobject-setobjectattributes) pour mettre à jour les attributs SCP.</span><span class="sxs-lookup"><span data-stu-id="95757-112">If either of these values have changed, the code calls the [**IDirectoryObject::SetObjectAttributes**](/windows/desktop/api/iads/nf-iads-idirectoryobject-setobjectattributes) method to update the SCP attributes.</span></span>
