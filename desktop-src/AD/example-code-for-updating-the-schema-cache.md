...
ms.locfileid: "104028610"
---
# <a name="example-code-for-updating-the-schema-cache"></a><span data-ttu-id="aaabe-104">Exemple de code pour la mise à jour du cache de schéma</span><span class="sxs-lookup"><span data-stu-id="aaabe-104">Example Code for Updating the Schema Cache</span></span>

<span data-ttu-id="aaabe-105">Les fonctions C/C++ suivantes récupèrent le nom DNS du contrôleur de schéma (GetSchemaMasterDNSName) et mettent à jour le cache de schéma sur le contrôleur de schéma (UpdateSchemaMasterCache).</span><span class="sxs-lookup"><span data-stu-id="aaabe-105">The following C/C++ functions get the DNS name of the schema master (GetSchemaMasterDNSName) and update the schema cache at the schema master (UpdateSchemaMasterCache).</span></span> <span data-ttu-id="aaabe-106">Pour utiliser ces fonctions, appelez GetSchemaMasterDNSName pour récupérer le nom DNS du contrôleur de schéma, transmettez le nom DNS du contrôleur de schéma à UpdateSchemaMasterCache, appelez CoTaskMemFree sur la chaîne contenant le nom DNS du contrôleur de schéma.</span><span class="sxs-lookup"><span data-stu-id="aaabe-106">To use these functions, call GetSchemaMasterDNSName to get the schema master DNS name, pass the schema master DNS name to UpdateSchemaMasterCache, call CoTaskMemFree on string containing the schema master DNS name.</span></span>

<span data-ttu-id="aaabe-107">Pour plus d’informations, consultez [mise à jour du cache de schéma](updating-the-schema-cache.md).</span><span class="sxs-lookup"><span data-stu-id="aaabe-107">For more information, see [Updating the Schema Cache](updating-the-schema-cache.md).</span></span>
