---
title: Lecture de l’objectGUID d’un objet et création d’une représentation sous forme de chaîne du GUID
description: Utilisez la méthode de récupération \_ des identificateurs des objets IADs pour récupérer la forme de chaîne pouvant être liée de l’objectGUID d’un objet d’annuaire.
...
---
# <a name="reading-an-objects-objectguid-and-creating-a-string-representation-of-the-guid"></a><span data-ttu-id="29c1e-103">Lecture de l’objectGUID d’un objet et création d’une représentation sous forme de chaîne du GUID</span><span class="sxs-lookup"><span data-stu-id="29c1e-103">Reading an Object's objectGUID and Creating a String Representation of the GUID</span></span>

<span data-ttu-id="29c1e-104">La propriété **objectGUID** de chaque objet dans Active Directory Domain Services est stockée dans le répertoire sous la forme d’une chaîne d’octets.</span><span class="sxs-lookup"><span data-stu-id="29c1e-104">The **objectGUID** property of each object in Active Directory Domain Services is stored in the directory as an octet string.</span></span> <span data-ttu-id="29c1e-105">Une chaîne d’octets est un tableau de caractères d’un octet.</span><span class="sxs-lookup"><span data-stu-id="29c1e-105">An octet string is an array of one-byte characters.</span></span> <span data-ttu-id="29c1e-106">Utilisez la méthode de [**\_ GUID IADs :: obtenir**](/windows/desktop/ADSI/iads-property-methods) pour récupérer la forme de chaîne pouvant être liée de l' **objectGUID** d’un objet d’annuaire.</span><span class="sxs-lookup"><span data-stu-id="29c1e-106">Use the [**IADs::get\_GUID**](/windows/desktop/ADSI/iads-property-methods) method to retrieve the bindable string form of a directory object's **objectGUID**.</span></span>

<span data-ttu-id="29c1e-107">Les exemples de code suivants illustrent une fonction qui lit l’attribut **objectGUID** et retourne une représentation sous forme de chaîne du GUID utilisé pour la liaison à l’objet.</span><span class="sxs-lookup"><span data-stu-id="29c1e-107">The following code examples show a function that reads the **objectGUID** attribute and returns a string representation of the GUID used to bind to the object.</span></span>

-   [<span data-ttu-id="29c1e-108">Exemple de Visual Basic</span><span class="sxs-lookup"><span data-stu-id="29c1e-108">Visual Basic Example</span></span>](#visual-basic-example)
-   [<span data-ttu-id="29c1e-109">Exemple C++</span><span class="sxs-lookup"><span data-stu-id="29c1e-109">C++ Example</span></span>](#c-example)

## <a name="visual-basic-example"></a><span data-ttu-id="29c1e-110">Exemple de Visual Basic</span><span class="sxs-lookup"><span data-stu-id="29c1e-110">Visual Basic Example</span></span>
...
## <a name="c-example"></a><span data-ttu-id="29c1e-111">Exemple C++</span><span class="sxs-lookup"><span data-stu-id="29c1e-111">C++ Example</span></span>
