...
La propriété **Status** récupère le code d’État http de la dernière réponse.
...
Valeur de type **long** qui reçoit le code d’état retourné.
...
L’exemple suivant montre comment ouvrir une connexion HTTP, envoyer une requête HTTP, afficher l' **État** et [**StatusText**](iwinhttprequest-statustext.md), et lire les en-têtes de réponse. Cet exemple doit être exécuté à partir d’une invite de commandes.
...
L’exemple de script suivant montre comment ouvrir une connexion HTTP, envoyer une requête HTTP, afficher l' **État** et [**StatusText**](iwinhttprequest-statustext.md), et lire le texte de la réponse.
