...
Représente l’état du service d’échange de paires clé/valeur, qui fournit un mécanisme permettant d’échanger des données entre l’ordinateur virtuel et le système d’exploitation s’exécutant sur le système d’exploitation de gestion.
...
La classe **MSVM \_ KvpExchangeComponent** possède les types de membres suivants :
...
La classe **MSVM \_ KvpExchangeComponent** possède ces méthodes.
...
La classe **MSVM \_ KvpExchangeComponent** possède les propriétés suivantes.
...
État activé de l’élément. Cette propriété est héritée de la [**\_ EnabledLogicalElement CIM**](/previous-versions//cc136818(v=vs.85)).
...
Tableau d’instances [**MSVM \_ KvpExchangeDataItem**](msvm-kvpexchangedataitem.md) incorporées qui contiennent l’ensemble de paires clé-valeur que les services en cours d’exécution dans le système d’exploitation invité ont poussés pour pouvoir accéder aux clients externes. Ce tableau ne contient pas d’éléments intrinsèques qui sont envoyés directement par le service d’intégration.
...
Tableau d’instances [**MSVM \_ KvpExchangeDataItem**](msvm-kvpexchangedataitem.md) incorporées qui contiennent l’ensemble de paires clé-valeur que le système d’exploitation invité a envoyées pour pouvoir être accessibles aux clients externes. Ce tableau ne contient pas d’éléments de données envoyés par d’autres services exécutés dans le système d’exploitation invité.
...
Date ou heure de la dernière modification de l’état activé de l’élément. Cette propriété est héritée de [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85)), mais elle n’est pas utilisée.
