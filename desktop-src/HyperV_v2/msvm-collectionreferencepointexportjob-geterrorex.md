...
Récupère des informations supplémentaires sur une erreur. Lorsque le travail est en cours d’exécution ou s’est terminé sans erreur, **GetErrorEx** ne retourne aucune instance d' [**\_ erreur MSVM**](msvm-error.md) . Toutefois, si la tâche a échoué en raison d’un problème interne ou parce que la tâche a été arrêtée par un client, **GetErrorEx** retourne une ou plusieurs instances d' **\_ erreur MSVM** .
