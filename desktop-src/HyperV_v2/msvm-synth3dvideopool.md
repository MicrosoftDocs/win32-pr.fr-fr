...
Contient des informations sur les unités de traitement graphique (GPU) vidéo 3D synthétiques disponibles sur le système hôte. Cette classe est utilisée uniquement avec les systèmes hôtes qui prennent en charge RemoteFX.
...
La classe **MSVM \_ Synth3dVideoPool** possède les types de membres suivants :
...
La classe **MSVM \_ Synth3dVideoPool** possède ces méthodes.
...
La classe **MSVM \_ Synth3dVideoPool** possède les propriétés suivantes.
...
Unités d’allocation utilisées par le pool de ressources. Cette propriété est héritée de la [**\_ ResourcePool CIM**](/previous-versions//cc136903(v=vs.85)).
...
Spécifie les unités pour les propriétés **MaxConsumableResource** et **CurrentlyConsumedResource** . Cette propriété est héritée de la [**\_ ResourcePool CIM**](/previous-versions//cc136903(v=vs.85)).
...
Spécifie la quantité de ressources que le pool de ressources présente actuellement aux consommateurs. Cette propriété est différente de la propriété **réservée** en ce qu’elle décrit la vue consommateurs de la ressource, tandis que la propriété **réservée** décrit la vue des producteurs de la ressource. Cette propriété est héritée de la [**\_ ResourcePool CIM**](/previous-versions//cc136903(v=vs.85)).
...
Spécifie la version de DirectX la plus ancienne prise en charge par les cartes au sein du pool de ressources.
...
État actuel de l’élément. Cette propriété est héritée de [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)et est toujours définie sur 2 (OK). Hyper-V n’utilise jamais le premier élément de ce tableau.
...
Spécifie la version la plus basse de DirectX requise par les cartes dans le pool de ressources.
...
Type de ressource que ce pool de ressources peut allouer. Cette propriété est héritée de la [**\_ ResourcePool CIM**](/previous-versions//cc136903(v=vs.85))et est toujours définie sur 4 (« mémoire »).
...
Chaînes qui décrivent les différentes valeurs de tableau [**OperationalStatus**](msvm-bioselement.md) . Cette propriété est héritée de [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)et est toujours définie sur « OK ». Hyper-V n’utilise jamais le premier élément de ce tableau.
