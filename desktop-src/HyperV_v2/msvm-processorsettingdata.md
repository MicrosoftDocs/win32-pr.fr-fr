...
---
# <a name="msvm_processorsettingdata-class"></a>MSVM \_ ProcessorSettingData, classe

Représente les paramètres de processeur virtuel pour un ordinateur virtuel.
...
La classe **MSVM \_ ProcessorSettingData** possède les types de membres suivants :

-   [Propriétés](#properties)

### <a name="properties"></a>Propriétés

La classe **MSVM \_ ProcessorSettingData** possède les propriétés suivantes.
...
Décrit la visibilité du consommateur sur la ressource allouée. Cette propriété est héritée de la [**\_ ResourceAllocationSettingData CIM**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata).
...
ID du groupe de processeurs auquel cette machine virtuelle est liée. Lorsque la valeur est 0, cela signifie qu’elle n’est pas liée à un groupe de PROCESSEURs spécifique.
...
Nom complet de l’objet. Cette propriété est héritée de la [**\_ SettingData CIM**](/previous-versions//cc136911(v=vs.85)). La modification de cette propriété modifie le **ElementName** du dérivé de l’appareil logique associé.
...
Indique si la machine virtuelle doit activer les fonctionnalités qui augmentent la protection des ressources de l’hôte de la charge de travail en cours d’exécution dans la machine virtuelle.
...
Indique si Hyper-V doit exposer les extensions de virtualisation de matériel virtualisées à la machine virtuelle.
...
Indique le nombre de threads SMT par cœur signalés à l’invité. Ce rapport est indépendant du fait que le matériel pour SMT est présent.
...
Quantité maximale de ressources processeur qui peuvent être consommées par l’ordinateur virtuel. Cette propriété est héritée de la [**\_ ResourceAllocationSettingData CIM**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata).
...
Indique si l’ordinateur virtuel doit réduire l’identificateur de l’UC. Certains systèmes d’exploitation plus anciens peuvent vous obliger à limiter les fonctionnalités du processeur de cette façon afin de les exécuter.
...
Indique si l’ordinateur virtuel doit limiter les fonctionnalités de l’UC exposées au système d’exploitation. Le fait de limiter les fonctionnalités du processeur permet à la machine virtuelle d’être migrée vers différents systèmes d’ordinateurs hôtes avec des processeurs différents. La migration d’ordinateurs virtuels entre des ordinateurs équipés de processeurs de différents fournisseurs n’est pas prise en charge.
...
Nombre maximal de nœuds NUMA qui peuvent être observés dans l’ordinateur virtuel comme appartenant à un seul socket de processeur.
...
Nombre maximal de processeurs virtuels qui peuvent être observés dans l’ordinateur virtuel comme appartenant à un seul nœud NUMA virtuel.
...
L’accès à la classe **MSVM \_ ProcessorSettingData** peut être limité par le filtrage UAC. Pour plus d’informations, consultez [contrôle de compte d’utilisateur et WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi).
