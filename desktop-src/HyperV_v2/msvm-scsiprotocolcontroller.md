...
Représente un contrôleur SCSI synthétique. Un contrôleur SCSI synthétique peut prendre en charge jusqu’à 256 lecteurs.
...
La classe **MSVM \_ SCSIProtocolController** possède les types de membres suivants :
...
La classe **MSVM \_ SCSIProtocolController** possède ces méthodes.
...
La classe **MSVM \_ SCSIProtocolController** possède les propriétés suivantes.
...
Nombre maximal d’unités qui peuvent être contrôlées par ou accessibles via ce contrôleur de protocole. Cette propriété est héritée de la [**\_ ProtocolController CIM**](/previous-versions/windows/desktop/iscsitarg/cim-protocolcontroller).
...
Identifie la manière dont le nom du contrôleur de protocole SCSI est sélectionné. Cette propriété est héritée de la [**\_ SCSIProtocolController CIM**](/previous-versions/windows/desktop/iscsitarg/cim-protocolcontroller).
...
Chaîne qui décrit la façon dont le contrôleur de protocole est identifié lorsque la valeur de la méthode **NameFormat** est 1 (autre). Cette propriété est héritée de la [**\_ SCSIProtocolController CIM**](/previous-versions/windows/desktop/iscsitarg/cim-protocolcontroller).
