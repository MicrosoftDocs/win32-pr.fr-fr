...
Point de communication utilisé pour envoyer et recevoir des données entre les systèmes, les interfaces d’ordinateur et les réseaux logiques.
...
La **classe \_ ProtocolEndpoint CIM** possède les types de membres suivants :

-   [Propriétés](#properties)

### <a name="properties"></a>Propriétés

La **classe \_ ProtocolEndpoint CIM** possède ces propriétés.
...
Qualificateurs : [**override**](/windows/desktop/WmiSdk/standard-qualifiers) ("Description"), [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| If-MIB. ifDescr ")
</dt> </dl>
...
Qualificateurs : [**override**](/windows/desktop/WmiSdk/standard-qualifiers) ("EnabledState"), [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| If-MIB. ifAdminStatus ")
</dt> </dl>

État activé d’un élément.
...
Qualificateurs : [**override**](/windows/desktop/WmiSdk/standard-qualifiers) ("Name"), [**MaxLen**](/windows/desktop/WmiSdk/standard-qualifiers) (256)
</dt> </dl>

Identificateur unique du point de terminaison de protocole qui indique la fonctionnalité managée. La Convention d’affectation de noms pour cette propriété est définie dans la propriété **NameFormat** .
...
Convention d’affectation de noms utilisée par la propriété **Name** pour garantir l’unicité des valeurs de **nom** . Par exemple, vous pouvez ajouter la valeur de la propriété **ProtocolIFType** au début du nom, suivi d’un trait de soulignement.
...
Qualificateurs : [**override**](/windows/desktop/WmiSdk/standard-qualifiers) ("OperationalStatus"), [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| If-MIB. ifOperStatus ")
</dt> </dl>

Tableau qui contient l’état actuel de l’élément. La première valeur du tableau **OperationalStatus** doit contenir l’état principal de l’élément.
...
Qualificateurs : [**MaxLen**](/windows/desktop/WmiSdk/standard-qualifiers) (64), [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("**\_ ProtocolEndpoint CIM**.**ProtocolType**« , »**CIM \_ ProtocolEndpoint**.**ProtocolIFType**")
</dt> </dl>

Description d’un type de protocole réseau qui est utilisé lorsque la propriété **ProtocolIFType** est définie sur « 1 » (autre); dans le cas contraire, cette valeur doit être définie sur null.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| If-MIB. ifType "), [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("**\_ ProtocolEndpoint CIM**.**OtherTypeDescription**")
</dt> </dl>

Énumération utilisée pour classer et classer différentes instances de cette classe. Les valeurs possibles de cette propriété sont synchronisées avec l’IANA (Internet Assigned Numbers Authority) ifType (Management Information base), qui est conservée dans https://www.iana.org/assignments/ianaiftype-mib . Des valeurs supplémentaires définies par le DMTF sont incluses.

> [!Note]  
> Si **ProtocolIFType** a la valeur 1 (autre), les informations sur le type de protocole doivent être fournies dans la propriété **OtherTypeDescription** .
...
**Série point à point propriétaire** (22)
...
**Propriétaire virtuel/interne** (53)
...
**Interconnexion du relais de trames** (58)
...
**Réseau local émulé ATM pour 802,3** (59)
...
**Réseau local émulé ATM pour 802,5** (60)
...
**Commutation des liaisons de données** (74)
...
**Pontage d’itinéraires source distants** (79)
...
**Système de rapports ext POS Loc** (87)
...
**Protocole d’accès à distance AppleTalk** (88)
...
**Propriétaire sans connexion** (89)
...
**Bloc-notes de l’hôte ITU X. 29** (90)
...
**Réception et transmission vocales** (100)
...
**Bundle à liaisons multiples PPP** (108)
...
Carte **Token Ring ISO 802.5 j** (115)
...
**IP (pour APPN HPR dans les réseaux IP)** (126)
...
**Commutateur 12MPP de Avalon** (130)
...
**Service d’émulation de circuit** (133)
...
**VLAN de couche 2 avec 802.1 q** (135)
...
**Réseau local virtuel de couche 3 utilisant IP** (136)
...
**Réseau local virtuel de couche 3 utilisant IPX** (137)
...
**Messagerie multimédia sur IP** (139)
...
**RFC1483 Multiprotocol sur ATM** (159)
...
**Agrégat de liens ad IEEE 802.3** (161)
...
**Gestion des stratégies BGP** (162)
...
**Lien de signalisation à plusieurs fréquences** (167)
...
**Liaison de données de l’installation DS1** (170)
...
**Signal non associé à une installation** (175)
...
**Couche Mac sans fil propriétaire** (180)
...
En **aval sans fil propriétaire** (181)
...
**Flux de communication sans fil propriétaire** (182)
...
**Point d’accès sans fil haut débit propriétaire vers Mulipoint** (184)
...
**Canal de surcharge du wrapper numérique** (186)
...
**Point de terminaison DLCI de relais de trame** (193)
...
**Point de terminaison VCI ATM** (194)
...
**Groupe de fonctionnalités vocal E&M** (211)
...
**Point de terminaison de canal B RNIS** (4106)
...
**Point de terminaison de canal D RNIS** (4107)
...
Qualificateurs : [**déconseillé**](/windows/desktop/WmiSdk/standard-wmi-qualifiers) ("**\_ ProtocolEndpoint CIM**.**ProtocolIFType**"), [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("**\_ ProtocolEndpoint CIM**.**OtherTypeDescription**")
</dt> </dl>

> [!Note]  
> Description déconseillée : énumération utilisée pour classer et classer les différentes instances de cette classe.

 

Cette propriété est déconseillée. Utilisez plutôt la propriété **ProtocolIFType** .
...
**Point de terminaison BRI RNIS** (19)
...
**Point de terminaison de canal B RNIS** (20)
...
**Point de terminaison de canal D RNIS** (21)
...
Qualificateurs : [**override**](/windows/desktop/WmiSdk/standard-qualifiers) ("TimeOfLastStateChange"), [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| If-MIB. ifLastChange ")
</dt> </dl>

Date et heure de la dernière modification de la propriété **EnabledState** . Si **EnabledState** n’a pas changé et que cette propriété est remplie, elle doit être définie sur une valeur d’intervalle égale à zéro. Si une modification d’État a été demandée, mais rejetée ou n’a pas encore été traitée, la propriété ne doit pas être mise à jour.
