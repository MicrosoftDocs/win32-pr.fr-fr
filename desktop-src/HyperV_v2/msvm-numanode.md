...
---
# <a name="msvm_numanode-class"></a>MSVM \_ NumaNode, classe

Représente un nœud NUMA (non-Uniform Memory Access) d’un système virtuel.
...
La classe **MSVM \_ NumaNode** possède les types de membres suivants :
...
La classe **MSVM \_ NumaNode** possède ces méthodes.
...
La classe **MSVM \_ NumaNode** possède les propriétés suivantes.
...
Nombre de blocs de mémoire actuellement disponibles pour la consommation par les ordinateurs virtuels.
...
Nombre total de cœurs de processeurs logiques dans ce nœud.
...
Nombre total de cœurs de processeur dans ce nœud NUMA. Cela peut être différent du nombre d' [**objets \_ processeur MSVM**](msvm-processor.md) associés à ce nœud si chaque cœur de processeur prend en charge plusieurs threads de calcul.
