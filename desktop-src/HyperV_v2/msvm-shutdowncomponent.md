...
Représente l’état du service d’arrêt, qui fournit un mécanisme permettant d’arrêter le système d’exploitation de l’ordinateur virtuel à partir des interfaces de gestion sur le système hôte.
...
La classe **MSVM \_ ShutdownComponent** possède les types de membres suivants :
...
La classe **MSVM \_ ShutdownComponent** possède ces méthodes.
...
| [**InitiateReboot**](msvm-shutdowncomponent-initiatereboot.md)         | Lance une opération de redémarrage du système d’exploitation sur l’ordinateur virtuel enfant associé.<br/> |
| [**InitiateShutdown**](msvm-shutdowncomponent-initiateshutdown.md)     | Lance une opération d’arrêt du système d’exploitation sur l’ordinateur virtuel enfant associé.<br/>   |
...
La classe **MSVM \_ ShutdownComponent** possède les propriétés suivantes.
...
État activé de l’élément. Cette propriété est héritée de [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85)) et a la valeur 2 (activé) ou 3 (désactivé).
...
Tableau de chaînes de forme libre qui fournissent des explications et des détails sur les entrées du tableau **OtherIdentifyingInfo** . Cette propriété est héritée de [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice), mais elle n’est pas utilisée.
