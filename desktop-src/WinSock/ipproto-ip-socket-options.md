...
| \_contexte de \_ redirection \_ WFP IP | Oui | Oui | WSACMSGHDR avec données de contrôle | un type de données connexe de socket datagramme ( \_ type cmsg) pour indiquer le contexte de redirection pour un socket UDP utilisé par un mode utilisateur Windows service de redirection de plateforme de filtrage (WFP). |
| \_enregistrements de \_ redirection \_ WFP IP | Oui | Oui | WSACMSGHDR avec données de contrôle | un type de données connexe de socket datagramme ( \_ type cmsg) pour indiquer l’enregistrement de redirection pour un socket UDP utilisé par un mode utilisateur Windows service de redirection de plateforme de filtrage (WFP). |

## <a name="windows-support-for-ip_proto-options"></a>prise en charge Windows pour les \_ options IP PROTO
...
| \_RECVIF IP | à partir de Windows 10, version 1703 | x | x | x | x | x |
| \_RECVTTL IP | x | | | | | |
| \_TOS IP | x | x | x | | | |
| \_TTL IP | x | x | x | x | x | x |
