---
description: Contient les définitions des termes de sécurité qui commencent par la lettre G.
ROBOTS: NOINDEX, NOFOLLOW
ms.assetid: c9567a5b-bd56-4ae1-9eac-af0bb5a6842a
title: G (Glossaire sécurité)
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 07cf3c047f6bfb1192c4049a408d98e3197bd743
ms.sourcegitcommit: 831e8f3db78ab820e1710cede244553c70e50500
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 01/08/2021
ms.locfileid: "103866638"
---
# <a name="g-security-glossary"></a>G (Glossaire sécurité)

[A](a-gly.md) [B](b-gly.md) [C](c-gly.md) [D](d-gly.md) [e](e-gly.md) F G [H](h-gly.md) [I](i-gly.md) J [K](k-gly.md) [L](l-gly.md) [M](m-gly.md) [N](n-gly.md) [O](o-gly.md) [P](p-gly.md) Q [R](r-gly.md) [S](s-gly.md) [T](t-gly.md) [U](u-gly.md) [V](v-gly.md) [W](w-gly.md) [X](x-gly.md) Y Z

<dl> <dt>

<span id="_security_gdi_gly"></span><span id="_SECURITY_GDI_GLY"></span>**GDI**
</dt> <dd>

Consultez *Graphics Device Interface*.

</dd> <dt>

<span id="_security_gina_gly"></span><span id="_SECURITY_GINA_GLY"></span>**GINA**
</dt> <dd>

Bibliothèque de liens dynamiques (DLL) d’identification et d’authentification graphique. GINA est un composant DLL remplaçable qui est chargé par l’exécutable [*Winlogon*](w-gly.md) . GINA implémente la stratégie d’authentification du modèle de connexion interactive et est censée effectuer toutes les interactions utilisateur d’identification et d’authentification.

</dd> <dt>

<span id="_security_graphics_device_interface_gly"></span><span id="_SECURITY_GRAPHICS_DEVICE_INTERFACE_GLY"></span>**Graphics Device Interface**
</dt> <dd>

Programme exécutable qui traite les appels de fonction graphique d’une application Windows et transmet ces appels au pilote de périphérique approprié, qui exécute les fonctions spécifiques au matériel qui génèrent la sortie.

</dd> </dl>

 

 



