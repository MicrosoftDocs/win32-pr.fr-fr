...
À compter de Windows 8, l’API [DirectWrite](direct-write-portal.md) prend également en charge un certain nombre de nouvelles métriques afin d’exprimer des informations utiles sur les polices pour votre application. Ces nouvelles métriques incluent ces informations.
...
vous utilisez l’énumération de l' [**\_ ID de \_ chaîne \_ d’information DWRITE**](/windows/win32/api/dwrite/ne-dwrite-dwrite_informational_string_id) pour obtenir le nom complet, le nom PostScript et le nom CID PostScript de toute police sur le système. Ces informations sont utiles lorsque vous devez faire correspondre les polices de votre application aux polices appropriées sur le système local.
