---
description: Cette section contient des informations de référence pour les macros fournies par Direct3D.
ms.assetid: cabb715a-2a6a-4d56-8113-5e5ed26fc107
title: Macros (Direct3D 9)
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 99a957ca9d15784e995ec3bd145cc55387151797
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 09/13/2021
ms.locfileid: "127124456"
---
# <a name="macros-direct3d-9"></a>Macros (Direct3D 9)

Cette section contient des informations de référence pour les macros fournies par Direct3D.

-   [**D3DCLIPPLANEn**](d3dclipplanen.md)
-   [**D3DCOLOR \_ ARGB**](d3dcolor-argb.md)
-   [**D3DCOLOR \_ AYUV**](d3dcolor-ayuv.md)
-   [**D3DCOLOR \_ COLORVALUE**](d3dcolor-colorvalue.md)
-   [**D3DCOLOR \_ RVBA**](d3dcolor-rgba.md)
-   [**D3DCOLOR \_ XRGB**](d3dcolor-xrgb.md)
-   [**D3DCOLOR \_ XYUV**](d3dcolor-xyuv.md)
-   [**Fin de D3DDECL \_**](d3ddecl-end.md)
-   [**D3DFVF \_ TEXCOORDSIZEN**](d3dfvf-texcoordsizen.md)
-   [**\_Vidage D3DGETDATA**](d3dgetdata-flush.md)
-   [**Début de D3DISSUE \_**](d3dissue-begin.md)
-   [**Fin de D3DISSUE \_**](d3dissue-end.md)
-   [**VERSION de D3DPS \_**](d3dps-version.md)
-   [**D3DTS \_ World**](d3dts-world.md)
-   [**D3DTS \_ WORLDMATRIX**](d3dts-worldmatrix.md)
-   [**D3DTS \_ worldn**](d3dts-worldn.md)
-   [**VERSION de D3DVS \_**](d3dvs-version.md)

### <a name="requirements"></a>Spécifications

Toutes les macros Direct3D présentent la configuration requise suivante :



| Condition requise | Valeur |
|--------|-------------|
| En-tête | d3d9types. h |



 

## <a name="related-topics"></a>Rubriques connexes

<dl> <dt>

[Référence Direct3D](dx9-graphics-reference-d3d.md)
</dt> </dl>

 

 



