---
description: 'Les exemples suivants montrent comment utiliser les objets de synchronisation :'
ms.assetid: 19e33f9f-a9c0-44db-becf-9af3895eb182
title: Utilisation de la synchronisation
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 6a1e4d1970ea9e311bd44a9e53e006e488661fcb
ms.sourcegitcommit: 831e8f3db78ab820e1710cede244553c70e50500
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 01/08/2021
ms.locfileid: "104209670"
---
# <a name="using-synchronization"></a>Utilisation de la synchronisation

Les exemples suivants montrent comment utiliser les objets de synchronisation :

-   [En attente de plusieurs objets](waiting-for-multiple-objects.md)
-   [Utilisation d’objets nommés](using-named-objects.md)
-   [Utilisation d’objets d’événement](using-event-objects.md)
-   [Utilisation d’objets mutex](using-mutex-objects.md)
-   [Utilisation d’objets Semaphore](using-semaphore-objects.md)
-   [Utilisation d’objets Timer waitables](using-waitable-timer-objects.md)
-   [Utilisation de minuteries waitables avec un appel de procédure asynchrone](using-a-waitable-timer-with-an-asynchronous-procedure-call.md)
-   [Utilisation d’objets de section critique](using-critical-section-objects.md)
-   [Utilisation de variables de condition](using-condition-variables.md)
-   [Utilisation de l’initialisation unique](using-one-time-initialization.md)
-   [Utilisation de listes liées de façon unique](using-singly-linked-lists.md)
-   [Utilisation des files d’attente du minuteur](using-timer-queues.md)

 

 



