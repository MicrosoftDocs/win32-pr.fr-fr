...
Au cœur de MSMQ se trouve la base de données MQIS (message queue Information Service), qui s’exécute sur SQL Server. Une entreprise possède un MQIS maître unique, appelé contrôleur d’entreprise principal. Chaque site dispose de son propre MQIS, appelé *contrôleur de site principal* et zéro ou plusieurs *contrôleurs de site de sauvegarde*. Enfin, il existe des ordinateurs clients individuels, chacun ayant son propre gestionnaire de files d’attente, implémenté comme un service. Le contrôleur principal d’entreprise peut également être un contrôleur principal de site, et tout contrôleur peut également être un client.
