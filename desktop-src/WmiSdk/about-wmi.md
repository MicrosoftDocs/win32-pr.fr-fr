...
de nombreuses fonctionnalités de Windows ont des fournisseurs WMI associés, comme le [fournisseur de Données de configuration de démarrage (BCD) (BCD)](/previous-versions/windows/desktop/bcd/boot-configuration-data-portal) ou le [fournisseur de Volume Stockage](/previous-versions/windows/desktop/vdswmi/storage-volume-provider). les fournisseurs wmi implémentent la fonctionnalité décrite par les méthodes et propriétés des classes wmi pour gérer les fonctionnalités de Windows associées. Pour plus d’informations, consultez [fournisseurs WMI](wmi-providers.md) et [classes WMI](wmi-classes.md).
