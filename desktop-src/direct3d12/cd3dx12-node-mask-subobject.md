...
description: Classe d’assistance pour la création d’un sous-objet State qui identifie les nœuds GPU auxquels s’applique l’objet d’État.
keywords:
...
ms.locfileid: "121812906"
---
# <a name="cd3dx12_node_mask_subobject-class"></a>Classe CD3DX12_NODE_MASK_SUBOBJECT

Classe d’assistance pour la création d’un sous-objet State qui identifie les nœuds GPU auxquels s’applique l’objet d’État.
...
Constructeur par défaut. Crée une nouvelle instance, initialisée par défaut, d’un **CD3DX12_NODE_MASK_SUBOBJECT**.

`CD3DX12_NODE_MASK_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC&)`

Constructeur qui crée une nouvelle instance d’un **CD3DX12_NODE_MASK_SUBOBJECT** initialisé avec le contenu d’un objet [**CD3DX12_STATE_OBJECT_DESC**](cd3dx12-state-object-desc.md) .

`SetNodeMask(UINT)`

Fonction permettant de définir le masque de nœud.

`Type`

Récupère le type du sous-objet, représenté par la constante [D3D12_STATE_SUBOBJECT_TYPE_NODE_MASK](/windows/win32/api/d3d12/ne-d3d12-d3d12_state_subobject_type) .
...
Opérateur de conversion qui retourne une référence à une constante [D3D12_NODE_MASK](/windows/win32/api/d3d12/ns-d3d12-d3d12_node_mask) objet décrivant l’objet d’État.
