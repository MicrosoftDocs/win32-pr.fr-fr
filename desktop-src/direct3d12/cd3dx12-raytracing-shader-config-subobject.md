...
description: Classe d’assistance pour la création d’un sous-objet d’état de configuration du nuanceur Raytracing.
keywords:
...
ms.locfileid: "121812920"
---
# <a name="cd3dx12_raytracing_shader_config_subobject-class"></a>Classe CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT

Classe d’assistance pour la création d’un sous-objet d’état de configuration du nuanceur Raytracing.
...
Constructeur par défaut. Crée une nouvelle instance, initialisée par défaut, d’un **CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT**.

`CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC&)`

Constructeur qui crée une nouvelle instance d’un **CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT** initialisé avec le contenu d’un objet [**CD3DX12_STATE_OBJECT_DESC**](cd3dx12-state-object-desc.md) .

`Config(UINT, UINT)`

Fonction permettant de configurer la taille maximale de la charge utile et la taille maximale de l’attribut (en octets).

`Type`

Récupère le type du sous-objet, représenté par la constante [D3D12_STATE_SUBOBJECT_TYPE_RAYTRACING_SHADER_CONFIG](/windows/win32/api/d3d12/ne-d3d12-d3d12_state_subobject_type) .
...
Opérateur de conversion qui retourne une référence à une constante [D3D12_RAYTRACING_SHADER_CONFIG](/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_shader_config) objet décrivant l’objet d’État.
