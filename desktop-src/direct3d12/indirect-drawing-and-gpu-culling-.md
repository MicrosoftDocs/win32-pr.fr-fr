...
-   [Définir les commandes indirectes](#define-the-indirect-commands)
-   [Créer un graphique et une signature racine de calcul](#create-a-graphics-and-compute-root-signature)
-   [Créer une vue de ressource de nuanceur (SRV) pour le nuanceur de calcul](#create-a-shader-resource-view-srv-for-the-compute-shader)
...
## <a name="define-the-indirect-commands"></a>Définir les commandes indirectes

Nous commençons par définir à quoi ressemblent les commandes indirectes. Dans cet exemple, les commandes que vous souhaitez exécuter sont les suivantes :

<dl> 1. Mettez à jour la vue de la mémoire tampon constante (CBV).  
2. Dessinez le triangle.  
</dl>

Ces commandes de dessin sont représentées par la structure suivante dans la définition de la classe **D3D12ExecuteIndirect** . Les commandes sont exécutées de façon séquentielle dans l’ordre dans lequel elles sont définies dans cette structure.
...
|--------------------------------------------------------|------------|
| \_Adresse virtuelle du GPU D3D12 \_ \_ (tout simplement UINT64)         |            |
...
Pour accompagner la structure de données, une signature de commande est également créée, qui indique au GPU comment interpréter les données transmises à l’API [**ExecuteIndirect**](/windows/desktop/api/d3d12/nf-d3d12-id3d12graphicscommandlist-executeindirect) . Cela, et la plus grande partie du code suivant, est ajouté à la méthode **LoadAssets** .
...
| Workflow d’appel                                                               | Paramètres                                                              |
|-------------------------------------------------------------------------|-------------------------------------------------------------------------|
| [**Description de l' \_ argument D3D12 indirect \_ \_**](/windows/desktop/api/d3d12/ns-d3d12-d3d12_indirect_argument_desc) | [**\_Type d' \_ argument \_ indirect D3D12**](/windows/desktop/api/d3d12/ne-d3d12-d3d12_indirect_argument_type) |
...
## <a name="create-a-graphics-and-compute-root-signature"></a>Créer un graphique et une signature racine de calcul

Nous créons également un graphique et une signature racine de calcul. La signature racine Graphics définit simplement un CBV racine. Notez que nous allons mapper l’index de ce paramètre racine dans [**l' \_ argument indirect D3D12 \_ \_ desc**](/windows/desktop/api/d3d12/ns-d3d12-d3d12_indirect_argument_desc) (indiqué ci-dessus) lorsque la signature de la commande est définie. La signature de la racine de calcul définit :

-   Un tableau de descripteurs commun avec trois emplacements (deux SRV et un UAV) :
    -   Un SRV expose les mémoires tampons constantes au nuanceur de calcul
    -   Un SRV expose le tampon de commande au nuanceur de calcul
    -   Le UAV est l’emplacement où le nuanceur de calcul enregistre les commandes pour les triangles visibles
-   Quatre constantes racine :
    -   Moitié de la largeur d’un côté du triangle
    -   Position z des sommets du triangle
    -   Décalage +/-x du plan d’élimination dans un espace homogène \[ -1, 1\]
    -   Nombre de commandes indirectes dans la mémoire tampon de commande
...
## <a name="create-the-indirect-command-buffers"></a>Créer les mémoires tampons de commande indirectes

Nous créons ensuite les mémoires tampons de commande indirectes et définissons leur contenu à l’aide du code suivant. Nous dessinons les mêmes sommets de triangle 1024 fois, mais vous pointez vers un emplacement de mémoire tampon constante différent avec chaque appel de dessin.
...
| Workflow d’appel                    | Paramètres                                                          |
|------------------------------|---------------------------------------------------------------------|
| \_ \_ Adresse virtuelle du GPU D3D12 \_ | [**GetGPUVirtualAddress**](/windows/desktop/api/d3d12/nf-d3d12-id3d12resource-getgpuvirtualaddress) |



 

Après avoir chargé les tampons de commande dans le GPU, nous créons également un SRV de ceux-ci pour que le nuanceur de calcul les lise. Cela est très similaire au SRV créé de la mémoire tampon constante.
...
## <a name="create-the-compute-uavs"></a>Créer le calcul UAVs

Nous devons créer le UAVs qui stockera les résultats du travail de calcul. Lorsqu’un triangle est considéré par le nuanceur de calcul comme visible pour la cible de rendu, il est ajouté à ce UAV, puis utilisé par l’API [**ExecuteIndirect**](/windows/desktop/api/d3d12/nf-d3d12-id3d12graphicscommandlist-executeindirect) .
...
Ensuite, nous allons exécuter les commandes dans UAV (élimination du GPU activée) ou dans le tampon de commandes complet (élimination du GPU désactivée).
...
Si nous sommes en mode d’élimination du GPU, la file d’attente de commandes Graphics attend la fin du travail de calcul avant de commencer à exécuter les commandes indirectes. Dans la méthode **OnRender** , l’extrait de code suivant est ajouté.
...
[Dessin indirect](indirect-drawing.md)
</dt> </dl>
