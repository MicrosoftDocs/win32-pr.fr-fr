...
---
# <a name="directml-helper-functions"></a>Fonctions d’assistance DirectML
...
En d’autres termes, la taille minimale d’un tenseur est l’index de l’élément le plus ancien, multiplié par la taille de l’élément (par exemple, 2 octets pour un tenseur **FLOAT16** ). En outre, DirectML exige que toutes les mémoires tampons liées aient une taille totale qui est alignée sur la **valeur DWORD**. par conséquent, la taille implicite minimale en octets doit être arrondie à la limite de 4 octets la plus proche.
...
Cette fonction d’assistance calcule des Strides pour les dizaines de dizaines de NCHW ou de NHWC, et une diffusion facultative.
