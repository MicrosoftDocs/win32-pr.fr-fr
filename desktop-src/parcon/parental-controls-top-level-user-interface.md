...
le lien fort entre la sécurité des familles et les comptes d’utilisateurs de Windows a entraîné une catégorie combinée apparaissant sous la forme de **comptes d’utilisateur et de sécurité des familles** dans le panneau de configuration pour les références de Windows Vista orientées consommateurs.
...
-   une section Paramètres avec le lien restrictions Web implémentées par microsoft, qui est indiquée de manière conditionnelle, et les types de restrictions non mis en œuvre principaux et hors connexion disponibles sont indiqués.
-   plus Paramètres zone qui s’affiche si les extensions d’Interface utilisateur sont inscrites par Microsoft ou par des tiers.
