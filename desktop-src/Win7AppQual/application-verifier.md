---
description: Application Verifier (Windows 7 et Windows Server 2008 R2 livre de recettes sur la qualité des applications)
...
-   nous avons combiné les versions 32 et 64 bits du package pour traiter les modifications apportées à Windows 7, y compris les besoins de test des composants 32 bits sous une version 64 bits de Windows, ainsi que pour une simplification générale.
-   nous avons inclus des vérifications supplémentaires pour les applications multithread, en exécutant des applications 32 bits sur les Windows 64 bits, ainsi que de nombreux correctifs de bogues.
...
-   **détails des arrêts TLS** -garantit que les api Thread Local Stockage sont utilisées correctement
...
notez que certaines de ces vérifications s’effectuent uniquement pour les Windows 7, tandis que d’autres s’exécutent simplement sous Windows 7.
...
1.  Inscrivez votre société à WinQual, ce qui nécessite un ID VeriSign. vous trouverez Windows 7 informations sur WinQual dans le portail des développeurs, regroupés sous Windows Vista SP1 \\ Windows Server 2008. il aura bientôt un emplacement Windows 7.
