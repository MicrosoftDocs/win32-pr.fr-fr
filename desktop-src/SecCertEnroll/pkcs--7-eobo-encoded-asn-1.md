---
description: L’exemple suivant contient une \# demande d’inscription PKCS 7 pour le compte de (EOBO) indiquée dans le format ASN. 1. L’exemple a été généré à l’aide des outils Certreq.exe et Certutil.exe. Le fichier. inf utilisé comme entrée pour Certreq.exe contient la configuration suivante.
ms.assetid: b6945071-8019-40bb-9f71-13ae3bd57948
title: '\#ASN 7 EOBO encodé ASN. 1'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "120127204"
---
# <a name="pkcs-7-eobo-encoded-asn1"></a>\#ASN 7 EOBO encodé ASN. 1

L’exemple suivant contient une \# demande d’inscription PKCS 7 pour le compte de (EOBO) indiquée dans le format ASN. 1. L’exemple a été généré à l’aide des outils Certreq.exe et Certutil.exe. Le fichier. inf utilisé comme entrée pour Certreq.exe contient la configuration suivante.
...
Cette configuration génère l’exemple de sortie suivant. La configuration spécifie le type de demande (PKCS \# 7), le nom de l’entité qui demande l’inscription et le nom du modèle. Le modèle utilisateur spécifie que :
