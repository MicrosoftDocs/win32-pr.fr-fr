---
description: L’exemple suivant contient une demande CMC ENROLL pour le compte de (EOBO) présentée dans le format ASN. 1. L’exemple a été généré à l’aide des outils Certreq.exe et Certutil.exe. Le fichier. inf utilisé comme entrée pour Certreq.exe contient la configuration suivante.
ms.assetid: 6a339085-de9e-4854-9d17-b8586b0ef5a5
title: ASN EOBO encodé par CMC. 1
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "118902035"
---
# <a name="cmc-eobo-encoded-asn1"></a>ASN EOBO encodé par CMC. 1

L’exemple suivant contient une demande CMC ENROLL pour le compte de (EOBO) présentée dans le format ASN. 1. L’exemple a été généré à l’aide des outils Certreq.exe et Certutil.exe. Le fichier. inf utilisé comme entrée pour Certreq.exe contient la configuration suivante.
...
Cette configuration génère l’exemple de sortie suivant. La configuration spécifie le type de demande (CMC), le nom de l’entité qui demande l’inscription et le nom du modèle. Le modèle utilisateur spécifie que :
