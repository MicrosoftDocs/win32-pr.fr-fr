...
---
# <a name="httpcalculatorwithusernamexmltokenoversslclient-example"></a>Exemple HttpCalculatorWithUsernameXmlTokenOverSslClient

Cet exemple montre un client HTTP qui utilise le proxy de service pour communiquer avec un service de calculatrice, avec un jeton de sécurité XML sur la sécurité SSL en mode mixte. Dans cette configuration, la connexion de transport est protégée (signée, chiffrée) par SSL, ce qui permet également l’authentification du serveur. L’authentification du client est fournie par un WS-Security paire nom d’utilisateur/mot de passe utilisée comme jeton de sécurité XML par l’exemple.

Notez que cet exemple côté client utilise la [**\_ liaison de \_ \_ sécurité de \_ message \_ de jeton WS XML**](/windows/desktop/api/WebServices/ns-webservices-ws_xml_token_message_security_binding). Une méthode équivalente pour faire la même sécurité côté client à l’aide de la [**liaison de sécurité de \_ \_ message WS \_ \_ username**](/windows/desktop/api/WebServices/ns-webservices-ws_username_message_security_binding) est illustrée par l’exemple [HttpCalculatorWithUsernameOverSslClientExample](httpcalculatorwithusernameoversslclientexample.md).

Le côté serveur correspondant à ces deux exemples côté client est fourni par l’exemple [HttpCalculatorWithUserNameOverSslServiceExample](httpcalculatorwithusernameoversslserviceexample.md).
