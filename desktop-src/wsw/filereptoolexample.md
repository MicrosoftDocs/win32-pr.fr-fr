...
ms.locfileid: "119026547"
---
# <a name="filereptoolexample"></a>FileRepToolExample

Il s’agit de l’outil en ligne de commande qui pilote le [service FileRep](filerepserviceexample.md). Une description plus détaillée de cet exemple est disponible ici.
...
URL du service client : obligatoire. URL du service client.

URL du service serveur : obligatoire. URL du service serveur.

Fichier source : obligatoire. Nom local complet du fichier source. Le fichier se trouve sur l’ordinateur sur lequel s’exécute le service serveur.

Fichier de destination : obligatoire. Nom local complet du fichier de destination. Le fichier se trouve sur l’ordinateur sur lequel s’exécute le service client.

Encodage : facultatif. Spécifie l’encodage de message pour les messages envoyés entre le client et les services serveur. L’encodage utilisé pour la communication entre l’outil en ligne de commande et le service client ne peut pas être modifié pour des raisons de simplicité. Les paramètres valides sont Binary, Text et MTOM. Si le paramètre n’est pas spécifié, l’encodage par défaut pour le transport est utilisé.

Sync : facultatif. Lorsque cette valeur est définie, la requête se termine de façon synchrone. Dans le cas contraire, la demande se termine de façon asynchrone.

-   [Outil. cpp](#toolcpp)
-   [Common. h](#commonh)
-   [Rubriques connexes](#related-topics)

## <a name="toolcpp"></a>Outil. cpp
