...
FileRep récupère les fichiers à partir d’un serveur et les copie sur un client. Pour ce faire, il utilise trois composants : le service serveur en cours d’exécution sur l’ordinateur avec le fichier source, le service client en cours d’exécution sur l’ordinateur où le fichier de destination sera stocké et un outil en ligne de commande pour contrôler la copie. Les services du client et du serveur exécutent constamment des services Web lorsque l’outil de ligne de commande est démarré par l’utilisateur et se termine après une demande.

Cet exemple illustre l’utilisation du canal et de la couche de sérialisation.

Il s’agit du service. L’outil en ligne de commande est disponible [ici](filereptoolexample.md). Le service dispose d’un client et d’un mode serveur, où le serveur envoie des fichiers et le client reçoit des fichiers.
...
Obligatoire. Indique que le service s’exécute en tant que client.

</dd> <dt>

<span id="Service_Url"></span><span id="service_url"></span><span id="SERVICE_URL"></span>URL du service
</dt> <dd>

Obligatoire. Indique l’URL sur laquelle le service écoute.
...
<span id="Server"></span><span id="server"></span><span id="SERVER"></span>Serveurs
</dt> <dd>

Obligatoire. Indique que le service s’exécute en tant que serveur de fichiers.
...
Détails de l’implémentation. Comme cet exemple est écrit par rapport à la couche de canal, il doit effectuer manuellement certaines tâches que la couche de modèle de service peut effectuer automatiquement. La gestion des canaux est une tâche de ce type. Pour fournir une réponse rapide aux demandes, il existe toujours (jusqu’à une limite) plusieurs canaux prêts à accepter des demandes. Le fait de disposer de plusieurs canaux prêts est plus performant dans le cas de plusieurs demandes que l’utilisation d’un seul canal au moment où la création d’un canal prend du temps. En outre, la réutilisation de l’État améliore également les performances. Saphir fournit des API pour réinitialiser la plupart des États afin d’éviter d’avoir à les libérer et à les recréer. Cet exemple tire parti de cela en réutilisant le canal et l’état de la demande dans la mesure du possible et en créant ou détruisant uniquement l’État lorsque certains seuils sont passés. Vous trouverez le code relatif à la gestion des canaux dans CChannelManager.

La boucle de traitement de message principale se trouve dans CRequest. Cette classe contient l’État indépendant de l’application et les méthodes nécessaires pour une boucle de traitement de messagerie saphir asynchrone. Le code spécifique à l’application se trouve dans CFileRepClient (service client) et CFileRepServer (service serveur). Ces deux classes héritent de CFileRep, qui contient le code générique lié au service.

L’exemple utilise le sérialiseur et effectue une sérialisation personnalisée. La sérialisation personnalisée est utilisée lors du traitement de gros segments de données pour réduire la consommation de mémoire en optimisant manuellement l’allocation de mémoire à des fins spécifiques. En raison du code complexe à bas niveau, la sérialisation manuelle ne doit être effectuée qu’en cas de nécessité absolue.

Modèle d’échange de messages :

-   Le service client obtient un message de demande à partir de l’outil en ligne de commande.
-   Si la demande est asynchrone, renvoyez immédiatement une confirmation.
-   Le service client envoie une demande d’informations de fichier au service serveur. Une demande de découverte est indiquée par une position de segment de-1.
-   Le service serveur retourne les informations du fichier.
-   Le service client demande les segments individuels un par un à partir du serveur. Les segments sont identifiés par leur position dans le fichier.
-   Répétez l’opération jusqu’à ce que le transfert de fichiers soit terminé ou qu’une erreur se soit produite.
-   Si la demande est un message de réussite ou d’échec d’envoi synchrone à l’outil en ligne de commande.

Pour connaître les structures de données individuelles associées à chaque message, consultez Common. h.
