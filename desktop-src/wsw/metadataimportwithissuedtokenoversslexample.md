...
Cet exemple illustre l’importation de métadonnées à partir d’un point de terminaison qui prend en charge l’utilisation d’un jeton émis obtenu à partir du service d’émission de jeton de sécurité (STS) qui utilise [**WsRequestSecurityToken**](/windows/desktop/api/WebServices/nf-webservices-wsrequestsecuritytoken) et présenté au service à l’aide de la liaison de sécurité de [**message de \_ \_ jeton \_ \_ \_ WS XML**](/windows/desktop/api/WebServices/ns-webservices-ws_xml_token_message_security_binding) avec liaison de [**sécurité de \_ \_ transport \_ \_ WS SSL**](/windows/desktop/api/WebServices/ns-webservices-ws_ssl_transport_security_binding). Pour plus d’informations, consultez [Fédération](federation.md) .
