...
ms.locfileid: "118963178"
---
# <a name="httppurchaseorderwithkerberosoversslclientexample"></a>HttpPurchaseOrderWithKerberosOverSslClientExample

Cet exemple montre un client HTTP qui utilise le proxy de service pour communiquer avec un service PurchaseOrder, avec Kerberos sur la sécurité SSL en mode mixte. Dans cette configuration, la connexion de transport est protégée (signée, chiffrée) par SSL, ce qui permet également l’authentification du serveur. L’authentification du client est fournie par un ticket Kerberos APREQ dans un en-tête de WS-Security dans le message.
