...
Cet exemple montre un client HTTP qui envoie des messages de demande-réponse, avec le nom d’utilisateur sur la sécurité SSL en mode mixte. Dans cette configuration, la connexion de transport est protégée (signée, chiffrée) par SSL, ce qui permet également l’authentification du serveur. L’authentification du client est fournie par une paire nom d’utilisateur/mot de passe dans un en-tête de WS-Security dans le message.
