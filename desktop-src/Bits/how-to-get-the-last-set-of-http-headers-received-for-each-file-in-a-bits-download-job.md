...
### <a name="step-2-initialize-com-and-instantiate-a-bits-background-copy-manager-object-interface"></a>Étape 2 : initialiser COM et instancier une interface d’objet du gestionnaire de copie en arrière-plan BITS

Avant d’instancier l’interface [**IBackgroundCopyManager**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopymanager) (utilisée pour créer un travail bits), vous devez initialiser com et définir le modèle de thread com souhaité.
...
### <a name="step-3-create-the-bits-job"></a>Étape 3 : créer le travail BITS
...
### <a name="step-4-add-the-files-to-the-bits-job"></a>Étape 4 : ajouter les fichiers à la tâche BITS

L’exemple suivant télécharge les documents disponibles publiquement à partir du centre de téléchargement Microsoft. Vous devrez modifier ces valeurs pour votre environnement spécifique.
...
### <a name="step-5-start-the-bits-job"></a>Étape 5 : démarrer le travail BITS

Après avoir configuré le travail BITS, appelez la fonction [**Resume**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-resume) de l’interface [**méthode ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) pour démarrer ou continuer le téléchargement.
...
### <a name="step-6-monitor-and-display-the-bits-jobs-progress"></a>Étape 6 : surveiller et afficher la progression de la tâche BITS

La `MonitorJobProgress` fonction d’assistance prend un objet [**méthode ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) comme seul paramètre et interroge le travail sur un état toutes les 500 millisecondes. Cette fonction ne retourne aucune réponse tant que le travail n’est pas terminé ou n’a pas été annulé.
...
### <a name="step-7-display-the-downloaded-file-headers"></a>Étape 7 : afficher les en-têtes de fichier téléchargés

La `DisplayFileHeaders` fonction d’assistance énumère les travaux définis pour un objet [**méthode ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) .
