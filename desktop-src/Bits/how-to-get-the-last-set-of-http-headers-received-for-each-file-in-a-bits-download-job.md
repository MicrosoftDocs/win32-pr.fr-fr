---
title: Comment obtenir le dernier ensemble d’en-têtes HTTP reçus pour chaque fichier dans un travail de téléchargement BITS
description: Cet exemple montre comment utiliser la méthode GetProperty de l’interface IBackgroundCopyJob5 pour obtenir le dernier jeu d’en-têtes HTTP reçu pour chaque fichier dans un travail de téléchargement Service de transfert intelligent en arrière-plan (BITS).
...
---
# <a name="how-to-get-the-last-set-of-http-headers-received-for-each-file-in-a-bits-download-job"></a><span data-ttu-id="4b78e-104">Comment obtenir le dernier ensemble d’en-têtes HTTP reçus pour chaque fichier dans un travail de téléchargement BITS</span><span class="sxs-lookup"><span data-stu-id="4b78e-104">How to get the last set of HTTP headers received for each file in a BITS download job</span></span>

<span data-ttu-id="4b78e-105">Cet exemple montre comment utiliser la méthode [**GetProperty**](/windows/desktop/api/Bits5_0/nf-bits5_0-ibackgroundcopyjob5-getproperty) de l’interface [**IBackgroundCopyJob5**](/windows/desktop/api/Bits5_0/nn-bits5_0-ibackgroundcopyjob5) pour obtenir le dernier jeu d’en-têtes http reçu pour chaque fichier dans un travail de téléchargement service de transfert intelligent en arrière-plan (bits).</span><span class="sxs-lookup"><span data-stu-id="4b78e-105">This sample shows how to use the new [**IBackgroundCopyJob5**](/windows/desktop/api/Bits5_0/nn-bits5_0-ibackgroundcopyjob5) interface's [**GetProperty**](/windows/desktop/api/Bits5_0/nf-bits5_0-ibackgroundcopyjob5-getproperty) method to obtain the last set HTTP headers received for each file in a Background Intelligent Transfer Service (BITS) download job.</span></span> <span data-ttu-id="4b78e-106">Les informations contenues dans l’en-tête HTTP peuvent être utilisées, par exemple, pour déterminer le type de fichier ou le moment où il a été modifié pour la dernière fois sur le serveur.</span><span class="sxs-lookup"><span data-stu-id="4b78e-106">The information in the HTTP header could be used, for example, to determine the file type or when it last changed on the server.</span></span> <span data-ttu-id="4b78e-107">Avant Windows 8 et Windows Server 2012, le service BITS n’offrait pas de moyen permettant à l’application de récupérer et d’inspecter les en-têtes de réponse HTTP d’un téléchargement terminé.</span><span class="sxs-lookup"><span data-stu-id="4b78e-107">Prior to Windows 8 and Windows Server 2012, BITS did not provide a means by which the application could retrieve and inspect the HTTP response headers of a completed download.</span></span> <span data-ttu-id="4b78e-108">Cet exemple montre comment utiliser l’API BITS pour créer un travail BITS avec plusieurs URL à télécharger, répertorier les URL d’un travail et récupérer les en-têtes de réponse HTTP pour chaque URL.</span><span class="sxs-lookup"><span data-stu-id="4b78e-108">This sample shows how to use the BITS API to create a BITS job with multiple URLs to download, list the URLs in a job, and retrieve the HTTP response headers for each URL.</span></span>

## <a name="what-you-need-to-know"></a><span data-ttu-id="4b78e-109">Ce que vous devez savoir</span><span class="sxs-lookup"><span data-stu-id="4b78e-109">What you need to know</span></span>

### <a name="technologies"></a><span data-ttu-id="4b78e-110">Technologies</span><span class="sxs-lookup"><span data-stu-id="4b78e-110">Technologies</span></span>

### <a name="prerequisites"></a><span data-ttu-id="4b78e-111">Prérequis</span><span class="sxs-lookup"><span data-stu-id="4b78e-111">Prerequisites</span></span>

-   <span data-ttu-id="4b78e-112">Microsoft Visual Studio</span><span class="sxs-lookup"><span data-stu-id="4b78e-112">Microsoft Visual Studio</span></span>

## <a name="instructions"></a><span data-ttu-id="4b78e-113">Instructions</span><span class="sxs-lookup"><span data-stu-id="4b78e-113">Instructions</span></span>

### <a name="step-1-include-the-required-bits-header-files"></a><span data-ttu-id="4b78e-114">Étape 1 : inclure les fichiers d’en-tête BITS requis</span><span class="sxs-lookup"><span data-stu-id="4b78e-114">Step 1: Include the required BITS header files</span></span>

<span data-ttu-id="4b78e-115">Insérez les directives d’en-tête suivantes en haut du fichier source.</span><span class="sxs-lookup"><span data-stu-id="4b78e-115">Insert the following header directives at the top of the source file.</span></span>
...
### <a name="step-2-initialize-com-and-instantiate-a-bits-background-copy-manager-object-interface"></a><span data-ttu-id="4b78e-116">Étape 2 : initialiser COM et instancier une interface d’objet du gestionnaire de copie en arrière-plan BITS</span><span class="sxs-lookup"><span data-stu-id="4b78e-116">Step 2: Initialize COM and instantiate a BITS Background Copy Manager object interface</span></span>

<span data-ttu-id="4b78e-117">Avant d’instancier l’interface [**IBackgroundCopyManager**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopymanager) (utilisée pour créer un travail bits), vous devez initialiser com et définir le modèle de thread com souhaité.</span><span class="sxs-lookup"><span data-stu-id="4b78e-117">Before instantiating the [**IBackgroundCopyManager**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopymanager) interface (used to create a BITS job), you must initialize COM and set the desired COM threading model.</span></span>
...
### <a name="step-3-create-the-bits-job"></a><span data-ttu-id="4b78e-118">Étape 3 : créer le travail BITS</span><span class="sxs-lookup"><span data-stu-id="4b78e-118">Step 3: Create the BITS job</span></span>

<span data-ttu-id="4b78e-119">Seul l’utilisateur qui crée le travail ou un utilisateur disposant de privilèges d’administrateur peut ajouter des fichiers au travail et modifier les propriétés du travail.</span><span class="sxs-lookup"><span data-stu-id="4b78e-119">Only the user who creates the job or a user with administrator privileges can add files to the job and change the job's properties.</span></span>
...
### <a name="step-4-add-the-files-to-the-bits-job"></a><span data-ttu-id="4b78e-120">Étape 4 : ajouter les fichiers à la tâche BITS</span><span class="sxs-lookup"><span data-stu-id="4b78e-120">Step 4: Add the files to the BITS job</span></span>

<span data-ttu-id="4b78e-121">L’exemple suivant télécharge les documents disponibles publiquement à partir du centre de téléchargement Microsoft.</span><span class="sxs-lookup"><span data-stu-id="4b78e-121">The following example downloads publicly available documents from the Microsoft Download Center.</span></span> <span data-ttu-id="4b78e-122">Vous devrez modifier ces valeurs pour votre environnement spécifique.</span><span class="sxs-lookup"><span data-stu-id="4b78e-122">You'll need to change these values for your specific environment.</span></span>
...
### <a name="step-5-start-the-bits-job"></a><span data-ttu-id="4b78e-123">Étape 5 : démarrer le travail BITS</span><span class="sxs-lookup"><span data-stu-id="4b78e-123">Step 5: Start the BITS job</span></span>

<span data-ttu-id="4b78e-124">Après avoir configuré le travail BITS, appelez la fonction [**Resume**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-resume) de l’interface [**méthode ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) pour démarrer ou continuer le téléchargement.</span><span class="sxs-lookup"><span data-stu-id="4b78e-124">After setting up the BITS job, call the [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) interface's [**Resume**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-resume) function to start or continue the download.</span></span>
...
### <a name="step-6-monitor-and-display-the-bits-jobs-progress"></a><span data-ttu-id="4b78e-125">Étape 6 : surveiller et afficher la progression de la tâche BITS</span><span class="sxs-lookup"><span data-stu-id="4b78e-125">Step 6: Monitor and display the BITS job's progress</span></span>

<span data-ttu-id="4b78e-126">La `MonitorJobProgress` fonction d’assistance prend un objet [**méthode ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) comme seul paramètre et interroge le travail sur un état toutes les 500 millisecondes.</span><span class="sxs-lookup"><span data-stu-id="4b78e-126">The `MonitorJobProgress` helper function takes an [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) object as its only parameter and polls the job for a status every 500 milliseconds.</span></span> <span data-ttu-id="4b78e-127">Cette fonction ne retourne aucune réponse tant que le travail n’est pas terminé ou n’a pas été annulé.</span><span class="sxs-lookup"><span data-stu-id="4b78e-127">This function does not return until the job has completed or been canceled.</span></span>
...
### <a name="step-7-display-the-downloaded-file-headers"></a><span data-ttu-id="4b78e-128">Étape 7 : afficher les en-têtes de fichier téléchargés</span><span class="sxs-lookup"><span data-stu-id="4b78e-128">Step 7: Display the downloaded file headers</span></span>

<span data-ttu-id="4b78e-129">La `DisplayFileHeaders` fonction d’assistance énumère les travaux définis pour un objet [**méthode ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) .</span><span class="sxs-lookup"><span data-stu-id="4b78e-129">The `DisplayFileHeaders` helper function enumerates the jobs defined for an [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) object.</span></span>
...
## <a name="example"></a><span data-ttu-id="4b78e-130">Exemple</span><span class="sxs-lookup"><span data-stu-id="4b78e-130">Example</span></span>

<span data-ttu-id="4b78e-131">L’exemple de code suivant est une application console entièrement fonctionnelle qui montre comment utiliser l’API BITS pour créer un travail BITS avec plusieurs URL à télécharger, répertorier les URL d’un travail et récupérer les en-têtes de réponse HTTP pour chaque URL.</span><span class="sxs-lookup"><span data-stu-id="4b78e-131">The following code example is a fully working console application that shows how to use the BITS API to create a BITS job with multiple URLs to download, list the URLs in a job, and retrieve the HTTP response headers for each URL.</span></span>
