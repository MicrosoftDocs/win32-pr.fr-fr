...
ms.locfileid: "120004829"
---
# <a name="setting-and-retrieving-the-properties-of-a-job"></a>Définition et récupération des propriétés d’un travail

Le propriétaire du travail ou un utilisateur disposant de privilèges d’administrateur peut définir et récupérer les propriétés du travail à tout moment. Pour obtenir la liste complète des propriétés que vous pouvez définir et récupérer, consultez les interfaces [**méthode ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob), [**IBackgroundCopyJob2**](/windows/desktop/api/Bits1_5/nn-bits1_5-ibackgroundcopyjob2), [**IBackgroundCopyJob3**](/windows/desktop/api/Bits2_0/nn-bits2_0-ibackgroundcopyjob3)et [**IBackgroundCopyJob4**](/windows/desktop/api/Bits3_0/nn-bits3_0-ibackgroundcopyjob4) .

Les fichiers contiennent également des propriétés. Pour plus d’informations sur la récupération d’un fichier et de ses propriétés à partir d’un travail, consultez [énumération de fichiers dans un travail](enumerating-files-in-a-job.md).

Pour transférer des fichiers, vous n’avez pas besoin de modifier les valeurs par défaut des propriétés du travail. BITS utilise les valeurs par défaut appropriées pour l’application typique.

## <a name="setting-the-properties-of-a-job"></a>Définition des propriétés d’un travail

L’exemple suivant montre comment définir les propriétés que votre application est le plus susceptible de modifier : [priorité](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-setpriority), [notifier l’interface](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-setnotifyinterface), [signaler les indicateurs](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-setnotifyflags)et le nom du [fichier de réponse](/windows/desktop/api/Bits1_5/nf-bits1_5-ibackgroundcopyjob2-setreplyfilename). L’exemple suppose que le pointeur d’interface [**méthode ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) , pJob, est valide.
...
Par défaut, BITS télécharge le contenu à partir du serveur d’origine. Pour télécharger du contenu à partir d’un homologue, l’ordinateur et le travail doivent activer la mise en cache d’homologue. Pour activer la mise en cache d’homologue sur l’ordinateur, définissez le paramètre de stratégie de groupe EnablePeerCaching. Vous pouvez également appeler la méthode [**IBitsPeerCacheAdministration :: SetConfigurationFlags**](/windows/desktop/api/Bits3_0/nf-bits3_0-ibitspeercacheadministration-setconfigurationflags) pour activer la mise en cache d’homologue sur l’ordinateur. Toutefois, le paramètre de préférence est remplacé par la stratégie, si elle est définie. Pour activer la mise en cache d’homologue pour le travail, vous devez appeler la méthode [**IBackgroundCopyJob4 :: SetPeerCachingFlags**](/windows/desktop/api/Bits3_0/nf-bits3_0-ibackgroundcopyjob4-setpeercachingflags) .

Pour spécifier des en-têtes personnalisés, un certificat client pour l’authentification du client et des options HTTP comme la stratégie de redirection, la vérification de la liste de révocation des certificats et la spécification des erreurs de certificat à ignorer, utilisez l’interface [**IBackgroundCopyJobHttpOptions**](/windows/desktop/api/Bits2_5/nn-bits2_5-ibackgroundcopyjobhttpoptions) . Pour obtenir l’interface **IBackgroundCopyJobHttpOptions** , interrogez l’une des interfaces [**méthode ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) .

## <a name="retrieving-the-properties-of-a-job"></a>Récupération des propriétés d’un travail

L’exemple suivant montre comment récupérer le [nom complet](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-getdisplayname), le [propriétaire](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-getowner), la [progression](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-getprogress)et les valeurs de propriété d' [État](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-getstate) d’un travail. L’exemple suppose que le pointeur d’interface [**méthode ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) , pJob, est valide.
