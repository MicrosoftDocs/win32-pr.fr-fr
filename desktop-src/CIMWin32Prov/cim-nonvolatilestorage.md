...
ms.locfileid: "119820959"
---
# <a name="cim_nonvolatilestorage-class"></a>\_Classe CIM NonVolatileStorage

La classe **CIM \_ NonVolatileStorage** représente les fonctionnalités et la gestion du stockage non volatile. La mémoire non volatile comprend en mode natif le stockage flash et ROM. En outre, la mémoire non volatile peut reposer sur un stockage volatile, si la mémoire volatile est sauvegardée par une batterie. Par exemple, ce scénario est décrit par une instance de la relation [**CIM \_ AssociatedBattery**](cim-associatedbattery.md) qui fait référence au stockage non volatile comme étant le **dépendant** et la batterie en tant qu' **antécédent**, et une instance de la relation [**CIM \_**](cim-basedon.md) qui fait référence au stockage non volatile en tant que stockage **dépendant** et le stockage volatil comme **antécédent**.
...
La classe **CIM \_ NonVolatileStorage** possède les types de membres suivants :
...
La classe **CIM \_ NonVolatileStorage** possède ces méthodes.



| Méthode                                                                        | Description                                                                                                                              |
|:------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Initialisation**](reset-method-in-class-cim-nonvolatilestorage.md)                 | Demande la réinitialisation de l’unité logique. Non implémenté par WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-nonvolatilestorage.md) | Définit l’état d’alimentation souhaité pour un périphérique logique et lorsqu’un appareil doit être placé dans cet État. Non implémenté par WMI.<br/> |
...
La classe **CIM \_ NonVolatileStorage** possède les propriétés suivantes.
...
Propriétés de lecture/écriture du média.
...
Énumération qui indique l’opération d’accès à la mémoire qui a provoqué la dernière erreur. Le type d’erreur est décrit par la propriété **errorInfo** . Si la propriété **errorInfo** est égale à 3 (« OK »), cette propriété n’a aucune signification.
...
Classement des données stockées dans la propriété **ErrorData** . Si **ErrorTransferSize** est égal à 0, cette propriété n’a aucune signification.
...
Type d’erreur qui s’est produite le plus récemment. Les valeurs 12 à 14 ne sont pas définies dans le schéma CIM, car DMI mélange la sémantique du type d’erreur et indique si elle a été corrigée. La propriété **CorrectableError** indique si une erreur peut être corrigée. Cette propriété est héritée de la [**\_ mémoire CIM**](cim-memory.md).
...
Spécifie si les algorithmes de parité, les algorithmes CRC, l’ECC ou d’autres mécanismes sont utilisés. Vous pouvez également fournir des détails sur l’algorithme.
...
Taille du transfert de données, en bits, à l’origine de la dernière erreur. La valeur 0 (zéro) indique qu’il n’y A pas d’erreur. Si la propriété **errorInfo** est égale à 3 (« OK »), cette propriété doit avoir la valeur 0 (zéro).
...
Si la **valeur est true**, le stockage non volatile peut être écrit.
...
Chaîne de forme libre qui fournit des informations si la propriété **ErrorType** a la valeur 1 (« other »). Si la valeur n’est pas définie sur 1, cette chaîne n’a aucune signification.
...
Windows Identificateur d’appareil Plug-and-Play de l’unité logique. Cette propriété est héritée de [**CIM \_ LogicalDevice**](cim-logicaldevice.md).
...
La classe **CIM \_ NonVolatileStorage** est dérivée de la [**\_ mémoire CIM**](cim-memory.md).
