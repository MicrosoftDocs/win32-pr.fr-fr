...
La classe **CIM \_ RemoteFileSystem** représente un système de fichiers distant accessible par le biais d’un service lié au réseau. Dans ce cas, le magasin de fichiers est hébergé par un ordinateur qui joue le rôle de serveur de fichiers. Par exemple, le magasin de fichiers pour un système de fichiers NFS n’est généralement pas sur un support contrôlé localement d’un ordinateur, et n’est pas directement accessible par le biais d’un pilote de périphérique. Les sous-classes **de \_ RemoteFileSystem CIM** contiennent des informations de configuration côté client relatives à l’accès du système de fichiers.
...
La classe **CIM \_ RemoteFileSystem** possède les types de membres suivants :

-   [Propriétés](#properties)

### <a name="properties"></a>Propriétés

La classe **CIM \_ RemoteFileSystem** possède les propriétés suivantes.
...
Quantité d’espace libre pour le système de fichiers, en octets. S’il est inconnu, entrez 0.
...
Taille, en octets, des blocs qui forment l’extension de stockage. S’il est inconnu, ou si un concept de bloc n’est pas valide (par exemple, pour les extensions d’agrégat, la mémoire ou les disques logiques), entrez 1 (un).
...
Jeux de caractères ou encodage pris en charge par le système de fichiers. Cette propriété est héritée [**du \_ système de fichiers CIM**](cim-filesystem.md).
...
Taille totale du système de fichiers, en octets. S’il est inconnu, entrez 0.
...
Longueur maximale d’un nom de fichier dans le système de fichiers. La valeur 0 indique que la longueur du nom de fichier est illimitée.
