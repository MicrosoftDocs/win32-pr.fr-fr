...
ms.locfileid: "119020837"
---
# <a name="cim_tapedrive-class-cimwin32-wmi-providers"></a>CIM_TapeDrive, classe (fournisseurs WMI CIMWin32)

La **classe \_ CIM CIM** représente un lecteur de bande sur le système. Les lecteurs de bande sont principalement distingués dans le fait qu’ils sont accessibles uniquement de façon séquentielle.
...
La **classe \_ CIM CIM** possède ces méthodes.



| Méthode                                                               | Description                                                                                                                              |
|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Initialisation**](reset-method-in-class-cim-tapedrive.md)                 | Demande la réinitialisation de l’unité logique. Non implémenté par WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-tapedrive.md) | Définit l’état d’alimentation souhaité pour un périphérique logique et lorsqu’un appareil doit être placé dans cet État. Non implémenté par WMI.<br/> |
...
Fonctionnalités de l’appareil d’accès aux médias. Cette propriété est héritée de la [**\_ MediaAccessDevice CIM**](cim-mediaaccessdevice.md).
...
Chaînes de forme libre qui fournissent des explications détaillées sur les fonctionnalités de l’appareil Access indiquées dans le tableau des **fonctionnalités** . Notez que chaque entrée de ce tableau est liée à l’entrée dans le tableau de **fonctionnalités** qui se trouve dans le même index.
...
La **classe \_ CIM CIM** est dérivée de la [**\_ MediaAccessDevice CIM**](cim-mediaaccessdevice.md).

WMI n’implémente pas cette classe. Pour les classes WMI dérivées de la page **CIM CIM \_**, consultez [classes Win32](win32-provider.md).
