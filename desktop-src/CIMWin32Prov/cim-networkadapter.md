...
ms.locfileid: "119021027"
---
# <a name="cim_networkadapter-class"></a>\_Classe NETWORKADAPTER CIM

La classe de la carte CIM est une classe abstraite qui définit les concepts généraux de matériel de mise en réseau (par exemple, une adresse permanente ou une vitesse de fonctionnement). **\_** Les informations sont transmises à l’aide de l’Association [**CIM \_ DeviceSAPImplementation**](cim-devicesapimplementation.md) .

Les cartes réseau et les points de terminaison associés représentent le potentiel pour la connectivité entre pairs. Le potentiel de connectivité est très différent de celui des relations maître ou subordonné, ou contrôlé par le contrôleur [**CIM \_**](cim-controller.md). Toutefois, il peut arriver qu’un seul appareil fasse office de carte réseau et de contrôleur, par exemple lorsqu’une carte Fibre Channel fonctionne comme contrôleur SCSI d’un système informatique. Dans ce cas, les aspects de l’appareil sont orientés sur le réseau et d’autres sont orientés par le contrôleur. Les classes de contrôleur et d’adaptateur doivent être instanciées et une relation d’identité d’appareil doit également être créée pour lier les différents aspects de l’appareil.
...
La classe de la carte **CIM \_** possède les types de membres suivants :
...
La classe de la carte **CIM \_** possède les méthodes suivantes.



| Méthode                                                                    | Description                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Initialisation**](reset-method-in-class-cim-networkadapter.md)                 | Demande la réinitialisation de l’unité logique. Non implémenté par WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-networkadapter.md) | Définit l’état d’alimentation souhaité pour un périphérique logique et lorsqu’un appareil doit être placé dans cet État. Non implémenté par WMI.<br/> |
...
La classe de la carte **CIM \_** possède les propriétés suivantes.
...
Si la **valeur est true**, la carte réseau peut déterminer automatiquement la vitesse ou d’autres caractéristiques de communication du support réseau attaché.
...
Vitesse maximale, en bits par seconde (bits/s), pour la carte réseau.
...
Liste des adresses réseau d’un adaptateur.
...
Adresse réseau codée en dur dans un adaptateur. L’adresse codée en dur peut être modifiée à l’aide d’une mise à niveau du microprogramme ou d’une configuration logicielle. En cas de modification, le champ doit être mis à jour lorsque la modification est apportée. Cette propriété doit être laissée vide si aucune adresse codée en dur n’existe pour la carte réseau.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| RFC1213-MIB. ifSpeed "," MIF. \|Carte réseau DMTF 802 Port \| 001,5 "), [**unités**](/windows/desktop/WmiSdk/standard-qualifiers) (" bits par seconde ")
</dt> </dl>

Estimation de la bande passante actuelle, en BPS. Pour les points de terminaison qui varient en bande passante ou pour ceux pour lesquels aucune estimation précise ne peut être effectuée, cette propriété doit contenir la bande passante nominale.
...
La **classe \_ NetworkAdapter CIM** est dérivée [**de \_ CIM LogicalDevice**](cim-logicaldevice.md).

WMI n’implémente pas cette classe. Pour les classes dérivées de la carte **CIM \_**, consultez [classes Win32](win32-provider.md).
