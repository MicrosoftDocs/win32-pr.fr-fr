...
ms.locfileid: "119816729"
---
# <a name="cim_volatilestorage-class"></a>\_Classe CIM VolatileStorage

La classe **CIM \_ VolatileStorage** représente les fonctionnalités et la gestion du stockage volatile.
...
La classe **CIM \_ VolatileStorage** possède les types de membres suivants :
...
La classe **CIM \_ VolatileStorage** possède ces méthodes.



| Méthode                                                                     | Description                                                                                                                              |
|:---------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Initialisation**](reset-method-in-class-cim-volatilestorage.md)                 | Demande la réinitialisation de l’unité logique. Non implémenté par WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-volatilestorage.md) | Définit l’état d’alimentation souhaité pour un périphérique logique et lorsqu’un appareil doit être placé dans cet État. Non implémenté par WMI.<br/> |
...
La classe **CIM \_ VolatileStorage** possède les propriétés suivantes.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Informations sur la mémoire des ressources système DMTF \| 001,5»)
</dt> </dl>

Si la **valeur est true**, cette mémoire peut être mise en cache.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Informations sur la mémoire des ressources système DMTF \| 001,6»)
</dt> </dl>

Type de cache qui est compatible avec la mémoire. Si la propriété **cacheable** est définie sur **false**, cette propriété n’a pas de signification et doit être définie sur 4 (« non applicable »).
...
Données capturées au cours du dernier accès mémoire erroné. Les données occupent les *n* premiers octets du tableau nécessaire pour contenir le nombre de bits spécifié par la propriété **ErrorTransferSize** . Si **ErrorTransferSize** est égal à 0, cette propriété n’a aucune signification.
...
Classement des données stockées dans le tableau **ErrorData** . Si **ErrorTransferSize** est égal à 0, cette propriété n’a aucune signification.
...
Type d’erreur qui s’est produite le plus récemment. Les valeurs 12-14 ne sont pas définies dans le schéma CIM, car la sémantique du type d’erreur et la nécessité ou non de la corriger sont mélangées dans DMI. La propriété **CorrectableError** indique si elle a été corrigée.
...
Chaîne de forme libre qui décrit le type de détection d’erreurs et de correction pris en charge par l’extension de stockage.
...
Taille du transfert de données en bits qui a provoqué la dernière erreur. 0 indique qu’il n’y a pas d’erreur. Si la propriété **errorInfo** est égale à 3, « OK », cette propriété doit être définie sur 0.
...
Chaîne de forme libre qui fournit des informations supplémentaires si la propriété **ErrorType** a la valeur 1 (« other »). Si une valeur autre que 1 (un), cette chaîne n’a aucune signification.
...
Adresse de début, référencée par une application ou un système d’exploitation et mappée par un contrôleur de mémoire, pour l’objet mémoire. L’adresse de départ est spécifiée en kilo-octets.
...
La classe **CIM \_ VolatileStorage** est dérivée de la [**\_ mémoire CIM**](cim-memory.md).
