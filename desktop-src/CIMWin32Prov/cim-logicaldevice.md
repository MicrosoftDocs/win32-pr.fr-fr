...
La classe **CIM \_ LogicalDevice** représente une entité matérielle qui peut ou ne peut pas être réalisée sur du matériel physique.
...
La méthode **SetPowerState** est prise en charge. Cette méthode se trouve sur la classe parente du **\_ LogicalDevice CIM** et peut être implémentée. Pour plus d’informations, consultez [conception de Classes format MOF (MOF)](/windows/desktop/WmiSdk/designing-managed-object-format--mof--classes).
...
La **classe \_ LogicalDevice CIM** est dérivée de la [**\_ LogicalElement CIM**](cim-logicalelement.md).

WMI n’implémente pas cette classe. Pour les classes dérivées de **CIM \_ LogicalDevice**, consultez [classes Win32](win32-provider.md).
