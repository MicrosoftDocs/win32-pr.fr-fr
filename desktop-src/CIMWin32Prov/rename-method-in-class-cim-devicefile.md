...
La méthode **Rename** renomme le fichier d’appareil (ou le répertoire) spécifié dans le chemin d’accès de l’objet. Le changement de nom n’est pas pris en charge si la destination se trouve sur un autre lecteur ou si le remplacement d’un fichier logique existant est nécessaire. Cette méthode est héritée de la [**\_ LogicalFile CIM**](cim-logicalfile.md).
