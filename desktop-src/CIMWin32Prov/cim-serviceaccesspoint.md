...
La classe **CIM \_ ServiceAccessPoint** représente la possibilité d’utiliser ou d’appeler un service. Les points d’accès représentent les services qui peuvent être utilisés par d’autres entités.
...
WMI n’implémente pas cette classe. Pour les classes WMI dérivées de **CIM \_ ServiceAccessPoint**, consultez [classes Win32](win32-provider.md).
