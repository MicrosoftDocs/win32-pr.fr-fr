...
La **classe \_ NFS CIM** représente un système de fichiers distant monté, à l’aide du protocole NFS (Network File System), à partir d’un système informatique. Les propriétés de l’objet NFS correspondent aux aspects opérationnels du montage et représentent la configuration côté client pour l’accès NFS. Le type de système de fichiers doit être défini pour indiquer le type de système de fichiers tel qu’il apparaît au client.
...
La **classe \_ NFS CIM** possède les types de membres suivants :

-   [Propriétés](#properties)

### <a name="properties"></a>Propriétés

La **classe \_ NFS CIM** possède ces propriétés.
...
Si la **valeur est true**, la mise en cache des attributs de contrôle est activée. Si la **valeur est false**, le cache des attributs de contrôle est désactivé.
...
Nombre maximal de secondes pendant lesquelles les attributs mis en cache sont conservés après la mise à jour de l’annuaire.
...
Nombre minimal de secondes pendant lesquelles les attributs mis en cache sont conservés après la mise à jour de l’annuaire.
...
Nombre maximal de secondes pendant lesquelles les attributs mis en cache sont conservés après la modification du fichier.
...
Nombre minimal de secondes pendant lesquelles les attributs mis en cache sont conservés après la modification du fichier.
...
Quantité totale d’espace libre, en octets, pour le système de fichiers. S’il est inconnu, entrez 0.
...
Les systèmes de fichiers peuvent lire ou écrire des données dans des blocs qui sont définis indépendamment des extensions de stockage sous-jacentes. Cette propriété capture la taille de bloc du système de fichiers pour le stockage et la récupération des données.
...
Jeux de caractères ou encodage pris en charge par le système de fichiers.
...
Taille totale du système de fichiers, en octets. S’il est inconnu, entrez 0 (zéro).
...
Si la **valeur est true**, les nouvelles tentatives sont effectuées au premier plan. Si la **valeur est false** et que la première tentative de montage échoue, les nouvelles tentatives sont effectuées en arrière-plan.
...
Si la **valeur est true**, une fois le système de fichiers monté, les demandes NFS sont retentées jusqu’à ce que le système d’hébergement réponde. Si la **valeur est false**, une erreur est retournée si le système d’hébergement ne répond pas après le montage du système de fichiers.
...
Si la **valeur est true**, les interruptions sont autorisées pour les montages matériels. Si la **valeur est false**, les interruptions sont ignorées pour les montages matériels.
...
Longueur maximale d’un nom de fichier dans le système de fichiers. La valeur 0 (zéro) indique qu’il n’existe aucune limite pour la longueur du nom de fichier.
...
Nombre maximal de nouvelles tentatives d’échec de montage autorisées.
...
Nombre maximal de retransmissions NFS autorisées.
...
Qualificateurs : [**unités**](/windows/desktop/WmiSdk/standard-qualifiers) (« dixièmes de secondes »)
</dt> </dl>

Délai d’expiration NFS, en dixièmes de seconde.
...
Numéro de port UDP du système de l’ordinateur distant.
