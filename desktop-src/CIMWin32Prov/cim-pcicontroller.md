...
ms.locfileid: "118678959"
---
# <a name="cim_pcicontroller-class"></a>\_Classe CIM PCIController

La classe **CIM \_ PCIController** représente les propriétés et la gestion d’un contrôleur PCI. Les propriétés de cette classe et de ses sous-classes sont définies dans les différentes spécifications PCI publiées par le SIG PCI.
...
La classe **CIM \_ PCIController** possède les types de membres suivants :
...
La classe **CIM \_ PCIController** possède ces méthodes.



| Méthode                                                                   | Description                                                                                                                              |
|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Initialisation**](reset-method-in-class-cim-pcicontroller.md)                 | Demande la réinitialisation de l’unité logique. Non implémenté par WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-pcicontroller.md) | Définit l’état d’alimentation souhaité pour un périphérique logique et lorsqu’un appareil doit être placé dans cet État. Non implémenté par WMI.<br/> |
...
La classe **CIM \_ PCIController** possède les propriétés suivantes.
...
Le contrôleur PCI n’est pas disponible.
...
La méthode [**SetPowerState**](setpowerstate-method-in-class-cim-pcicontroller.md) est prise en charge.
...
La méthode [**SetPowerState**](setpowerstate-method-in-class-cim-pcicontroller.md) peut être appelée avec le paramètre *PowerState* défini sur 5 (« Power cycle »).
...
La méthode [**SetPowerState**](setpowerstate-method-in-class-cim-pcicontroller.md) peut être appelée avec le paramètre *PowerState* défini sur 5 (« Power cycle ») et le paramètre *Time* défini sur une date et une heure spécifiques, ou un intervalle, pour la mise sous tension.
...
La classe **CIM \_ PCIController** est dérivée [**du \_ contrôleur CIM**](cim-controller.md).
