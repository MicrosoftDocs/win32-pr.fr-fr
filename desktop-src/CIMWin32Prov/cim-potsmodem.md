...
ms.locfileid: "118677541"
---
# <a name="cim_potsmodem-class"></a>\_Classe CIM POTSModem

La classe **CIM \_ POTSModem** représente un appareil qui convertit les données binaires en modulations d’onde pour une transmission basée sur le son en se connectant au réseau pots (Plain Old Telephone System).
...
La classe **CIM \_ POTSModem** possède les types de membres suivants :
...
La classe **CIM \_ POTSModem** possède ces méthodes.



| Méthode                                                               | Description                                                                                                                              |
|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Initialisation**](reset-method-in-class-cim-potsmodem.md)                 | Demande la réinitialisation de l’unité logique. Non implémenté par WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-potsmodem.md) | Définit l’état d’alimentation souhaité pour un périphérique logique et lorsqu’un appareil doit être placé dans cet État. Non implémenté par WMI.<br/> |
...
La classe **CIM \_ POTSModem** possède les propriétés suivantes.
...
La réponse automatique actuelle ou le paramètre de rappel pour le modem.
...
Pays ou régions dans lesquels le modem peut fonctionner.
...
Pays ou région pour lequel le modem est actuellement programmé. Lorsque plusieurs pays ou régions sont pris en charge, cette propriété définit celle qui est actuellement sélectionnée pour être utilisée.
...
Mots de passe actuellement définis pour le modem. Ce tableau peut être laissé vide pour des raisons de sécurité.
...
Nombre maximal de mots de passe définissables dans le modem lui-même. Si cette fonctionnalité n’est pas prise en charge, entrez 0 (zéro).
...
Niveau de volume des tonalités audibles du modem. Par exemple, un volume élevé, moyen ou faible peut être signalé.
...
La classe **CIM \_ POTSModem** est dérivée de [**CIM \_ LogicalDevice**](cim-logicaldevice.md).

WMI n’implémente pas cette classe. Pour les classes WMI dérivées de **CIM \_ POTSModem**, consultez [classes Win32](win32-provider.md).
