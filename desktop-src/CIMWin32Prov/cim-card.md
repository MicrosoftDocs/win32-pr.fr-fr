...
La classe de la **\_ carte CIM** possède les types de membres suivants :
...
La classe de **\_ carte CIM** possède ces méthodes.



| Méthode                                                        | Description                                                                                                                                    |
|:--------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------|
| [**IsCompatible**](iscompatible-method-in-class-cim-card.md) | Vérifie si l’élément physique référencé peut être contenu ou inséré dans le package physique. Non implémenté par WMI.<br/> |
...
La classe de la **\_ carte CIM** possède ces propriétés.
...
Si la **valeur est true**, cette carte est une carte mère ou, plus génériquement, une carte de carte dans un châssis.
...
Qualificateurs : [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("**\_ carte CIM**.**SpecialRequirements**")
</dt> </dl>

Chaîne de forme libre qui décrit la façon dont la carte est physiquement unique à partir d’autres cartes. Cette propriété n’a de sens que lorsque la propriété booléenne correspondante, **SpecialRequirements**, a la valeur **true**.
...
Si la **valeur est true**, pour fonctionner correctement, au moins une carte fille ou auxiliaire est requise.
...
Chaîne de forme libre qui décrit le positionnement de l’emplacement, l’utilisation typique, les restrictions, les espaces d’emplacement individuels ou d’autres informations pertinentes pour les emplacements sur une carte.
...
Qualificateurs : [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("**\_ carte CIM**.**RequirementsDescription**")
</dt> </dl>

Si la **valeur est true**, la carte est physiquement unique des autres cartes du même type et, par conséquent, nécessite un emplacement spécial. Par exemple, une carte à double largeur requiert deux emplacements. Un autre exemple est lorsqu’une carte peut être utilisée pour la même fonction générale que les autres cartes, mais elle nécessite un emplacement spécial (très long, par exemple); en revanche, d’autres cartes peuvent être placées dans n’importe quel emplacement disponible. Si la **valeur est true**, la propriété **RequirementsDescription** correspondante doit spécifier la nature de l’unicité ou de l’objectif de la carte.
...
La classe de la **\_ carte CIM** est dérivée de la [**\_ PhysicalPackage CIM**](cim-physicalpackage.md).

WMI n’implémente pas cette classe. Pour plus d’informations sur les classes dérivées de la **\_ carte CIM**, consultez [classes Win32](win32-provider.md).
