...
La classe **CIM \_ ProtectedSpaceExtent** représente des adresses de blocs logiques adressables, qui sont traitées comme une extension de stockage unique, mais qui se trouvent sur une seule extension physique. Les extensions d’espace protégé excluent tous les blocs logiques mappés en tant que données de vérification et contiennent des informations de mappage de profondeur de répartition des données utilisateur. Une autre possibilité, si la configuration automatique est utilisée, consiste à instancier ou étendre la classe [**CIM \_ AggregatePSExtent**](cim-aggregatepsextent.md) .
...
La classe **CIM \_ ProtectedSpaceExtent** possède les types de membres suivants :
...
La classe **CIM \_ ProtectedSpaceExtent** possède ces méthodes.



| Méthode                                                                          | Description                                                                                                                              |
|:--------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Réinitialiser**](reset-method-in-class-cim-protectedspaceextent.md)                 | Demande la réinitialisation de l’unité logique. Non implémenté par WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-protectedspaceextent.md) | Définit l’état d’alimentation souhaité pour un périphérique logique et lorsqu’un appareil doit être placé dans cet État. Non implémenté par WMI.<br/> |
...
La classe **CIM \_ ProtectedSpaceExtent** possède les propriétés suivantes.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Extension de l’espace protégé DMTF \| 001,6 "), [**unités**](/windows/desktop/WmiSdk/standard-qualifiers) (" octets ")
</dt> </dl>

Pour une **classe \_ ProtectedSpaceExtent CIM** allouée à une [**classe \_ CIM VolumeSet**](cim-volumeset.md) , cette propriété correspond au nombre d’octets de données utilisateur placés dans l’étendue de l’espace protégé avant de passer à l’extension d’espace protégé suivante dans l’ensemble de volumes. Dans le cas contraire, cette extension d’espace protégé est considérée comme non allouée et cette propriété doit être définie sur 0.
...
La classe **CIM \_ ProtectedSpaceExtent** est dérivée de [**CIM \_ StorageExtent**](cim-storageextent.md).
