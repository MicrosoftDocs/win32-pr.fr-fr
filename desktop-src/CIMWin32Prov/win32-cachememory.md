...
ms.locfileid: "119986349"
---
# <a name="win32_cachememory-class"></a>\_Classe CacheMemory Win32

La [classe WMI](/windows/desktop/WmiSdk/retrieving-a-class) **Win32 \_ CacheMemory** représente la mémoire cache interne et externe sur un système informatique.
...
La classe **Win32 \_ CacheMemory** possède les types de membres suivants :
...
La classe **Win32 \_ CacheMemory** possède ces méthodes.



| Méthode            | Description                                                                                                                                                                                                  |
|:------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Réinitialiser**         | Non implémenté. Pour implémenter cette méthode, consultez la méthode [**Reset**](reset-method-in-class-cim-controller.md) dans [**CIM \_ CacheMemory**](cim-cachememory.md) pour obtenir de la documentation.<br/>                 |
| **SetPowerState** | Non implémenté. Pour implémenter cette méthode, consultez la méthode [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) dans [**CIM \_ CacheMemory**](cim-cachememory.md) pour obtenir de la documentation.<br/> |
...
La classe **Win32 \_ CacheMemory** a ces propriétés.
...
Énumération entière qui définit l’associativité du cache système.

Cette valeur provient du membre **associativité** de la structure d' **informations du cache** dans les informations SMBIOS.

Cette propriété est héritée de la [**\_ CacheMemory CIM**](cim-cachememory.md).
...
Cette valeur provient du membre de **configuration du cache** de la structure d’informations du **cache** dans les informations SMBIOS.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) (« SMBIOS \| type 7 \| cache Speed »), [**Units**](/windows/desktop/WmiSdk/standard-qualifiers) (nanosecondes)
</dt> </dl>

Vitesse du cache.

Cette valeur provient du membre **Vitesse** du cache de la structure d' **informations du cache** dans les informations SMBIOS.
...
Cette valeur provient du membre de **type de cache système** de la structure d' **informations du cache** dans les informations SMBIOS.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS \| type 7 \| Current SRAM type")
</dt> </dl>

Tableau de types de mémoire vive (SRAM) statique utilisé pour la mémoire cache.

Cette valeur provient du membre de **type SRAM actuel** de la structure d' **informations du cache** dans les informations SMBIOS.
...
Identificateur unique du cache représenté par une instance de **Win32 \_ CacheMemory**.

Cette propriété est héritée de [**CIM \_ LogicalDevice**](cim-logicaldevice.md).

Exemple : « cache Memory 1 »
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) (« \| type de correction d’erreur de type SMBIOS 7 \| »)
</dt> </dl>

Méthode de correction d’erreur utilisée par la mémoire cache.

Cette valeur provient du membre de **type de correction d’erreur** de la structure d’informations du **cache** dans les informations SMBIOS.
...
Type d’erreur qui s’est produite le plus récemment. Les valeurs 12-14 ne sont pas définies dans le schéma CIM car, dans DMI, elles combinent la sémantique du type d’erreur et si elles sont correctes ou non. Ce dernier est indiqué dans la propriété **CorrectableError**.
...
Durée maximale, en secondes, pendant laquelle les lignes modifiées ou les compartiments peuvent demeurer dans le cache avant d’être vidés. La valeur 0 (zéro) indique qu’un vidage du cache n’est pas contrôlé par un minuteur de vidage.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) (« SMBIOS \| type 7 \| Taille installée »), [**Units**](/windows/desktop/WmiSdk/standard-qualifiers) (« kilo-octets »)
</dt> </dl>

Taille actuelle de la mémoire cache installée.

Cette valeur provient du membre de la **Taille installée** de la structure d' **informations du cache** dans les informations SMBIOS.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS \| type 7 \| location")
</dt> </dl>

Emplacement physique de la mémoire cache.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS \| type 7 \| Max cache size"), [**Units**](/windows/desktop/WmiSdk/standard-qualifiers) ("kilo-octets")
</dt> </dl>

Taille maximale du cache pouvant être installée sur cette mémoire cache particulière.

Cette valeur provient du membre de la **taille de cache maximale** de la structure d' **informations du cache** dans les informations SMBIOS.
...
Chaîne de forme libre fournissant plus d’informations si la propriété **ErrorType** a la valeur 1, other. Dans le cas contraire, cette chaîne n’a aucune signification.
...
Cette valeur provient du membre de **désignation de socket** de la structure d’informations du **cache** dans les informations SMBIOS.
...
Stratégie qui doit être utilisée par le cache pour gérer les demandes de lecture.
...
**Lecture et lecture anticipée** (5)
...
Algorithme permettant de déterminer les lignes ou les compartiments du cache qui doivent être réutilisés.
...
Le **moins récemment utilisé (LRU)** (3)
...
**Premier entré, premier sorti (FIFO)** (4)
...
**Dernier entré, premier sorti (LIFO)** (5)
...
Le **moins fréquemment utilisé (LFU)** (6)
...
Le **plus fréquemment utilisé (MFU)** (7)
...
**Algorithmes multiples dépendants des données** (8)
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS \| type 7 \| prenait en charge le type SRAM")
</dt> </dl>

Tableau de types pris en charge de mémoire vive statique (SRAM) qui peuvent être utilisés pour la mémoire cache.

Cette valeur provient du membre de **type SRAM pris en charge** de la structure d' **informations du cache** dans les informations SMBIOS.
...
Si la **valeur est true**, les informations d’adresse dans la propriété **ErrorAddress** sont une adresse de niveau système. Si la **valeur est false**, il s’agit d’une adresse physique. Si la propriété **errorInfo** est égale à 3, cette propriété n’a aucune signification.
...
Écrire une définition de stratégie.
...
**Varie en fonction de l’adresse** (5)
...
La classe **Win32 \_ CacheMemory** est dérivée de [**CIM \_ CacheMemory**](cim-cachememory.md).
