...
ms.locfileid: "119080293"
---
# <a name="cim_usbhub-class"></a>CIM \_ USBHub (classe)

La classe **CIM \_ USBHub** représente les fonctionnalités et la gestion d’un concentrateur USB.
...
La classe **CIM \_ USBHub** possède les types de membres suivants :
...
La classe **CIM \_ USBHub** possède ces méthodes.



| Méthode                                                            | Description                                                                                                                              |
|:------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**GetDescriptor**](getdescriptor-method-in-class-cim-usbhub.md) | Retourne le descripteur de périphérique USB. Non implémenté par WMI.<br/>                                                                    |
| [**Initialisation**](reset-method-in-class-cim-usbhub.md)                 | Demande la réinitialisation de l’unité logique. Non implémenté par WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-usbhub.md) | Définit l’état d’alimentation souhaité pour un périphérique logique et lorsqu’un appareil doit être placé dans cet État. Non implémenté par WMI.<br/> |
...
La classe **CIM \_ USBHub** possède ces propriétés.
...
Cette propriété est héritée de la [**\_ USBDevice CIM**](cim-usbdevice.md).
...
Qualificateurs : [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**CIM \_ USBDevice**](cim-usbdevice.md).**CurrentConfigValue**")
</dt> </dl>

Paramètres USB alternatifs pour chaque interface dans la configuration actuellement sélectionnée (indiquée par la propriété **CurrentConfigValue** ). Ce tableau possède une entrée pour chaque interface de la configuration. Si la propriété **CurrentConfigValue** a la valeur 0 (zéro), ce qui indique que l’appareil n’est pas configuré, le tableau n’est pas défini. Pour plus d’informations sur l’analyse de cette chaîne d’octets, consultez la spécification USB.
...
Qualificateurs : [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**CIM \_ USBDevice**](cim-usbdevice.md).**CurrentAlternateSettings**")
</dt> </dl>

Configuration actuellement sélectionnée pour l’appareil. Si cette valeur est 0 (zéro), l’appareil n’est pas configuré.
...
Si la **valeur est true**, l’alimentation est basculée simultanément sur tous les ports du concentrateur. Si la **valeur est false**, l’alimentation est commutée individuellement pour chaque port.
...
Nombre de ports en aval sur le concentrateur, y compris ceux incorporés dans le silicium du concentrateur.
...
Version USB la plus récente prise en charge par le périphérique USB. Cette propriété est exprimée sous la forme d’une valeur décimale codée binaire (BCD) où une virgule décimale est impliquée entre le deuxième et le troisième chiffre. Par exemple, la valeur 0x201 indique que la version 2,01 est prise en charge.
...
La **classe \_ CIM CIM** est dérivée de la [**\_ USBDevice CIM**](cim-usbdevice.md).

WMI n’implémente pas cette classe. Pour les classes WMI dérivées de **CIM \_ USBHub**, consultez [classes Win32](win32-provider.md).
