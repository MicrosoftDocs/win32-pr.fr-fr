...
ms.locfileid: "118678839"
---
# <a name="cim_pcmciacontroller-class"></a>\_Classe CIM PCMCIAController

La classe **CIM \_ PCMCIAController** représente les fonctionnalités et la gestion d’un contrôleur PCMCIA (Personal Computer Memory Card International Association).
...
La classe **CIM \_ PCMCIAController** possède les types de membres suivants :
...
La classe **CIM \_ PCMCIAController** possède ces méthodes.



| Méthode                                                                      | Description                                                                                                                              |
|:----------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Initialisation**](reset-method-in-class-cim-pcmciacontroller.md)                 | Demande la réinitialisation de l’unité logique. Non implémenté par WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-pcmciacontroller.md) | Définit l’état d’alimentation souhaité pour un périphérique logique et lorsqu’un appareil doit être placé dans cet État. Non implémenté par WMI.<br/> |
...
La classe **CIM \_ PCMCIAController** possède les propriétés suivantes.
...
Disponibilité et état de l’appareil. Cette propriété est héritée de [**CIM \_ LogicalDevice**](cim-logicaldevice.md).
...
La méthode [**SetPowerState**](setpowerstate-method-in-class-cim-pcmciacontroller.md) est prise en charge.
...
La méthode [**SetPowerState**](setpowerstate-method-in-class-cim-pcmciacontroller.md) peut être appelée avec le paramètre *PowerState* défini sur 5 (« Power cycle »).
...
La méthode [**SetPowerState**](setpowerstate-method-in-class-cim-pcmciacontroller.md) peut être appelée avec le paramètre *PowerState* défini sur 5 (« Power cycle ») et le paramètre *Time* défini sur une date et une heure spécifiques, ou un intervalle, pour la mise sous tension.
...
Date et heure de la dernière réinitialisation de ce contrôleur, ce qui signifie que le contrôleur a été mis hors tension ou réinitialisé.
...
La classe **CIM \_ PCMCIAController** est dérivée [**du \_ contrôleur CIM**](cim-controller.md).

WMI n’implémente pas cette classe. Pour les classes WMI dérivées de **CIM \_ PCMCIAController**, consultez [classes Win32](win32-provider.md).
