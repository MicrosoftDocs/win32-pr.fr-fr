...
ms.locfileid: "118677044"
---
# <a name="cim_volumeset-class"></a>\_Classe CIM VolumeSet

La classe **CIM \_ VolumeSet** représente une plage contiguë de blocs logiques présentée à l’environnement d’exploitation pour la lecture et l’écriture de données utilisateur. Les jeux de volumes ne doivent pas se chevaucher les uns les autres et se baser sur une ou plusieurs étendues physiques, sur des extensions d’espace protégé ou sur des étendues d’agrégat (tous du même type). Les associations de base doivent être instanciées ou sous-classées en fonction des besoins.
...
La classe **CIM \_ VolumeSet** possède les types de membres suivants :
...
La classe **CIM \_ VolumeSet** possède ces méthodes.



| Méthode                                                               | Description                                                                                                                              |
|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Initialisation**](reset-method-in-class-cim-volumeset.md)                 | Demande la réinitialisation de l’unité logique. Non implémenté par WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-volumeset.md) | Définit l’état d’alimentation souhaité pour un périphérique logique et lorsqu’un appareil doit être placé dans cet État. Non implémenté par WMI.<br/> |
...
La classe **CIM \_ VolumeSet** possède les propriétés suivantes.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Groupe de volumes DMTF \| 001,5 ")
</dt> </dl>

Nombre d’étendues d’espace protégé à entrelacer comme un ensemble collectif. Dans SCC, cette valeur est définie comme le nombre de bandes à compter avant de continuer à mapper dans le jeu d’étendues contigu suivant, au-delà de la bande actuelle.
...
Qualificateurs : [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Groupe de volumes DMTF \| 001,4 ")
</dt> </dl>

Nombre d’extensions d’espace protégé contiguës comptabilisées avant de revenir à la première extension d’espace protégé de l’agrégat actuel. Il s’agit du nombre d’étendues formant la répartition des données utilisateur.
...
La classe **CIM \_ VolumeSet** est dérivée de [**CIM \_ StorageExtent**](cim-storageextent.md).
