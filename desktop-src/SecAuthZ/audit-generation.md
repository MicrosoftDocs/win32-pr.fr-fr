...
Pour lire ou écrire la liste SACL d’un objet, un thread doit d’abord activer le \_ privilège de nom de sécurité se \_ . Pour plus d’informations, consultez [droit d’accès SACL](sacl-access-right.md).

L’API Windows fournit également une prise en charge pour les applications serveur afin de générer des messages d’audit lorsqu’un client tente d’accéder à un objet privé. Pour plus d’informations, consultez [audit de l’accès aux objets privés](auditing-access-to-private-objects.md).
