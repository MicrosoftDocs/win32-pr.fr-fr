...
> [!IMPORTANT]
> Les profils listés ci-dessous utilisent tous la version 8 Windows Media Audio et les codecs de Windows Media Video. Il n’existe pas de profils système prédéfinis qui utilisent les codecs Windows Media 9 Series. Vous pouvez créer votre propre profil de la série Windows Media 9 en utilisant un profil de version 8 comme point de départ. Pour plus d’informations, consultez [réutilisation des configurations de flux](reusing-stream-configurations.md).
...
| Windows Media Video 8 pour les modems d’accès à distance ou RNIS à canal unique (28,8 à 56 Kbps) | WMProfile \_ V80 \_ 28856VideoMBR  | Utilisez ce profil à vitesse de transmission multiple pour les audiences cibles avec des modems d’accès à distance ou des connexions RNIS à canal unique (la bande passante est comprise entre 28,8 kbit/s et 56 kbit/s).        |
| Windows Media Video 8 pour réseau local, modem câble ou xDSL (100 à 768 Kbps)             | WMProfile \_ V80 \_ 100768VideoMBR | Utilisez ce profil à vitesse de transmission multiple pour les audiences cibles avec des connexions RNIS, LAN, modem câble ou xDSL à double canal (la bande passante est comprise entre 100 Kbit/s et 500 kbit/s). |
...
| Windows Media Audio 8 pour modem d’accès à distance (stéréo radio FM, 28,8 Kbits/s)              | WMProfile \_ V80 \_ 288StereoAudio | Utilisez ce profil pour le contenu radio et musique (audio uniquement).                                                                                                          |
| Windows Media Audio 8 pour le modem d’accès à distance (32 Kbits/s)                                 | WMProfile \_ V80 \_ 32StereoAudio  | Utilisez ce profil pour le contenu radio et musique (audio uniquement).                                                                                                          |
...
| VBR basé sur la qualité Windows Media 8 pour le haut débit                              | WMProfile \_ V80 \_ FAIRVBRVideo   | Profil de qualité équitable à haute qualité pour le contenu VBR dont la qualité est restreinte.                                                                                     |
| VBR haute qualité Windows Media 8 pour le haut débit.                             | WMProfile \_ V80 \_ HIGHVBRVideo   | Profil le plus élevé à la meilleure qualité pour le contenu VBR dont la qualité est restreinte.                                                                                     |
| Le VBR Windows Media 8 est recommandé pour la haute qualité.                             | WMProfile \_ V80 \_ BESTVBRVideo   | Meilleur profil basé sur la qualité pour le contenu VBR dont la qualité est restreinte.                                                                                             |
