...
Le writer est conçu pour gérer la taille des paquets en interne. Toutefois, vous pouvez avoir des exigences spécifiques pour votre application qui appellent un contrôle manuel sur la taille des paquets dans les fichiers ASF que vous écrivez. Le kit de développement logiciel (SDK) du format Windows Media fournit deux interfaces, [**IWMPacketSize**](/previous-versions/windows/desktop/api/wmsdkidl/nn-wmsdkidl-iwmpacketsize) et [**IWMPacketSize2**](/previous-versions/windows/desktop/api/wmsdkidl/nn-wmsdkidl-iwmpacketsize2) , qui vous permettent de contrôler la taille maximale et minimale des paquets.
