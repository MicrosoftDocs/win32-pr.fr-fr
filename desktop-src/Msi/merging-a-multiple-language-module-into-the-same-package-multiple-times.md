...
Lors de la fusion d’un module plusieurs fois dans un seul fichier de .msi, vous devrez peut-être modifier l’ordre des fichiers dans la [table de fichiers](file-table.md) pour utiliser les .cab existants à partir du module directement dans votre installation. L’ordre des fichiers dans la table de fichiers doit correspondre à l’ordre des fichiers dans le .cab. Lors de la fusion d’un module plusieurs fois dans une base de données d’installation, la séquence peut être modifiée, car les fichiers partagés entre les langues peuvent déjà exister dans le module à partir d’une fusion antérieure, et ils ont un numéro de séquence relatif différent.
