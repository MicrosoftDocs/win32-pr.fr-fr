---
description: pour installer efficacement vos applications et réduire le coût total de possession (TCO) de vos clients, vous pouvez utiliser la Windows Installer.
ms.assetid: fdeb8cc2-af4e-419d-a234-6d86061219fe
title: à propos de Windows Installer
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: f4834370107c7a8ce70145634e7549583dfee836
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 09/13/2021
ms.locfileid: "127092917"
---
# <a name="about-windows-installer"></a>à propos de Windows Installer

pour installer efficacement vos applications et réduire le coût total de possession (TCO) de vos clients, vous pouvez utiliser la Windows Installer. Cette section traite des principales zones fonctionnelles du programme d’installation :

-   [vue d’ensemble de Windows Installer](overview-of-windows-installer.md)
-   [Installation administrative](administrative-installation.md)
-   [Restauration de l’installation](rollback-installation.md)
-   [Installation de maintenance](maintenance-installation.md)
-   [Windows Extensions de fichier du programme d’installation](windows-installer-file-extensions.md)
-   [Options de ligne de commande](command-line-options.md)
-   [Redémarrages du système](system-reboots.md)
-   [Stratégie système](system-policy.md)
-   [Résilience source](source-resiliency.md)
-   [utilisation de Windows Installer et Protection des ressources Windows](windows-resource-protection-on-windows-vista.md)
-   [Points de restauration système et Windows Installer](system-restore-points-and-the-windows-installer.md)
-   [Règles de contrôle de version des fichiers](file-versioning-rules.md)
-   [Codes de produit](product-codes.md)
-   [Codes de package](package-codes.md)
-   [Fusions et transformations](merges-and-transforms.md)
-   [Composants qualifiés](qualified-components.md)
-   [Windows Journalisation du programme d’installation](windows-installer-logging.md)
-   [Fichiers auxiliaires](companion-files.md)
-   [Composants isolés](isolated-components.md)
-   [Contexte d’installation](installation-context.md)

## <a name="related-topics"></a>Rubriques connexes

<dl> <dt>

[Versions, outils et redistribuables publiés](released-versions-tools-and-redistributables.md)
</dt> </dl>

 

 



