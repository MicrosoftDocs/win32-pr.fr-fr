...
La table IniLocator contient les informations nécessaires pour rechercher un fichier ou un répertoire à l’aide d’un fichier. ini ou pour rechercher une entrée. ini particulière. Le fichier. ini doit être présent dans le répertoire par défaut de Microsoft Windows.
...
Nom de la section dans le fichier. ini.
...
Champ de la ligne. ini. Si le champ a la valeur null ou est égal à 0, la ligne entière est lue. Il doit s’agir d’un nombre non négatif.
...
Valeur qui détermine si la valeur. ini est un emplacement de fichier, un emplacement de répertoire ou une valeur RAW. ini.
...
| **msidbLocatorTypeRawValue**  | 0x002       | 2       | Valeur RAW. ini.     |
