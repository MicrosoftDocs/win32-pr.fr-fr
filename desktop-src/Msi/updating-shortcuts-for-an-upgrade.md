---
description: L’exemple de package de mise à niveau met à jour les raccourcis vers les nouveaux fichiers et fonctionnalités.
ms.assetid: 5a12932d-053d-49c3-9f06-0c122ca293d0
title: Mise à jour des raccourcis pour une mise à niveau
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 9a3192bc9468dbe84c2968a968631b4a091b0705
ms.sourcegitcommit: 831e8f3db78ab820e1710cede244553c70e50500
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 01/07/2021
ms.locfileid: "106514812"
---
# <a name="updating-shortcuts-for-an-upgrade"></a>Mise à jour des raccourcis pour une mise à niveau

L’exemple de package de mise à niveau met à jour les raccourcis vers les nouveaux fichiers et fonctionnalités.

Utilisez votre éditeur de base de données pour ouvrir MNP2001.msi et entrez les données suivantes dans le [tableau de raccourcis](shortcut-table.md).



| Raccourci    | Répertoire\_ | Nom           | -\_ | Cible               | Arguments | Description | Touche d’accès rapide | Icône\_         | IndexIcône | ShowCmd | WkDir |
|-------------|-------------|----------------|-------------|----------------------|-----------|-------------|--------|----------------|-----------|---------|-------|
| sBaseball   | MENUDIR     | Baseba01.txt   | Chaussures    | Chaussures             |           |             |        | \_icon.exe Orca |           |         |       |
| sConcert    | MENUDIR     | Concer01.txt   | Concert     | \[\#Concer01.txt\]   |           |             |        |                |           |         |       |
| sDancell    | MENUDIR     | Dance01.txt    | Jongl       | \[\#Dance01.txt\]    |           |             |        |                |           |         |       |
| sFootball   | MENUDIR     | Footba01.txt   | Terrain    | \[\#Footba01.txt\]   |           |             |        |                |           |         |       |
| sHelp       | MENUDIR     | Help.txt       | Aide        | \[\#Help.txt\]       |           |             |        |                |           |         |       |
| sJanuary    | MENUDIR     | Januar01.txt   | Janvier     | \[\#Januar01.txt\]   |           |             |        |                |           |         |       |
| sNewYears   | MENUDIR     | NewYea01.txt   | NewYears    | \[\#NewYea01.txt\]   |           |             |        |                |           |         |       |
| sNotepad    | MENUDIR     | Redpark.exe    | Bloc-notes     | \[\#Redpark.exe\]    |           |             |        |                |           |         |       |
| sReadme     | MENUDIR     | Lisezmoi.txt     | Bloc-notes     | \[\#Readme.txt\]     |           |             |        |                |           |         |       |
| sMemorial   | MENUDIR     | Memori01.txt   | Souvenir    | \[\#Memori01.txt\]   |           |             |        |                |           |         |       |
| sBasketball | MENUDIR     | Basketba01.txt | Isole  | \[\#Basketba01.txt\] |           |             |        |                |           |         |       |
| sOpera      | MENUDIR     | Opera01.txt    | Opera       | \[\#Opera01.txt\]    |           |             |        |                |           |         |       |



 

[Continuer](updating-upgrade-table-for-an-upgrade.md)

 

 



