...
1.  À partir de la ligne de commande, utilisez le chemin d’accès **msiexec/fvomus \[** _pour mettre à jour le fichier .msi_ *_\]_* ou le chemin d’accès **msiexec/i \[** _à updated .msi file_ *_\] REINSTALL = ALL REINSTALLMODE = vomus_*.
...
2.  À partir de l’invite de commandes, utilisez : **\[ msiexec/i** _chemin d’accès mis à jour .msi fichier_ *_\] REINSTALL = \[_*_Feature List \]_ **REINSTALLMODE = vomus**.
