...
-   [À propos de l’aide](#about-help)
    ...
    -   [aide sur l’affichage et l’aide Windows](#help-display-and-windows-help)
...
### <a name="help-display-and-windows-help"></a>aide sur l’affichage et l’aide Windows

Une fois qu’une application reçoit une demande d’aide, elle doit afficher les informations d’aide appropriées. étant donné que le Windows aide de l’application fournit une interface utilisateur cohérente, il est recommandé que les applications utilisent Windows aide plutôt que d’autres méthodes. pour indiquer à Windows aide d’afficher des informations d’aide, une application utilise la fonction [**WinHelp**](/windows/desktop/api/Winuser/nf-winuser-winhelpa) , en spécifiant des détails tels que les informations à afficher et la forme de la fenêtre dans laquelle l’afficher. Les sections suivantes expliquent comment utiliser **WinHelp** pour afficher des informations d’aide.

### <a name="help-files"></a>fichiers d'aide

Pour afficher les informations d’aide, vous devez spécifier un fichier d’aide lors de l’appel de la fonction [**WinHelp**](/windows/desktop/api/Winuser/nf-winuser-winhelpa) . le fichier d’aide doit avoir le format de fichier d’aide de Windows (. hlp) et une ou plusieurs rubriques. Chaque rubrique est une unité d’information distincte, telle qu’une description conceptuelle, un ensemble d’instructions, une image, une définition de Glossaire, et ainsi de suite. les rubriques doivent être identifiées de manière unique afin que Windows aide puissent les localiser chaque fois qu’elles sont demandées. en interne, Windows aide utilise des identificateurs de rubrique pour localiser des rubriques, mais les applications utilisent souvent des identificateurs de contexte (valeurs entières uniques) pour spécifier les rubriques à afficher. L’auteur du fichier d’aide doit mapper explicitement les identificateurs de contexte aux identificateurs de rubrique dans la \[ \] section de mappage du fichier projet utilisé pour générer le fichier d’aide.
...
Vous pouvez permettre à l’utilisateur de rechercher et d’afficher des rubriques en affectant des mots clés aux rubriques du fichier d’aide. Un mot clé est simplement une chaîne associée à une ou plusieurs rubriques. Windows L’aide collecte tous les mots clés dans un fichier d’aide, les place dans un tableau et les affiche dans la liste index de la boîte de dialogue **rubriques d’aide** . lorsque l’utilisateur sélectionne un mot clé, Windows aide affiche la rubrique d’aide associée ou, si plusieurs rubriques sont associées au mot clé, affiche une liste des rubriques à partir desquelles l’utilisateur peut choisir.
...
par défaut, Windows aide ne reconnaît que le mot clé table identifié par le caractère de note de bas de page K dans le fichier source d’aide. vous pouvez diriger Windows aide pour créer des tables de mots clés supplémentaires en ajoutant un caractère de note de bas de page autre que K, avec la définition de mot clé, dans le fichier source d’aide. (Toutefois, le caractère de note de bas de page A est réservé.) Vous devez définir des tables de mots clés supplémentaires à l’aide des instructions relations dans la \[ \] section Options du fichier projet lors de la génération du fichier d’aide.

une application peut utiliser la \_ commande HELP SETINDEX avec la fonction [**WinHelp**](/windows/desktop/api/Winuser/nf-winuser-winhelpa) pour diriger Windows aide pour afficher une table de mots clés autre que K dans sa liste d’Index. pour indiquer à Windows aide de rechercher un mot clé dans une autre table de mots clés, une application peut utiliser la \_ commande Help relations. Vous spécifiez le mot clé et la table des mots clés dans une structure [**MULTIKEYHELP**](/windows/win32/api/winuser/ns-winuser-multikeyhelpa) , que vous transmettez à **WinHelp**.
...
une application lance l’instance de carte de formation de Windows aide en appelant la fonction [**WinHelp**](/windows/desktop/api/Winuser/nf-winuser-winhelpa) et en spécifiant la \_ commande help TCARD en combinaison avec une autre commande, telle que le contexte d’aide \_ . par la suite, quand l’utilisateur clique sur un bouton dans la carte de formation, clique sur une zone réactive assignée à la macro TCard ou ferme la carte de formation, Windows aide à l’application en lui envoyant un message [**WM \_ TCard**](wm-tcard.md) . Le paramètre *wParam* identifie le bouton ou l’action de l’utilisateur, et le paramètre *lParam* contient des données supplémentaires, dont la signification dépend de la valeur de *wParam*.

### <a name="canceling-help"></a>Annulation de l’aide

Windows L’aide requiert qu’une application annule explicitement l’aide pour pouvoir libérer toutes les ressources utilisées pour effectuer le suivi de l’application et de ses fichiers d’aide. L’application peut le faire à tout moment en appelant la fonction [**WinHelp**](/windows/desktop/api/Winuser/nf-winuser-winhelpa) et en spécifiant la \_ commande help Quit. notez que cela n’est pas vrai pour l’instance contextuelle de Windows aide. Une application ne doit pas essayer de fermer une instance contextuelle.

Si une application a effectué des appels à [**WinHelp**](/windows/desktop/api/Winuser/nf-winuser-winhelpa), elle doit annuler l’aide avant de fermer sa fenêtre principale (par exemple, en réponse au \_ message WM Destroy dans la procédure de fenêtre principale). Une application doit appeler **WinHelp** une seule fois pour annuler l’aide, quel que soit le nombre de fichiers d’aide ouverts. Windows L’aide reste en cours d’exécution jusqu’à ce que toutes les applications ou dll aient annulé l’aide.

pour fermer l’instance de la carte de formation d’Windows aide, vous devez spécifier les \_ commandes help TCARD et help \_ QUIT lors de l’appel de [**WinHelp**](/windows/desktop/api/Winuser/nf-winuser-winhelpa). une application n’a pas besoin d’annuler l’instance de la carte de formation de Windows aide si l’utilisateur l’annule en premier. Windows Permet de notifier une application lorsque l’utilisateur annule l’instance de la carte de formation en envoyant le message [**WM \_ TCARD**](wm-tcard.md) avec le paramètre *wParam* défini sur IDCLOSE.
...
grâce à la commande **help \_ CONTEXTMENU** , Windows aide à afficher un menu avant d’afficher la rubrique d’aide. Le menu est défini par le système. Elle permet à l’utilisateur d’afficher de l’aide pour le contrôle ou d’afficher la boîte de dialogue **rubriques d’aide** .
