...
Identifie la valeur du paramètre *pwszAssoc* de [**IQueryAssociations :: init**](/windows/win32/api/shlwapi/nf-shlwapi-iqueryassociations-init) comme un nom de fichier exécutable. Si cet indicateur n’est pas défini, la clé racine est définie sur le ProgID associé à la clé **. exe** au lieu du ProgID du fichier exécutable.
...
Fait en sorte que les méthodes [**IQueryAssociations**](/windows/win32/api/shlwapi/nn-shlwapi-iqueryassociations) ignorent Rundll.exe et retournent des informations sur sa cible. En général, les méthodes **IQueryAssociations** retournent des informations sur le premier fichier. exe ou. dll dans une chaîne de commande. Si une commande utilise Rundll.exe, la définition de cet indicateur indique à la méthode d’ignorer Rundll.exe et de retourner des informations sur sa cible.
...
**Introduit dans Windows 7**. Spécifie que le ProgID « inconnu » doit être ignoré ; au lieu de cela, échoue.

 <span id="ASSOCF_INIT_FIXED_PROGID"></span><span id="assocf_init_fixed_progid"></span>**ASSOCF \_ init \_ ( \_ ProgID fixe)** 

**Introduit dans Windows 8**. Spécifie que le ProgID fourni doit être mappé à l’aide des valeurs par défaut du système, plutôt que les valeurs par défaut de l’utilisateur actuel.
...
|-------------------------------------|--------------------------------------------------------------------------------------|
| Client minimal pris en charge | Windows 2000 professionnel, applications de \[ Bureau Windows XP uniquement\]               |
