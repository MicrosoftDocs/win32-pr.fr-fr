...
Pour les éléments du panneau de configuration qui sont implémentés en tant que fichiers. exe, aucune exportation spéciale ou gestion des messages n’est requise. Tout fichier. exe peut être enregistré en tant qu’objet de commande pour s’afficher avec un point d’entrée dans le dossier du panneau de configuration.

Un exemple est utilisé ici pour illustrer les exigences d’inscription. L’exemple montre comment inscrire un élément du panneau de configuration appelé **mes paramètres** comme objet de commande afin qu’il apparaisse dans la fenêtre du panneau de configuration. La fenêtre **mes paramètres** s’affiche également lorsque la commande `MyApp.exe /settings` est exécutée.
...
-   **System. ControlPanel. Category**. PRIX \_ SZ Valeur qui déclare les catégories du panneau de configuration où l’élément apparaît. Plusieurs catégories sont séparées par des virgules. Dans le cas de l’exemple ci-dessus, l’entrée indique que l’élément **mes paramètres** doit apparaître à la fois dans les catégories **apparence et personnalisation** et **programmes** . Consultez [assignation de catégories du panneau de configuration](assigning-control-panel-categories.md) pour connaître les valeurs de catégorie possibles.
