---
title: Modifier les messages de contrôle
description: Modifier les messages de contrôle
ms.assetid: b5c12b71-b7c3-4331-a656-15718121ddf4
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 0d57fcf4077b74217140b201cf3ac95bfc620912
ms.sourcegitcommit: 92e74c99f8f4d097676959d0c317f533c2400a80
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 03/09/2021
ms.locfileid: "103869866"
---
# <a name="edit-control-messages"></a>Modifier les messages de contrôle

## <a name="in-this-section"></a>Dans cette section

-   [**EM \_ CANUNDO**](em-canundo.md)
-   [**\_CHARFROMPOS em**](em-charfrompos.md)
-   [**\_EMPTYUNDOBUFFER em**](em-emptyundobuffer.md)
-   [**\_FMTLINES em**](em-fmtlines.md)
-   [**\_GETCUEBANNER em**](em-getcuebanner.md)
-   [**\_GETFIRSTVISIBLELINE em**](em-getfirstvisibleline.md)
-   [**EM \_ GETHANDLE**](em-gethandle.md)
-   [**\_GETHILITE em**](em-gethilite.md)
-   [**\_GETIMESTATUS em**](em-getimestatus.md)
-   [**\_GETLIMITTEXT em**](em-getlimittext.md)
-   [**\_GETLINE em**](em-getline.md)
-   [**\_GETLINECOUNT em**](em-getlinecount.md)
-   [**\_GETMARGINS em**](em-getmargins.md)
-   [**\_GETMODIFY em**](em-getmodify.md)
-   [**\_GETPASSWORDCHAR em**](em-getpasswordchar.md)
-   [**\_GETRECT em**](em-getrect.md)
-   [**\_GETSEL em**](em-getsel.md)
-   [**\_GETTHUMB em**](em-getthumb.md)
-   [**\_GETWORDBREAKPROC em**](em-getwordbreakproc.md)
-   [**\_HIDEBALLOONTIP em**](em-hideballoontip.md)
-   [**\_LIMITTEXT em**](em-limittext.md)
-   [**\_LINEFROMCHAR em**](em-linefromchar.md)
-   [**\_LINEINDEX em**](em-lineindex.md)
-   [**\_LINELENGTH em**](em-linelength.md)
-   [**\_LINESCROLL em**](em-linescroll.md)
-   [**\_NOSETFOCUS em**](em-nosetfocus.md)
-   [**\_POSFROMCHAR em**](em-posfromchar.md)
-   [**\_REPLACESEL em**](em-replacesel.md)
-   [**\_défilement em**](em-scroll.md)
-   [**\_SCROLLCARET em**](em-scrollcaret.md)
-   [**\_SETCUEBANNER em**](em-setcuebanner.md)
-   [**\_SETHANDLE em**](em-sethandle.md)
-   [**\_SETHILITE em**](em-sethilite.md)
-   [**\_SETIMESTATUS em**](em-setimestatus.md)
-   [**\_SETLIMITTEXT em**](em-setlimittext.md)
-   [**\_setMargins em**](em-setmargins.md)
-   [**\_SETMODIFY em**](em-setmodify.md)
-   [**\_SETPASSWORDCHAR em**](em-setpasswordchar.md)
-   [**\_SETREADONLY em**](em-setreadonly.md)
-   [**\_SETRECT em**](em-setrect.md)
-   [**\_SETRECTNP em**](em-setrectnp.md)
-   [**\_SETSEL em**](em-setsel.md)
-   [**\_SETTABSTOPS em**](em-settabstops.md)
-   [**\_SETWORDBREAKPROC em**](em-setwordbreakproc.md)
-   [**\_SHOWBALLOONTIP em**](em-showballoontip.md)
-   [**\_TAKEFOCUS em**](em-takefocus.md)
-   [**\_Effacer em**](em-undo.md)
-   [**désannulation WM \_**](wm-undo.md)

 

 




