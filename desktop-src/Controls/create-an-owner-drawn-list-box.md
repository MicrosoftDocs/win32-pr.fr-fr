...
Cette rubrique montre comment implémenter une zone de liste owner-drawn.

L’exemple de code C++ de cette rubrique montre comment dessiner une zone de liste qui contient cinq éléments owner-drawn : quatre implémentations de dessin et une fourche. Chaque élément de liste apparaît sous la forme d’une image bitmap suivie du nom de l’objet. Un bouton invite l’utilisateur à sélectionner un élément qui n’est pas le même que les autres. Le choix du bouton avec la fourche sélectionnée affiche un « vous êtes correct ! » et ferme la boîte de dialogue. Si vous sélectionnez le bouton avec un autre élément de liste, vous affichez « réessayer ! ». « Hello World ! ».

La zone de liste présente les styles [**\_ HASSTRINGS**](list-box-styles.md) [**\_ OWNERDRAWFIXED**](list-box-styles.md) et lbs, en plus des styles de zone de liste standard.
...
Pour initialiser une zone de liste owner-drawn, votre application doit charger la chaîne de texte et le bitmap associé pour chaque élément de zone de liste.

Dans l’exemple de code C++ suivant, la procédure de la boîte de dialogue initialise la zone de liste, **IDC \_ Listing \_ Stuff**, en envoyant le message [**lb \_ ADDSTRING**](lb-addstring.md) pour définir le texte, puis envoie le message [**\_ SETITEMDATA lb**](lb-setitemdata.md) pour associer un bitmap à chaque élément de zone de liste. Le code définit également la hauteur de chaque élément de zone de liste en traitant le message [**WM \_ MEASUREITEM**](wm-measureitem.md) et dessine le texte et l’image bitmap pour chaque élément en traitant le message [**WM \_ DRAWITEM**](wm-drawitem.md) .
