---
title: Comment faire défiler une bitmap
description: Cette section décrit les modifications que vous pouvez apporter à la procédure de fenêtre principale d’une application pour permettre à l’utilisateur de faire défiler une bitmap.
...
Cette section décrit les modifications que vous pouvez apporter à la procédure de fenêtre principale d’une application pour permettre à l’utilisateur de faire défiler une bitmap.
...
Lorsque le message [**WM \_ Create**](/windows/desktop/winmsg/wm-create) est traité, les variables requises pour le défilement sont initialisées. Utilisez la fonction [**CreateCompatibleDC**](/windows/desktop/api/wingdi/nf-wingdi-createcompatibledc) pour créer un contexte de périphérique (DC) compatible, la fonction [**CreateBitmap**](/windows/desktop/api/wingdi/nf-wingdi-createbitmap) pour créer une image bitmap et la fonction [**SelectObject**](/windows/desktop/api/wingdi/nf-wingdi-selectobject) pour sélectionner la bitmap du contrôleur de l’appareil. Notez qu’un contrôleur de périphérique compatible est également appelé contrôleur de périphérique de mémoire.

Les informations spécifiques à l’appareil sur le périphérique d’affichage sont récupérées. Si un contrôleur de périphérique compatible est créé pour l’écran, comme dans l’exemple, utilisez la fonction [**GetDeviceCaps**](/windows/desktop/api/wingdi/nf-wingdi-getdevicecaps) pour obtenir ces informations. Les informations incluent le nombre de bits de couleur adjacents par pixel, le nombre de plans de couleur, ainsi que la hauteur et la largeur du DC.

### <a name="processing-the-wm_size-message"></a>Traitement du \_ message de taille WM

Le traitement du message de [**\_ taille WM**](/windows/desktop/winmsg/wm-size) requiert l’ajustement de la plage et de la position de défilement, afin qu’il reflète les dimensions de la zone cliente et l’image bitmap qui sera affichée.
...
Lorsque les messages [**WM \_ HSCROLL**](wm-hscroll.md) et [**WM \_ VSCROLL**](wm-vscroll.md) sont traités, le code de demande de la barre de défilement est examiné et la position de défilement est définie sur une nouvelle valeur qui reflète l’action de défilement de l’utilisateur. Si la position de défilement est comprise dans la plage de défilement, la fenêtre défile vers la nouvelle position à l’aide de la fonction [**ScrollWindow**](/windows/desktop/api/Winuser/nf-winuser-scrollwindow) . La position de la case de défilement est ensuite ajustée à l’aide de la fonction [**SetScrollInfo**](/windows/desktop/api/Winuser/nf-winuser-setscrollinfo) .

Une fois qu’une fenêtre a été défilée, une partie de sa zone cliente est rendue non valide. Pour vous assurer que la région non valide est mise à jour, utilisez la fonction [**UpdateWindow**](/windows/desktop/api/winuser/nf-winuser-updatewindow) pour générer un message de [**\_ peinture WM**](/windows/desktop/gdi/wm-paint) . Lors du traitement du message **WM \_ Paint** , une application doit repeindre la région non valide au bas de la zone cliente. Lors du défilement ou du redimensionnement de la zone cliente, l’exemple utilise la fonction [**BitBlt**](/windows/desktop/api/wingdi/nf-wingdi-bitblt) pour copier la partie appropriée de l’image bitmap vers la partie non valide de la zone cliente.

## <a name="example-of-scrolling-a-bitmap"></a>Exemple de défilement d’une image bitmap

L’exemple suivant permet à l’utilisateur de capturer le contenu de l’écran dans une image bitmap et de faire défiler la bitmap dans la zone cliente. Le contenu de l’écran est capturé quand l’utilisateur clique avec le bouton droit sur la zone cliente.
