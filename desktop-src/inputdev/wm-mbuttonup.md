...
Publié lorsque l’utilisateur relâche le bouton central de la souris alors que le curseur se trouve dans la zone cliente d’une fenêtre. Si la souris n’est pas capturée, le message est publié dans la fenêtre sous le curseur. Dans le cas contraire, le message est publié dans la fenêtre qui a capturé la souris.
...
Notez que lorsqu’un menu contextuel est présent (affiché), les coordonnées sont relatives à l’écran, et non à la zone cliente. Étant donné que [**TrackPopupMenu**](/windows/desktop/api/winuser/nf-winuser-trackpopupmenu) est un appel asynchrone et que la notification **WM \_ MBUTTONUP** n’a pas d’indicateur spécial indiquant une dérivation de coordonnée, une application ne peut pas déterminer si les coordonnées x, y contenues dans *lParam* sont relatives à l’écran ou à la zone cliente.
