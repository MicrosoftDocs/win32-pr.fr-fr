...
---
# <a name="tunnel-method-api-call-sequence"></a>Séquence d’appel de l’API de méthode de tunnel

Cette rubrique couvre la séquence d’appels d’API pour les méthodes de tunnel

## <a name="tunnel-method-call-sequence-overview"></a>Vue d’ensemble de la séquence d’appels de méthode de tunnel
...
## <a name="user-interaction-with-the-tunnel-methods-api-call-flow"></a>Interaction de l’utilisateur avec le workflow de l’API méthodes de tunnel
...
-   La méthode tunnel EAP retourne le code d’action pour appeler l’interface utilisateur sur EapHost. Le demandeur appelle [**EapHostPeerGetUIContext**](/previous-versions/windows/desktop/api/eappapis/nf-eappapis-eaphostpeergetuicontext)pour obtenir les informations de contexte de l’interface utilisateur actuelle pour la boîte de dialogue de l’interface utilisateur.
